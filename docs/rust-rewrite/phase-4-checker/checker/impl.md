...
- [ ] 推断深化：contravariant 候选 + 优先级 lattice、mapped/conditional/template 推断、上下文-return 推断（C-B3）、回调返回 `U` 推断 + `inferFromSignatures`（C-B3）、lazy `InferenceTypeMapper` + 实例化缓存　`// Go: inference.go`（C-B3/C-C）
