...
- **transformer 消费**（`serializeTypeReferenceNode` 对 `TypeWithConstructSignatureAndValue` 发射 `serializeEntityNameAsExpression`（类标识符）作 `design:type`）：是 P5/P6 协调 transformer 轮的活。blocked-by: 一个同时改 checker（已就绪）+ transformers（`serialize_type_node_for_metadata` 的 `TypeReference` 臂改为调本方法、并据 kind 发射 entity / 构造器标识符）的协调跨-crate lane。
