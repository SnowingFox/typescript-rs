...
| `internal/checker/core/contextual.rs`（4bj 新；4bk 扩） | `ContextFlags`(pub(crate)) + `get_contextual_type`(var-decl 初始化器臂 + binary `=` RHS 臂 + **4bk** PropertyAssignment/ShorthandPropertyAssignment 臂 + ArrayLiteralExpression 臂；余 DEFER)/`get_contextual_type_for_initializer_expression`/`get_contextual_type_for_variable_like_declaration`/`get_contextual_type_for_binary_operand`/`get_contextual_type_for_assignment_expression`(Identifier 目标) + **4bk** `get_contextual_type_for_object_literal_element`/`get_contextual_type_for_array_literal_element`/`get_contextual_type_for_element_expression`/`get_type_of_property_of_contextual_type`/`get_type_from_index_infos_of_contextual_type`/`get_widened_literal_like_type_for_contextual_type`(pub(crate))/`is_literal_of_contextual_type`(pub(crate)) + `get_apparent_type_of_contextual_type`(薄包装)/`get_contextual_signature`/`get_contextual_call_signature`/`is_arity_smaller`/`get_contextually_typed_parameter_type`/`assign_contextual_parameter_types`/`try_get_type_at_position`/`contextually_check_function_expression` + 自由 fn `declaration_initializer`/`declaration_type_node`/`is_function_expression_or_arrow`/`function_like_parameters`/`parameter_index`/`parameter_is_optional_for_arity` + **4bk** `object_literal_element_static_name`/`array_literal_elements` | `checker.go:` |
