...
- [ ] DEFER(4h+)：重载选择 / 上下文敏感函数 / 赋值・控制流语句 / 类检查 / 未用检查；`CallExpression` 经 bound program 端到端（需声明构造可调用类型 + interface call-signature 收集）；not-callable / arg-count 诊断；数字・计算索引・索引签名；完整 `TypeFacts`（typeof/EQ-NE/discriminant）　`// Go: checker.go`　blocked-by: 函数类型构造（声明→签名）+ lib globals(P6) + node builder(4j)
...
| `internal/checker/core/check.rs` | `check_source_file`/`check_statement`/`check_expression`（literal/identifier/property/element）+ `check_identifier`/`check_property_access`/`check_element_access` + `Diagnostic` + `get_diagnostics`/`error` + `get_signatures_of_type`/`get_return_type_of_call` | `checker.go:checkSourceFile/checkExpression/checkIdentifier/checkPropertyAccessExpression/getDiagnostics/error/getSignaturesOfType/getReturnTypeOfSignature` |
...
> 其余 Go 文件（`nodebuilderimpl.go`/`grammarchecks.go`/`exports.go`/`emitresolver.go`/`services.go`/...）尚未落地，按子阶段 4i..4k 推进。`checker.go`（~3.2 万行，4g 落地 check 入口 + 表达式族 + 诊断 + 调用解析起步）/`jsx.go`（4h 落地核心：元素/自闭/片段 + 内在/值解析 + props + children）/`flow.go`（~2.7k 行）/`utilities.go`（4g `TypeFacts` 子集）/`relater.go`（~5k 行）/`inference.go`（~2.7k 行）继续在 `core/check.rs`/`core/jsx.rs`/`core/flow.rs`/`core/type_facts.rs`/`core/relations.rs`/`core/inference.rs`/... 增量拆分。
