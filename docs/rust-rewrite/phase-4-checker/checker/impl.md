...
- [x] `serialize_type_node_for_metadata`（legacy-decorator `__metadata("design:type", ..)` 地基；加法式 pub 枚举 `SerializedTypeNode` + pub 方法）——可达 keyword-type 子集，逐 Go switch 臂：`number`→`Number`、`string`→`String`、`boolean`→`Boolean`、`bigint`→`BigInt`、`symbol`→`Symbol`、`void`/`undefined`/`never` 及 `null` literal-type→`VoidZero`（`void 0`）、`any`/`unknown`/`object`（及 catch-all）→`Object`。DEFER 非-keyword 臂（`TypeReference`→entity ctor、union/intersection/conditional 递归、`FunctionType`/`ConstructorType`→`Function`、`ArrayType`/`TupleType`→`Array`、`TypePredicate`、`TemplateLiteralType`→`String`、非-`null` literal 臂、`SkipTypeParentheses`——暂落到保守 `Object` tail）（4at 落地，`core/emit_resolver.rs`）　`// Go: tstransforms/typeserializer.go:metadataSerializer.serializeTypeNode`
