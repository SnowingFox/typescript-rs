...
- **`// blocked-by: compiler.Program (P6)`**：`new_checker` 的全局/lib 绑定 + 保留 program；多文件符号查询；跨模块别名解析（`is_value_aliased`/`skip_alias` 全量）；~~`is_declaration_visible` 的 module-vs-script 判定~~（**D-F3 已落地**，`external_module_indicator`）；`TestGetSymbolAtLocation` 真 program 版 / `BenchmarkNewChecker`。
...
- [x] `enum TypeData` 其余变体（Intersection/Mapped/Conditional/IndexedAccess/IndexType/...）　`// Go: types.go:TypeData`（done: Intersection in 4v, Index+IndexedAccess in C-C1, Conditional+TemplateLiteral in later rounds; Mapped/Substitution still building out）
...
- [x] 其余 interning map（string/number/bigint/enum/intersection/indexedAccess/template/...）　`// Go: checker.go:Checker 字段`（done: intersection_types in 4v; others added incrementally in later rounds）
...
- [x] **C-B2 调用实参推断**：`check_call_expression` 泛型+无显式实参分支 → `infer_type_arguments_for_call`（两遍 by-context-sensitivity）+ `resolve_inferred_type_argument_signature`（推断→`get_signature_instantiation`→记忆到调用节点）+ `get_inferred_types_for_call`/`get_inferred_type_for_call`（covariant `getCommonSupertype` + 约束 + 默认/unknown）+ `instantiate_param_type`/`instantiate_anonymous_object_type`（anonymous object/函数类型程序感知深实例化，解锁 `{v:T}`/`(x:T)=>U`）（C-B2 落地，`core/inference.rs`+`core/check.rs`+`core/contextual.rs`）　`// Go: checker.go:inferTypeArguments/resolveCall + inference.go:getInferredType(s)/getCovariantInference/getCommonSupertype`
- [x] **C-B3 泛型调用收口**：回调返回 `U` 推断（`infer_from_signatures`/`infer_from_signature`：参数 contravariant 位 + 返回 covariant 候选）+ 上下文-return 推断（`infer_from_contextual_return_type`，`InferencePriority::RETURN_TYPE`）+ lazy `InferenceTypeMapper`（`get_fixing_inference_mapper`，可达实现）+ 优先级 lattice（`infer_types_with_priority`，`priority < inference.priority` 清候选；`InferenceInfo` 种子 `MAX_VALUE`）+ `get_return_type_from_body`（concise/block 返回体推断 + 字面量加宽）+ 实例化缓存（`create_type_reference` 按 `(target, type-args)` interning）（C-B3 落地，`core/inference.rs`+`core/check.rs`+`core/mod.rs`）　`// Go: inference.go:inferFromSignatures/inferFromSignature + checker.go:inferTypeArguments(contextual return)/getReturnTypeFromBody + mapper.go:newInferenceTypeMapper`
- [x] **C-C1 keyof/index 类型 + 索引访问 `T[K]` 深化**：`get_index_type`/`get_index_type_ex`（具体对象→属性名字面量并集 via `get_literal_type_from_properties`/`get_literal_type_from_property`/`is_key_type_included`；泛型→延迟 `TypeData::Index` via `should_defer_index_type`/`get_index_type_for_generic_type`）+ `get_indexed_access_type` 深化（`should_defer_indexed_access_type`/`is_generic_index_type`/`is_generic_object_type`→延迟 `TypeData::IndexedAccess`；union-index 分配；`get_property_type_for_index_type` 命名属性查找 via `get_property_name_from_index`）+ 类型节点接线（`get_type_from_type_operator_node` keyof 臂、`get_type_from_indexed_access_type_node`）+ 实例化（`instantiate_type_worker` 的 `Index`/`IndexedAccess` 臂）+ `could_contain_type_variables`（`INSTANTIABLE` 臂）+ 推断约束实例化（`get_inferred_type_for_call` 经 `inference_context_mapper` 实例化 `K extends keyof T` 约束）+ 类型印名（`keyof X`/`X[Y]`）（C-C1 落地，`core/types.rs`+`core/mod.rs`+`core/declared_types.rs`+`core/mapper.rs`+`core/inference.rs`+`core/nodebuilder.rs`+`core/check.rs`）　`// Go: checker.go: + instantiateTypeWorker(Index/IndexedAccess)`
- [ ] 推断深化（C-C2/C-C3）：full contravariant 候选格（`contraCandidates`/`getContravariantInference`）+ 完整 `InferencePriority` lattice（mapped/keyof/literal/substitution 位）、条件类型（C-C2）、mapped/template-literal 类型（C-C3）、keyof over mapped、reverse-mapped、`outerMapper`/`returnMapper` 嵌套泛型上下文、intra-expression inference sites、JSX 推断　`// Go: inference.go`（C-C2/C-C3/later）
...
- [x] `is_declaration_visible`（4k：顶层声明带 `export` 即可见 → **C-E 扩为忠实 Go `determineIfDeclarationIsVisible` switch 可达子集**：var/module/class/interface/typealias/function/enum/`import=` 组按 **combined** 修饰（折叠 VariableStatement `export`）+ 容器递归；`SourceFile`/`NamespaceExportDeclaration`/`TypeParameter` 恒可见；import clause/specifier/namespace import/`export=` false。非导出 → `IsGlobalSourceFile(parent)`（**D-F3 de-defer**：经新私有 `is_global_source_file` 查 `external_module_indicator` —— 全局脚本非导出顶层声明可见、module 内不可见）。新私有 helper `determine_if_declaration_is_visible`/`combined_modifier_flags`/`get_declaration_container`/`get_root_declaration`/`is_global_source_file`）（4k 落地 + C-E 扩 + D-F3 脚本案，`core/emit_resolver.rs`）　`// Go: emitresolver.go:IsDeclarationVisible(104)/determineIfDeclarationIsVisible(131) + utilities.go:GetCombinedModifierFlags/GetDeclarationContainer/GetRootDeclaration`
- [x] `get_constant_value` / `get_enum_member_value`（**C-E**：const-enum emit 内联原语；`EnumMember` 节点直接返值（Go 首臂，与 const 无关）+ 引用臂仅内联 **const** enum 成员（`E.A` PropertyAccess + bare ident，`IsEnumConst(member.Parent)` 守卫）；返 `EvalValue`（`None`=Go nil）；复用 `declared_types::get_enum_member_value`（`pub(crate)`，复用既有 `compute_enum_member_values`）。私有 helper `resolve_enum_member_reference`/`is_enum_const`。cmd/tsgo 实跑验 `var x = E.A` → `var x = 10`。DEFER element-access/qualified/跨模块/bookkeeping flags）（C-E 落地，`core/emit_resolver.rs`+`core/declared_types.rs`）　`// Go: services.go:GetConstantValue(819) + checker.go:getEnumMemberValue(23773) + emitresolver.go:GetEnumMemberValue(89)`
- [x] `get_global_promise_type` / `get_global_promise_constructor_symbol`（**C-E**：lib.d.ts globals 双侧 checker 注入便捷物；委托 `get_global_type("Promise")`/`get_global_symbol("Promise", VALUE)`；**有合成/真 lib 则解析、无则优雅降级 `None` 不 panic**——metadata 序列化 async `design:returntype`/`design:type` Promise 臂地基；真 lib 端到端经 P6）（C-E 落地，`core/mod.rs`）　`// Go: checker.go:getGlobalPromiseType/getGlobalPromiseConstructorSymbol`
...
- [ ] DEFER(post-C-E / D-F / P6)：`is_value_aliased`/`is_referenced_alias` 的**跨模块别名/导入解析分支**、`get_type_reference_serialization_kind` 的 **lib-globals 类**（`Promise` 臂 wiring 已备 `get_global_promise_constructor_symbol` 但接入序列化需 `resolveEntityName` value/type 一致性 + 真 lib；`NumberLikeType`/`ArrayLikeType`/`VoidNullableOrNeverType`/… 需 resolved global types + 构造/调用签名收集）、**`create_type_of_declaration`/`create_return_type_of_signature_declaration`/`create_type_of_expression`/`create_literal_const_value`/`create_late_bound_index_signatures`（声明 type-node 合成，输出类型*节点*而非字符串，需完整 node builder + `EmitContext` + `SymbolTracker`）→ 交接 D-F（declarations transformer）**、`is_entity_name_visible`/`has_visible_declarations`/`precalculate_declaration_emit_visibility`（声明 emit 别名可见性 + `SymbolAccessibilityResult`）、`get_referenced_value_declaration(s)`/`get_referenced_import_declaration`/`get_referenced_member_value_declaration`/`get_element_access_expression_name`（`ReferenceResolver` 委托面）、`is_late_bound`/`requires_adding_implicit_undefined`/`is_literal_const_declaration`/`is_expando_function_declaration`/`is_import_required_by_augmentation`/`is_definitely_reference_to_global_symbol_object`/`is_top_level_value_import_equals_with_entity_name`/`mark_linked_references_recursively`/`get_type_reference_directives_for_*`　`// Go: emitresolver.go`　blocked-by: `compiler.Program` + 真 lib globals（P6）+ 完整 node builder/`SymbolTracker`（P5 D-F）+ 各自基建（CheckFlagsLate/reverse-mapped/fresh-literal/expando/markLinkedReferences/...）
- [x] **C-E（const-value + `IsDeclarationVisible` 扩展 + lib globals Promise 双侧）落地** ⇒ **P4 checker 可达子集 C-A..C-E 全部完成**（见 C-E worklog）　`// Go: services.go:GetConstantValue + emitresolver.go:determineIfDeclarationIsVisible + checker.go:getGlobalPromiseType`
...
- [x] `lib.rs` 声明其余 `mod`（flow/jsx/grammar/contextual/reachability/type_facts + re-exports）（done: all core submodules declared in core/mod.rs; full NodeBuilder re-export DEFER）
...
| `internal/checker/core/contextual.rs`（4bj 新；4bk/4bl 扩） | `ContextFlags`(pub(crate)) + `get_contextual_type`(var-decl 初始化器臂 + binary `=` RHS 臂 + **4bk** PropertyAssignment/ShorthandPropertyAssignment 臂 + ArrayLiteralExpression 臂 + **4bl** CallExpression/NewExpression 实参臂；余 DEFER)/`get_contextual_type_for_initializer_expression`/`get_contextual_type_for_variable_like_declaration`/`get_contextual_type_for_binary_operand`/`get_contextual_type_for_assignment_expression`(Identifier 目标) + **4bk** `get_contextual_type_for_object_literal_element`/`get_contextual_type_for_array_literal_element`/`get_contextual_type_for_element_expression`/`get_type_of_property_of_contextual_type`/`get_type_from_index_infos_of_contextual_type`/`get_widened_literal_like_type_for_contextual_type`(pub(crate))/`is_literal_of_contextual_type`(pub(crate)) + **4bl** `get_contextual_type_for_argument`/`get_contextual_type_for_argument_at_index`/`get_resolved_signature_for_contextual_argument`(私有) + `get_apparent_type_of_contextual_type`(薄包装)/`get_contextual_signature`/`get_contextual_call_signature`/`is_arity_smaller`/`get_contextually_typed_parameter_type`/`assign_contextual_parameter_types`/`try_get_type_at_position`/`contextually_check_function_expression` + 自由 fn `declaration_initializer`/`declaration_type_node`/`is_function_expression_or_arrow`/`function_like_parameters`/`parameter_index`/`parameter_is_optional_for_arity` + **4bk** `object_literal_element_static_name`/`array_literal_elements` + **4bl** `call_arguments` | `checker.go:(实参上下文子集)` |
| `internal/checker/core/type_facts.rs` | `TypeFacts`(4az：EQ/NE/Is undefined/null + TRUTHY/FALSY，Go 位号) + `get_type_facts`（4az 忠实 per-kind，strict-aware，union OR-fold；**C-D1 加 `ES_SYMBOL_LIKE`/`NON_PRIMITIVE` 臂**）/`has_type_facts`/`get_type_with_facts` + `get_non_null_type`（4ay：`NEUndefinedOrNull` 抽 null/undefined/void 的可达子集，strict-gated） + **C-D1 `add_optionality_ex`/`get_optional_type`（可选属性/参数 `\| undefined` 注入，strict-gated）** | `checker.go:TypeFacts/getTypeFactsWorker/hasTypeFacts/addOptionalityEx/getOptionalType` + `utilities.go:getTypeWithFacts` + `checker.go:GetNonNullableType/getAdjustedTypeWithFacts` |
...
| `internal/checker/core/nodebuilder.rs` | `type_to_string`（命名/引用/匿名成员/union；**C-D1：EnumLike 印名 —— 成员→`E.A`、单成员（`getDeclaredTypeOfSymbol(parent)==t`）/联合/computed→`E`**；intrinsic/literal 委托）+ `symbol_to_string` + `serialize_members` | `checker.go:typeToString/symbolToString` + `nodebuilderimpl.go:typeToTypeNodeWorker(EnumLike)` + `printer.go` |
| `internal/checker/core/emit_resolver.rs` | `EmitResolver` + `is_declaration_visible`(**C-E：忠实 Go switch 可达子集**)/`serialize_type_of_declaration`/`is_implementation_of_overload`/`resolve_reference`/`is_referenced`/`is_value_alias_declaration`/`is_referenced_alias_declaration`/`get_referenced_export_container`/`serialize_type_node_for_metadata`/`get_type_reference_serialization_kind`/**C-E `get_constant_value`/`get_enum_member_value`**（const-enum 内联）+ `Checker::get_emit_resolver`(OnceCell)/`new_checker` | `emitresolver.go:EmitResolver/IsDeclarationVisible/determineIfDeclarationIsVisible/SerializeTypeOfDeclaration/IsImplementationOfOverload/GetEnumMemberValue` + `services.go:GetConstantValue` + `checker.go:GetEmitResolver/NewChecker` |
...
| `internal/checker/core/declared_types.rs` | `get_declared_type_of_symbol`(+generic/extends/type-param)/`get_type_of_symbol`/`get_type_from_type_node`(+TypeReference +4az `LiteralType`/`null` +4bj `FunctionType`/`ConstructorType`→`get_type_from_function_or_constructor_type_node`(`__call` 成员→call 签名))/`get_type_from_literal_type_node`(4az：`NullKeyword`→`null`；4bb：`"a"`/`1`/`true` → `regular_type_of_literal_type(check_expression(literal))`)/`get_signatures_of_symbol`/`get_signature_from_declaration`(4bj：+`FunctionType`/`ConstructorType` 臂)/`get_type_of_variable_or_property`(4bj：无注解 `Parameter` 臂→`get_contextually_typed_parameter_type`；**C-D1：注解臂 `add_optionality_ex` 注可选属性/参数 `\| undefined` + `is_optional_declaration`**)/**C-D1 枚举：`get_declared_type_of_enum`(成员值→字面量→`ENUM_LITERAL` 联合 + `mark_enum_union`)/`compute_enum_member_values`(auto-increment + 求值器常量 + 实体按名)/`get_type_of_enum_object`(值侧成员对象)/`get_type_of_enum_member`/`get_declared_type_of_enum_member`；`get_type_of_symbol` `ENUM_MEMBER`/`ENUM` 臂；`resolve_mapped_type_members_eager` `-?` stripOptional**/`get_apparent_type`/`get_property_of_type`/`get_type_of_property_of_type`/`get_properties_of_type`/`resolve_structured_type_members`/`get_global_type` | `checker.go:getDeclaredType*` |
...
- ~~`is_declaration_visible` 的 global-script-file 分支~~ → **D-F3 已落地**（`is_global_source_file` 查 `external_module_indicator`：脚本非导出可见 / module 不可见）。余下 ambient module + 成员/嵌套可见性（需 `GetEffectiveDeclarationFlags`）+ CommonJS indicator → blocked-by: `compiler.Program`（P6）。
...
## 4bj 落地记录（worklog 摘要）—— 上下文类型地基：箭头/函数表达式参数的上下文参数类型（`getContextualType` 家族 + 函数类型节点）

**目标**：落地上下文类型（contextual typing）的**地基可达切片**——`const f: T = <expr>` 的变量声明初始化器位 + `x = <expr>` 的赋值 RHS 位，把注解类型作为表达式的**上下文类型**；再据此为**箭头/函数表达式的无注解参数**赋上下文参数类型。这解锁回调参数类型、更好推断与多类诊断。在此之前无注解参数恒为 `any`/error，`const f: (x: number) => void = x => x.toFixed()` 不能给 `x` 类型 `number`。逐行为红→绿，一次一个。单 lane（无其它 lane）。cargo 仅 `-p tsgo_checker`（未 `--workspace`）。未 `git commit`。

**Go 真值（ground truth，已逐一核对）**：
- `internal/checker/checker.go:Checker.getContextualType(29019)`：大 switch on `parent.Kind`。本轮可达臂：`KindVariableDeclaration|KindParameter|KindPropertyDeclaration|KindPropertySignature|KindBindingElement → getContextualTypeForInitializerExpression`；`KindBinaryExpression → getContextualTypeForBinaryOperand`。其余臂（call/new 实参、return/arrow-body、yield/await、decorator、as/satisfies/paren/non-null 透传、对象/数组字面量元素、conditional/template、JSX）DEFER。
- `getContextualTypeForInitializerExpression(29099)`：`node == declaration.Initializer()` → `getContextualTypeForVariableLikeDeclaration`（binding-pattern 隐含类型 DEFER）。
- `getContextualTypeForVariableLikeDeclaration(29114)`：`typeNode != nil → getTypeFromTypeNode(typeNode)`；否则 `KindParameter → getContextuallyTypedParameterType`（BindingElement/static-property DEFER）。
- `getContextuallyTypedParameterType(29134)`：`fn := parameter.Parent`；非 `isContextSensitiveFunctionOrObjectLiteralMethod` → nil；IIFE 实参 DEFER；`contextualSignature := getContextualSignature(fn)`；`index := slices.Index(fn.Parameters(), parameter) - (thisParam?1:0)`；rest 用 `getRestTypeAtPosition`（DEFER），否则 `tryGetTypeAtPosition(contextualSignature, index)`。
- `getContextualSignature(10224)`：`t := getApparentTypeOfContextualType(node, ContextFlagsSignature)`；非 union → `getContextualCallSignature(t, node)`；union → 逐成员 `compareSignaturesIdentical` + `createUnionSignature`（DEFER）。
- `getContextualCallSignature(10265)`：`signatures := getSignaturesOfType(t, Call)`；`applicableByArity := Filter(!isAritySmaller)`；len==1 → 取之；否则 `getIntersectedSignatures`（noImplicitAny 合并，DEFER）。
- `isAritySmaller(10293)`：数 target 的必填参数（遇 `?`/初始化器/`...` 停）；`!hasEffectiveRestParameter && getParameterCount(sig) < targetParamCount`（this 减 1 / rest 豁免 DEFER）。
- `assignContextualParameterTypes(10309)` + `assignParameterType(10364)`：对每个 `declaration.Type() == nil` 的参数 `assignParameterType(param, tryGetTypeAtPosition(context, i))`；`assignParameterType` 若 `links.resolvedType != nil` 提前返回（typeParameters/thisParameter/rest/初始化器 widening/`addOptionalityEx`/binding-pattern fallback DEFER）。
- `getApparentTypeOfContextualType(30361)`：`getContextualType` → `instantiateContextualType` → `mapType(getApparentType)` → union 判别（对象字面量函数类型已是对象类型，`getApparentType` 恒等；instantiate/mapType/判别 DEFER）。
- `contextuallyCheckFunctionExpressionOrObjectLiteralMethod(10112)`：`contextualSignature := getContextualSignature(node)`；`isContextSensitive` 且有 contextualSignature → `assignContextualParameterTypes(signature, instantiatedContextualSignature)`（ContextChecked node-link 重入守卫、`assignNonContextualParameterTypes`、推断上下文实例化、return-type-from-body DEFER）。
- `getTypeFromTypeLiteralOrFunctionOrConstructorTypeNode`：`KindFunctionType`/`KindConstructorType` → 匿名对象类型，call 签名来自 `__type` 符号的 `__call` 成员（binder 的 `bindFunctionOrConstructorType` 建 `__type.members[__call]=signatureSymbol`，`__call` 符号声明=[functionTypeNode]）。
- `getTypeForVariableLikeDeclaration` 参数臂：无注解参数 → `getContextuallyTypedParameterType(declaration)`（this-参数/初始化器推断/`addOptionalityEx` DEFER）。

**可达裁剪（faithful-but-reachable）**：
- 新文件 `core/contextual.rs`（+`contextual_test.rs`）承载 `getContextualType` 家族。新 `pub(crate) ContextFlags` bitflags（`NONE`/`SIGNATURE`/`NO_CONSTRAINTS`/`SKIP_BINDING_PATTERNS`；仅 `SIGNATURE` 被分支消费）。
- `get_contextual_type` 仅实现 var-decl 初始化器臂 + binary `=` RHS 臂；其余 parent kind → None（DEFER）。
- `get_contextual_type_for_binary_operand` 仅 `EqualsToken` 且 `node==right` → `get_contextual_type_for_assignment_expression`；后者仅 **Identifier 目标** → `resolve_name` + `get_type_of_symbol`（access-expression/`module.exports`/assignment-declaration DEFER）。
- `get_apparent_type_of_contextual_type` 为 `get_contextual_type` 的薄包装（instantiate/getApparentType/判别 DEFER；可达上下文类型已是对象类型，getApparentType 恒等）。
- `get_contextual_signature` 非-union → `get_contextual_call_signature`（union DEFER）；`get_contextual_call_signature` 用 `is_arity_smaller` 过滤后 len==1 取之（`getIntersectedSignatures` DEFER）。`is_arity_smaller` 忠实（this 减/rest 豁免 DEFER）。
- `try_get_type_at_position` 无 rest 子集：`pos < params.len()` → 参数类型，否则 None。
- **函数类型节点**：`declared_types.rs:get_type_from_type_node` 新增 `Kind::FunctionType|ConstructorType → get_type_from_function_or_constructor_type_node`（读 `__type` 符号的 `__call` 成员声明 → `get_signatures_of_symbol`，建匿名对象类型）；`get_signatures_of_symbol`/`get_signature_from_declaration` 新增 `FunctionType|ConstructorType` 臂（参数 + 返回类型）。construct 签名（`__new`）/泛型/this/节点缓存 DEFER。
- **双通道赋值**（与 Go 一致）：**惰性**——`get_type_of_variable_or_property` 无注解 `Kind::Parameter` 臂 → `get_contextually_typed_parameter_type`（fallback `any`）；**急性**——`check_arrow_function`/`check_function_expression` 入口先 `contextually_check_function_expression`（`get_contextual_signature` + `assign_contextual_parameter_types` 缓存到 `value_symbol_links.resolved_type`）再下传 body。两通道结果一致（急性命中后惰性走缓存）。

**严格 TDD（逐行为 red→green，一次一个；每片单独 `cargo test -p tsgo_checker <name>` 看红/绿）**：

| # | 切片 | 最小 input → observable（实测红/绿） | 实现触点 |
|---|---|---|---|
| 1 tracer（genuine RED）| `arrow_parameter_is_contextually_typed_by_variable_annotation` | `const f: (x: number) => void = (x) => { x; };` → body `x` == `number`（红：旧无上下文源 → `get_type_of_symbol(x)` 落 `any`，实测 `TypeId(1)≠TypeId(8)`）；走**惰性**通道（直接查 `x`，不先 check 箭头）| `contextual.rs` 全家；`declared_types.rs` FunctionType 臂 + 参数惰性臂 |
| 2（genuine RED）| `contextual_parameter_type_flows_into_body_and_surfaces_mismatch` | `const f: (x: number) => void = (x) => { const s: string = x; };` → 1×2322 `number` 不可赋 `string`（红：中和 `get_contextual_signature` 后实测 `0≠1` 诊断）；走**急性**通道（`check_source_file` → `check_arrow_function`）| `check.rs` 急性接线 |
| 2 正控（green-on-arrival）| `contextual_parameter_type_flows_into_body_assignable_ok` | `... { const n: number = x; }` → 0 诊断 | 无 |
| 3 守卫（green-on-arrival）| `explicit_parameter_annotation_overrides_contextual_type` | `const f: (x: number) => void = (x: string) => { x; };` → `x` == `string`（注解胜，Go 不覆盖显式注解）| 无（惰性/急性均跳过有注解参数）|
| 3 守卫（无回归）| `bare_arrow_parameter_without_context_stays_any` | `const g = (x) => x;` → `x` == `any`（无上下文签名，不上下文化，无 panic）| 无 |
| 单测 | `get_contextual_type_of_variable_initializer_is_annotation_type` | `const x: number = 0;` → `get_contextual_type(0)` == `Some(number)` | `get_contextual_type` |
| 单测 | `get_contextual_type_of_unconstrained_expression_is_none` | `0;` → `get_contextual_type` == `None` | default 臂 |
| 单测 | `get_contextually_typed_parameter_type_uses_positional_signature_parameter` | `const f: (a: string, b: number) => void = (a, b) => {};` → `a`==`string`、`b`==`number`，签名 2 参 | `get_contextual_signature`/`get_contextually_typed_parameter_type` |
| 单测 | `assign_contextual_parameter_types_caches_on_symbol_links` | 赋值前 `resolved_type` 空 → `assign_contextual_parameter_types` 后 == `Some(number)`（急性独立见证）| `assign_contextual_parameter_types` |
| 单测 | `function_type_node_resolves_to_object_with_call_signature` | `let f: (x: number) => void;` → 类型节点 1 call 签名，参 0==`number`，返回==`void` | FunctionType 臂 |
| 单测 | `get_contextual_type_of_assignment_rhs_is_target_type` | `let x: number;\nx = 0;` → `get_contextual_type(0)` == `Some(number)` | binary `=` RHS 臂 |
| 单测 | `arrow_parameter_is_contextually_typed_via_assignment_rhs` | `let f: (x: number) => void;\nf = (x) => { x; };` → body `x` == `number` | 赋值 RHS → 箭头参数 |

> 红→绿证据：slice 1 **genuine RED**（初始无任何实现：`x`=`TypeId(1)` any ≠ `TypeId(8)` number；再临时中和**惰性**通道复现同红）；slice 2 **genuine RED**（临时中和 `get_contextual_signature` → `0≠1` 诊断，证明上下文参数类型真流入 body 才有 2322）。两片测**不同通道**（slice 1 惰性、slice 2 急性），互补。slice 2 正控 / slice 3 守卫 / 单测为 **green-on-arrival 守卫**——**如实记录非伪造红**（同 4bc–4bi 口径）。

**本轮交付**：
- `core/contextual.rs`（新）：`pub(crate) ContextFlags`；`impl Checker` 新 `pub(crate)` 方法 `get_contextual_type`/`get_apparent_type_of_contextual_type`/`get_contextual_signature`/`get_contextually_typed_parameter_type`/`assign_contextual_parameter_types`/`try_get_type_at_position`/`contextually_check_function_expression`，私有方法 `get_contextual_type_for_initializer_expression`/`get_contextual_type_for_variable_like_declaration`/`get_contextual_type_for_binary_operand`/`get_contextual_type_for_assignment_expression`/`get_contextual_call_signature`/`is_arity_smaller`；私有自由 fn `declaration_initializer`/`declaration_type_node`/`is_function_expression_or_arrow`/`function_like_parameters`/`parameter_index`/`parameter_is_optional_for_arity`。
- `core/mod.rs`：新 `pub mod contextual;`。
- `core/declared_types.rs`：`get_type_from_type_node` 新增 `FunctionType|ConstructorType` 臂 + 新私有 fn `get_type_from_function_or_constructor_type_node`；`get_signatures_of_symbol`/`get_signature_from_declaration` 加 `FunctionType|ConstructorType`；`get_type_of_variable_or_property` 加无注解参数 contextual 臂；import `INTERNAL_SYMBOL_NAME_CALL`。
- `core/check.rs`：`check_arrow_function`/`check_function_expression` 入口调 `contextually_check_function_expression`；更新两者 DEFER 注释。
- 测试：`contextual_test.rs`（新，12 测）。

**新公开 API 形状**：**无新 `pub` 项、无既有 `pub fn` 签名变更、无 `lib.rs` 改动、无新依赖、无 `.go` 改动**。全部新增项为 `pub(crate)`（contextual 方法 + `ContextFlags`）或私有（helper fn / `get_type_from_function_or_constructor_type_node`）。`get_type_from_type_node`（既有 `pub fn`）行为对 `FunctionType`/`ConstructorType` 由 `error_type` 变为对象类型——**additive 行为扩展**（先前该路径不可达地返回 error，无既有读者依赖；全套件零回归）。`cargo build -p tsgo_compiler` 绿（实测）。

**测试增量**：**475 单测**（+12，相对 4bi 基线 463）+ **134 doctest**（**±0**：新方法/类型均 `pub(crate)`/私有，不挂 doctest）。

**gate（实测，均已 RUN）**：`cargo test -p tsgo_checker --lib`（475 绿）+ `--doc`（134 绿）；`cargo clippy -p tsgo_checker --all-targets -- -D warnings` 干净；`cargo fmt -p tsgo_checker -- --check` 干净；`cargo build -p tsgo_compiler` 绿。未 `--workspace`（无其它 lane）。未 `git commit`。

**本轮 DEFER（带 blocked-by）**：
- **call/new 实参上下文类型**（`getContextualTypeForArgument`/`getContextualTypeForArgumentAtIndex`）：未做。blocked-by: call-resolution 的 contextual 通道（`getResolvedSignature` 期间回传上下文类型 + `signatureLinks.resolvedSignature`）。
- **return-position / arrow-body 上下文返回类型**（`getContextualTypeForReturnExpression`/`getContextualReturnType`）：未做。blocked-by: 容器函数上下文返回类型 + getReturnTypeFromBody。
- **yield/await 操作数**（`getContextualTypeForYieldOperand`/`AwaitOperand`）：未做。blocked-by: generator/Promise 迭代类型（lib globals P6）。
- **对象/数组字面量元素的上下文类型（类型流入字面量）**（`getContextualTypeForObjectLiteralElement`/`ElementExpression` + `getApparentTypeOfContextualType` 的 `discriminate*`）：未做（本轮是反方向——上下文从注解流向函数表达式参数）。blocked-by: `getApparentTypeOfContextualType` 的 mapType/getApparentType/union 判别 + 字面量 contextual widening（`isLiteralOfContextualType`/`getWidenedLiteralLikeTypeForContextualType`）。
- **JSX 上下文类型**（`getContextualTypeForJsxExpression`/`Attribute`/`getContextualJsxElementAttributesType`）：未做。blocked-by: JSX 属性类型解析。
- **`getApparentTypeOfContextualType` 的实例化/apparent/判别**（`instantiateContextualType`/`mapType(getApparentType)`/`discriminateContextualTypeByObjectMembers`）：薄包装。blocked-by: 推断上下文（returnMapper/nonFixingMapper）+ 原始/类型变量 apparent + union 判别。
- **union 上下文签名**（`getContextualSignature` 的 union 分支 + `compareSignaturesIdentical` + `createUnionSignature`）：union → None。blocked-by: 签名同一性比较 + union 签名。
- **`getIntersectedSignatures`**（`getContextualCallSignature` 的 noImplicitAny 多签名合并）：多签名 → None。blocked-by: `combineUnionOrIntersectionMemberSignatures`。
- **泛型上下文签名推断**（`assignContextualParameterTypes` 的 `typeParameters`/`inferFromAnnotatedParametersAndReturn`/`instantiateSignature`）：未做。blocked-by: 推断上下文 + 签名实例化。
- **`this` 参数上下文类型**（`getContextualThisParameterType` + index 偏移）：未做。blocked-by: `this`-typing。
- **rest/optional 参数上下文映射**（`getRestTypeAtPosition`/`tryGetTypeAtPosition` rest 臂/`addOptionalityEx`/IIFE 实参 `getSpreadArgumentType`）：未做。blocked-by: rest/tuple 类型 + IIFE 实参类型化 + optional-参数 widening。
- **参数默认初始化器与上下文类型的协调**（`assignContextualParameterTypes` 的 `checkDeclarationInitializer`+`widenTypeInferredFromInitializer`）：未做（无注解参数直接取上下文类型）。blocked-by: 参数初始化器推断。
- **binding-pattern 名 / binding-element 上下文类型**（`getContextualTypeForBindingElement`/`getTypeFromBindingPattern`）：未做。blocked-by: binding-pattern 类型化。
- **assignment access-expression 目标**（`obj.x = ...`/`obj[k] = ...`/`this.x = ...` + assignment-declaration `F.id = ...`）：仅 Identifier 目标。blocked-by: `getTypeOfPropertyOfContextualType` + assignment-declaration 符号。
- **`as`/`satisfies`/paren/non-null 透传 + 复合/逻辑赋值算子**（`&&=`/`||=`/`??=`/`||`/`??`/`&&`/`,`）：未做（仅 `=`）。blocked-by: 那些算子臂 + 透传 parent kind。
- **construct 签名 / 泛型函数类型节点 / 节点 `links.resolvedType` 缓存**：`get_type_from_function_or_constructor_type_node` 仅 call 签名、非泛型、每次重建（无 memo）。blocked-by: construct 签名（`__new`）+ 泛型签名 + 类型节点缓存键。
- **`isContextSensitive` 门 / `NodeCheckFlagsContextChecked` 重入守卫 / `assignNonContextualParameterTypes`**：`contextually_check_function_expression` 未建这些（可达单通道不重入；`assign_contextual_parameter_types` 本身只触无注解参数，等价于 isContextSensitive 门）。blocked-by: node-check-flags + 推断上下文重入。
- **`with` 语句早退 / `findContextualNode` 缓存（`contextualInfos`）**：未做。blocked-by: with-语句作用域 + 上下文类型缓存栈。

**conformance 切片（登记，端到端对拍仍在 P10）**：`tests/cases/conformance/functions/`（contextually typed 函数表达式参数）+ `tests/cases/conformance/expressions/contextualTyping/`（变量声明/赋值位的 contextual typing 最基础形态）——无 call-argument/return/JSX/对象字面量/泛型推断/union 签名。

**推荐下一轮（4bk）**：(a) **call/new 实参的上下文类型**（`getContextualTypeForArgument`——解锁 `arr.map(x => ...)` 回调参数类型，但需 call-resolution 的 contextual 通道）；(b) **`as`/`satisfies`/paren/non-null 透传臂**（廉价、解锁 `(x => x) as (n: number) => number` 等）；(c) **return-position 上下文返回类型**（`getContextualTypeForReturnExpression`，承接未注解函数体推断）。强 strict 语义最终仍 blocked-by P6 真 lib.d.ts。

## 4bk 落地记录（worklog 摘要）—— 上下文类型**流入**对象/数组字面量（注解类型反向流向字面量成员/元素，字面量保留）

**目标**：承接 4bj（上下文类型地基：变量声明初始化器位 + 赋值 RHS 位）与 4bf（对象/数组字面量自底向上类型化，无上下文影响）。本轮接通**反方向**：当字面量**有**上下文类型时，让注解的属性/元素类型**流入**字面量成员/元素，使**字面量值在字面量上下文中被保留**而非 widen。核心机制是 Go `checkExpressionForMutableLocation` 的 default 分支 `getWidenedLiteralLikeTypeForContextualType(t, getContextualType(node))`：在此之前每个对象属性值 / 数组元素恒经无上下文 `getWidenedLiteralType` widen。逐行为红→绿，一次一个。单 lane（无其它 lane）。cargo 仅 `-p tsgo_checker`（未 `--workspace`）。未 `git commit`。

**Go 真值（ground truth，已逐一核对）**：
- `internal/checker/checker.go:Checker.checkExpressionForMutableLocation(13784)`：`isConstContext → getRegularTypeOfLiteralType(t)`；`isTypeAssertion(node) → t`（DEFER）；else → `getWidenedLiteralLikeTypeForContextualType(t, instantiateContextualType(getContextualType(node, None), node, None))`。
- `getWidenedLiteralLikeTypeForContextualType(25374)`：`if !isLiteralOfContextualType(t, contextualType) { t = getWidenedUniqueESSymbolType(getWidenedLiteralType(t)) }; return getRegularTypeOfLiteralType(t)`。
- `isLiteralOfContextualType(25381)`：`contextualType` 为 union/intersection → `Some(...)`（DEFER）；InstantiableNonPrimitive（`T extends string` 等）→ 约束探测（DEFER）；否则**字面量原语同 kind**：`(StringLiteral|Index|TemplateLiteral|StringMapping) ∧ candidate maybeStringLiteral` / `NumberLiteral ∧ …` / `BigIntLiteral` / `BooleanLiteral` / `UniqueESSymbol`。可达子集仅 string/number/boolean literal。
- `getContextualType(29019)` 新可达臂：`KindPropertyAssignment|KindShorthandPropertyAssignment → getContextualTypeForObjectLiteralElement(parent)`；`KindArrayLiteralExpression → { t := getApparentTypeOfContextualType(parent); elementIndex := IndexOfNode(parent.Elements(), node); getContextualTypeForElementExpression(t, elementIndex, len, firstSpread, lastSpread) }`。
- `getContextualTypeForObjectLiteralElement(29596)`：`element.Type()!=nil && !isObjectLiteralMethod → getTypeFromTypeNode`（DEFER）；`t := getApparentTypeOfContextualType(element.Parent)`；`hasBindableName(element) → getTypeOfPropertyOfContextualTypeEx(t, symbol.Name, nameType)`；computed/dynamic 名 + index-info-by-name fallback（DEFER）。
- `getContextualTypeForElementExpression(29648)`：`mapTypeEx`（union DEFER）；`isTupleType(t)` → 位置元素（DEFER）；非元组：`firstSpread<0||index<firstSpread → propType := getTypeOfPropertyOfContextualType(t, itoa(index))`（命中返回），否则 `getIteratedTypeOrElementType(IterationUseElement, t, undefinedType, nil, false)`。
- `getTypeOfPropertyOfContextualType(30226)`/`Ex(30230)`：`mapTypeEx`（union DEFER）；intersection 逐 constituent（DEFER）；非 Object → nil；generic mapped（DEFER）；`getTypeOfConcretePropertyOfContextualType(t, name)`（`getPropertyOfType` → `removeMissingType(getTypeOfSymbol(prop), optional)`）命中返回，否则 `getTypeFromIndexInfosOfContextualType(t, name, nameType)`。
- `getTypeFromIndexInfosOfContextualType(30321)`：元组数字-rest（DEFER）；`nameType := getStringLiteralType(name)`；`findApplicableIndexInfo(getIndexInfosOfStructuredType(t), nameType)?.valueType`。
- `getApparentTypeOfContextualType(30361)`：`getContextualType` → `instantiateContextualType` → `mapType(getApparentType)` → union 判别（4bj 起为薄包装；可达上下文是对象/数组类型，apparent 恒等）。

**关键经验事实（实测，决定切片设计）**：任务命名的两个切片（`const xs: number[] = []; xs[0]`、`const o: { xs: number[] } = { xs: [] }; o.xs[0]`）**已 green-on-arrival**——`xs`/`o` 的类型来自**注解**（`getTypeOfVariableOrParameterOrProperty` 先读 type node），故 `xs[0]`/`o.xs[0]` 恒 `number`，与字面量本身的类型无关；**空数组字面量 `[]` 在 Go 里恒为 `Array<never>`**（`checkArrayLiteral` 空元素 → `implicitNeverType`，上下文类型**不**改空数组元素类型）。故本轮真正的 genuine RED 是**字面量保留**（literal preservation）：
- 行为 B（对象）：`const o: { a: "x" } = { a: "x" }` 旧版报**伪 2322**（`{ a: "x" }` widen 成 `{ a: string; }` 不可赋 `{ a: "x"; }`）；接通后 `a` 保 `"x"`，0 诊断。
- 行为 A（数组）：`const xs: "a"[] = ["a"]` 旧版字面量 `["a"]` 为 `Array<string>`；接通后元素 `"a"` 经上下文元素类型 `"a"`（`"a"[]` 的迭代元素类型）保留 → `Array<"a">`。

**可达裁剪（faithful-but-reachable）**：
- `check_expression_for_mutable_location`：default 分支由 `get_widened_literal_type(t)` 改为 `get_widened_literal_like_type_for_contextual_type(t, get_contextual_type(node, NONE))`。无上下文时退化为原 widen 行为（`is_literal_of_contextual_type(t, None)=false` → `get_widened_literal_type` → `regular_type_of_literal_type`，对既有类型恒等），零回归。`isTypeAssertion` 分支 / `instantiateContextualType` 仍 DEFER。
- `get_contextual_type` 新增 PropertyAssignment/ShorthandPropertyAssignment 臂 + ArrayLiteralExpression 臂。
- `get_contextual_type_for_object_literal_element`：`get_apparent_type_of_contextual_type(objectLiteral)` → 静态名（`object_literal_element_static_name`，computed/dynamic DEFER）→ `get_type_of_property_of_contextual_type`。
- `get_contextual_type_for_element_expression`：非-union/非-tuple 可达子集——先 `get_type_of_property_of_contextual_type(t, itoa(index))`（元组/数字索引位命中），否则 `check_iterated_type_or_element_type(t, None, true)`（数组迭代元素类型，复用 4ad/4ah 既有端口；`error_node=None` 故所有报错分支早退）。union `mapType` / tuple 位置元素 / spread offset DEFER。
- `get_type_of_property_of_contextual_type`：非 Object → None；`get_property_of_type` 命中 → `get_type_of_symbol`（`removeMissingType` 可选成员 DEFER）；否则 `get_type_from_index_infos_of_contextual_type`（`get_applicable_index_info_for_name` → value type）。union/intersection/generic-mapped/circular-mapped DEFER。
- `is_literal_of_contextual_type`：可达子集仅 string/number/boolean literal 同 kind（union/intersection/instantiable/bigint/unique-symbol/Index/template DEFER）。
- `get_widened_literal_like_type_for_contextual_type`：忠实（`getWidenedUniqueESSymbolType` DEFER，无 unique-symbol 字面量）。
- `check_iterated_type_or_element_type` 由 `fn` 提升为 `pub(crate)`（contextual.rs 复用为 `getContextualTypeForElementExpression` 内 `getIteratedTypeOrElementType` 调用的端口；仅以 `error_node=None` 调用，副作用极轻）。

**严格 TDD（逐行为 red→green，一次一个；每片单独 `cargo test -p tsgo_checker <name>` 看红/绿）**：

| # | 切片 | 最小 input → observable（实测红/绿） | 实现触点 |
|---|---|---|---|
| 1 tracer（genuine RED）| `object_literal_property_literal_preserved_by_contextual_type` | `const o: { a: "x" } = { a: "x" };` → 0 诊断（红：实测 1×2322 `Type '{ a: string; }' is not assignable to type '{ a: "x"; }'.`——字面量 widen 成 string）| `check_expression_for_mutable_location` 接线 + object-literal-element 臂 + `is_literal_of_contextual_type`/`get_widened_literal_like_type_for_contextual_type` |
| 1 守卫（green-on-arrival）| `object_literal_property_mismatched_literal_kind_still_reports_2322` | `const o: { a: "x" } = { a: 1 };` → 1×2322 `{ a: number; }` 不可赋 `{ a: "x"; }`（kind 不匹配，`1` widen 成 number 仍报错）| 无（`is_literal_of_contextual_type` kind 判别）|
| 2 tracer（genuine RED）| `array_literal_element_literal_preserved_by_contextual_type` | `const xs: "a"[] = ["a"];` → 字面量 `["a"]` == `Array<"a">`（红：经**临时 `if false` 禁用 ArrayLiteralExpression 臂**实测 `Array<string>`）| ArrayLiteralExpression 臂 + `get_contextual_type_for_element_expression` + `check_iterated_type_or_element_type` 提升 pub(crate) |
| 2 守卫（无回归）| `array_literal_without_context_still_widens_elements` | `const ys = ["a"];` → `Array<string>`（无上下文，退化为 widen）| 无 |
| 任务切片 1（green-on-arrival）| `annotated_empty_array_variable_element_access_is_number` | `const xs: number[] = []; xs[0];` → `number`（注解路径，本轮不变）| 无 |
| 任务切片 1 负控（Go 忠实）| `annotated_empty_array_literal_itself_stays_never_array` | `const xs: number[] = []; xs;` 字面量 `[]` == `Array<never>`（上下文**不**改空数组元素类型）| 无 |
| 任务切片 2（green-on-arrival）| `annotated_object_empty_array_property_element_access_is_number` | `const o: { xs: number[] } = { xs: [] }; o.xs[0];` → `number`（注解路径）| 无 |
| 单测 | `get_contextual_type_of_object_literal_property_value_is_property_type` | `const o: { a: "x" } = { a: "x" };` → `get_contextual_type(value)` == `Some("x")` | object-literal-element 臂 |
| 单测 | `get_contextual_type_of_unknown_object_literal_property_is_none` | `const o: { a: number } = { b: 1 };` → `get_contextual_type(b 值)` == `None` | `get_type_of_property_of_contextual_type` nil |
| 单测 | `get_contextual_type_of_array_literal_element_is_element_type` | `const xs: "a"[] = ["a"];` → `get_contextual_type(element)` == `Some("a")` | ArrayLiteralExpression 臂 + element-expression |
| 单测 | `is_literal_of_contextual_type_matches_same_kind_only` | string-lit/string-lit→true；string-lit/number-lit→false；string-lit/`string`→false；string-lit/None→false | `is_literal_of_contextual_type` |
| 单测 | `get_widened_literal_like_type_for_contextual_type_preserves_or_widens` | fresh `"x"` + ctx `"x"`→保 regular `"x"`；+ None→`string`；+ ctx `string`→`string` | `get_widened_literal_like_type_for_contextual_type` |

> 红→绿证据：slice 1 **genuine RED**（实测伪 2322，字面量 widen 成 string）；slice 2 **genuine RED**（临时 `if false` 禁用 ArrayLiteralExpression 臂复现 `Array<string>` ≠ `Array<"a">`）。任务命名的两切片为 **green-on-arrival 守卫**（经注解路径，本轮无改动），负控 `[]`==`Array<never>` 钉死「上下文不改空数组元素类型」（Go 忠实）。其余守卫 / 单测为 green-on-arrival——**如实记录非伪造红**（同 4bc–4bj 口径）。

**本轮交付**：
- `core/contextual.rs`：`get_contextual_type` 加 PropertyAssignment/ShorthandPropertyAssignment + ArrayLiteralExpression 臂；新私有方法 `get_contextual_type_for_object_literal_element`/`get_contextual_type_for_array_literal_element`/`get_contextual_type_for_element_expression`/`get_type_of_property_of_contextual_type`/`get_type_from_index_infos_of_contextual_type`；新 `pub(crate)` 方法 `get_widened_literal_like_type_for_contextual_type`/`is_literal_of_contextual_type`；新私有自由 fn `object_literal_element_static_name`/`array_literal_elements`；import `get_applicable_index_info_for_name`/`get_property_of_type`。
- `core/check.rs`：`check_expression_for_mutable_location` default 分支接 contextual 类型；`check_iterated_type_or_element_type` 由 `fn` 提升 `pub(crate)`；import `super::contextual::ContextFlags`。
- 测试：`contextual_test.rs`（+5 单测）、`check_test.rs`（+7：2 genuine-RED tracer + 1 kind-mismatch 守卫 + 1 无回归 + 3 任务切片/负控）。

**新公开 API 形状**：**无新 `pub` 项、无既有 `pub fn` 签名变更、无 `lib.rs` 改动、无新依赖、无 `.go` 改动**。新增项均 `pub(crate)`（2 contextual 方法）或私有（5 方法 + 2 自由 fn）；`check_iterated_type_or_element_type` `fn`→`pub(crate)` 为内部可见性放宽（非公开 API）。`check_expression_for_mutable_location` 行为对**有上下文字面量**由「恒 widen」变为「字面量上下文保留」——**additive 行为扩展**（无上下文恒退化为旧 widen，全套件零回归）。`cargo build -p tsgo_compiler` 绿（实测）。

**测试增量**：**487 单测**（+12，相对 4bj 基线 475）+ **134 doctest**（**±0**：新方法/类型均 `pub(crate)`/私有，不挂 doctest）。

**gate（实测，均已 RUN）**：`cargo test -p tsgo_checker --lib`（487 绿）+ `--doc`（134 绿）；`cargo clippy -p tsgo_checker --all-targets -- -D warnings` 干净；`cargo fmt -p tsgo_checker -- --check` 干净；`cargo build -p tsgo_compiler` 绿。未 `--workspace`（无其它 lane）。未 `git commit`。

**本轮 DEFER（带 blocked-by）**：
- **空数组/loosely-typed 数组的上下文元素类型「重写」**：Go `checkArrayLiteral` 空数组恒 `implicitNeverType`（上下文**不**改空数组元素类型），故无可做——任务命名的「修空数组」其实是注解路径的既有行为，非字面量类型变化。无需 blocked-by。
- **深层递归元素/属性上下文类型（>1 层）**：本轮接通 1 层（属性值 / 元素直接保留）；嵌套字面量（`{ a: { b: "x" } }`）的每层经各自 `check_expression_for_mutable_location` 递归触发，已天然覆盖单层 literal preservation；但 tuple 位置元素 / union 判别下的深层未做。blocked-by: 见下。
- **union 上下文类型 + 判别选择**（`getContextualTypeForObjectLiteralElement` 的 union 路径 / `discriminateContextualTypeByObjectMembers` / `getApparentTypeOfContextualType` 的 `mapType`+union 判别）：未做（可达子集仅非-union 上下文）。blocked-by: union `mapType` + 对象成员判别 + `getContextFreeTypeOfExpression`。
- **contextual tuple 位置类型化（`inTupleContext`）**：`checkArrayLiteral` 未检测 tuple-like 上下文，数组字面量不变成元组；`getContextualTypeForElementExpression` 的元组位置元素臂未做（元组上下文落 `getIteratedTypeOrElementType` 的数字索引 union）。blocked-by: `isTupleLikeType` 检测 + `forceTuple`/`inTupleContext` check mode + tuple 位置元素/spread-offset。
- **call/new 实参上下文类型**（`getContextualTypeForArgument`）：未做（下一轮）。blocked-by: call-resolution 的 contextual 通道。
- **`getContextualTypeForReturnExpression`**：未做。blocked-by: 容器函数上下文返回类型 + `getReturnTypeFromBody`。
- **spread 元素 / `SpreadAssignment` 祖父级 walk / spread 索引 offset**（`getSpreadIndices`）：未做（无 spread 元素）。blocked-by: spread/iterator 类型化。
- **computed/dynamic 属性名上下文类型**（`getPropertyNameFromType` + 按 nameType 的 index-info fallback `mapType`）：仅静态名。blocked-by: computed-name 类型化 + union `mapType`。
- **元素的显式类型注解臂 + object-literal 方法**（`getContextualTypeForObjectLiteralElement` 的 `element.Type()` / `isObjectLiteralMethod`）：未做。blocked-by: property-element 注解 elaboration + object-literal 方法签名。
- **`removeMissingType` 可选成员 / circular-mapped-property 守卫 / generic mapped 上下文属性**（`getTypeOfConcretePropertyOfContextualType` / `getIndexedMappedTypeSubstitutedTypeOfContextualType`）：未做。blocked-by: optional-member missing-type + 循环 mapped + generic mapped 类型。
- **`isTypeAssertion(node)` 分支 / `instantiateContextualType`**（`checkExpressionForMutableLocation` 中段 + 上下文类型实例化）：未做。blocked-by: 断言值 passthrough + 推断上下文。
- **`isLiteralOfContextualType` 的 union/intersection/instantiable/bigint/unique-symbol/Index/template/string-mapping 臂**：仅 string/number/boolean literal。blocked-by: union/intersection 上下文 + 基约束解析 + bigint/unique-symbol/template-literal 类型化。
- **`getWidenedUniqueESSymbolType`**：未做（无 unique-symbol 字面量）。blocked-by: unique-ES-symbol 类型化。
- **`pushCachedContextualType` / `findContextualNode`（`contextualInfos` memo）/ `with`-语句早退**：未做。blocked-by: 上下文类型缓存栈 + with-语句作用域。

**conformance 切片（登记，端到端对拍仍在 P10）**：`tests/cases/conformance/types/contextualTypes/`（contextual typing 流入对象/数组字面量的字面量保留最基础形态）+ `tests/cases/conformance/types/literal/`（字面量类型注解保留属性/元素 literal）——无 union 判别 / tuple 位置 / call-argument / computed-name / spread。

**推荐下一轮（4bl）**：(a) **call/new 实参的上下文类型**（`getContextualTypeForArgument`——解锁 `arr.map(x => ...)` 回调参数类型，需 call-resolution 的 contextual 通道）；(b) **union 上下文类型 + 对象成员判别**（`getApparentTypeOfContextualType` 的 `mapType`+`discriminateContextualTypeByObjectMembers`，解锁 discriminated-union 字面量）；(c) **contextual tuple 位置类型化**（`inTupleContext` + `getContextualTypeForElementExpression` 元组臂，解锁 `const t: [number, string] = [...]`）。强 strict 语义最终仍 blocked-by P6 真 lib.d.ts。

## 4bl 落地记录（worklog 摘要）—— 上下文类型流入 **call/new 实参**：回调参数从被调签名取上下文参数类型（`getContextualTypeForArgument`）

**目标**：承接 4bj（上下文类型地基：变量初始化器位/赋值 RHS 位 → 箭头/函数表达式无注解参数）与 4bk（上下文类型流入对象/数组字面量）。本轮接通 **call/new 实参位**：当箭头/函数表达式作为 `f(arrow)` 的实参时，让它从**被调签名第 i 个参数类型**取上下文类型，使回调参数被上下文化（`f((x) => …)` 中 `x` 取被调签名期望的参数类型）。在此之前 call 实参位的箭头无上下文签名，回调参数恒 `any`。逐行为红→绿，一次一个。单 lane（无其它 lane）。cargo 仅 `-p tsgo_checker`（未 `--workspace`）。未 `git commit`。

**Go 真值（ground truth，已逐一核对）**：
- `internal/checker/checker.go:Checker.getContextualType(29040)`：`KindCallExpression, KindNewExpression → getContextualTypeForArgument(parent, node)`。
- `getContextualTypeForArgument(29438)`：`args := getEffectiveCallArguments(callTarget)`；`argIndex := slices.Index(args, arg)`；`-1`（如 callee 表达式 / tagged-template 的 tag）→ nil；否则 `getContextualTypeForArgumentAtIndex(callTarget, argIndex)`。
- `getContextualTypeForArgumentAtIndex(29448)`：import-call（`string`/`ImportCallOptions`/`any`）DEFER；签名解析有 resolvingSignature 守卫——`if signatureLinks.Get(callTarget).resolvedSignature == c.resolvingSignature { signature = c.resolvingSignature } else { signature = c.getResolvedSignature(callTarget, nil, CheckModeNormal) }`；JSX 首实参 DEFER；rest（`signatureHasRestParameter && argIndex >= restIndex` → `getIndexedAccessTypeEx`）DEFER；否则 `return getTypeAtPosition(signature, argIndex)`。
- `getEffectiveCallArguments(29718)` default 臂：`node.Arguments()`（spreadIndex<0 → 原实参列）。tagged-template/decorator/JSX/binary-instanceof/spread 合成实参 DEFER。
- `relater.go:getTypeAtPosition(1754)`：`tryGetTypeAtPosition` 非 nil 则取之，否则 `anyType`。`tryGetTypeAtPosition(1762)`：`pos < paramCount` → `getTypeOfParameter`，rest 臂 DEFER。

**可达裁剪（faithful-but-reachable）**：
- `get_contextual_type` 新增 `Kind::CallExpression | Kind::NewExpression → get_contextual_type_for_argument(parent, node)` 臂。
- `get_contextual_type_for_argument`：`call_arguments(call_target)`（`getEffectiveCallArguments` 可达子集——原实参列，无 spread 展开/合成）→ `position(arg)`（找不到，如 callee 与 call 同父但非实参 → None）→ `get_contextual_type_for_argument_at_index`。
- `get_contextual_type_for_argument_at_index`：`get_resolved_signature_for_contextual_argument(call_target)?` → `try_get_type_at_position(signature, arg_index).unwrap_or(any_type)`（忠实 `getTypeAtPosition` 的 any fallback；rest indexed-access DEFER）。
- `get_resolved_signature_for_contextual_argument`：读 callee（CallExpression/NewExpression 的 `expression`）→ `check_expression(callee)`（**只类型化 callee，绝不重查实参**）→ `get_signatures_of_type` → 恰好 1 个 call 签名时取之，否则（重载 >1 / 非可调用 0）None。
- **避免递归**：上下文查找解析被调签名时**仅类型化 callee 表达式**（identifier 即符号类型查表，幂等且无诊断重复；happy-path 不重报），单一 call 签名的选择**不消费实参**，故上下文通道**永不重入实参检查**——天然无环（与 Go 的 `resolvingSignature` 哨兵守同一环等效）。重载-目标须实参类型消歧 → 本轮 None（DEFER）。
- **双通道复用（与 4bj 一致）**：**惰性**——回调体引用参数 `x` → `get_type_of_variable_or_property` 无注解参数臂 → `get_contextually_typed_parameter_type` → `get_contextual_signature(arrow)` → `get_contextual_type(arrow, SIGNATURE)` → **本轮 call 实参臂**；**急性**——`check_call_expression` → `check_applicable_signature_for_call` → `check_expression(arrow)` → `check_arrow_function` 入口 `contextually_check_function_expression`（已在 4bj 接线）。两通道结果一致。

**严格 TDD（逐行为 red→green，一次一个；每片单独 `cargo test -p tsgo_checker <name>` 看红/绿）**：

| # | 切片 | 最小 input → observable（实测红/绿） | 实现触点 |
|---|---|---|---|
| 1 tracer（genuine RED）| `callback_argument_parameter_is_contextually_typed_by_call_signature` | `function f(cb: (x: number) => void) {}\nf((x) => { x; });` → body `x` == `number`（红：旧无 call 实参臂 → `x`=`any`，实测 `TypeId(1)≠TypeId(8)`）；走**惰性**通道（直接 `check_expression(x)`）| `contextual.rs` call 实参臂 + 3 新方法 + `call_arguments` |
| 2（genuine RED）| `callback_argument_parameter_type_flows_into_body_and_surfaces_mismatch` | `function f(cb: (x: number) => void) {}\nf((x) => { const s: string = x; });` → 1×2322 `number`→`string`（红：临时 `if true { return None }` 中和 call 实参臂 → body 无 2322，实测 `0≠1`）；走**急性**通道（`check_source_file` → `check_arrow_function`）| 同 slice 1 臂 |
| 2 正控（green-on-arrival）| `callback_argument_parameter_type_flows_into_body_assignable_ok` | `... f((x) => { const n: number = x; });` → 0 诊断 | 无 |
| 3 守卫（无回归）| `plain_call_without_callback_still_checks_clean` | `function g(n: number) {}\ng(1);` → 0 诊断（无回调实参，普通调用不受扰）| 无 |
| 3 守卫（无 crash）| `call_with_non_function_argument_does_not_crash` | `function f(cb: (x: number) => void) {}\nf(1 as any);` → 0 诊断（非函数实参不 crash）| 无 |
| 3 守卫（递归）| `nested_callback_call_does_not_recurse_infinitely` | `function f(cb: (x: number) => void) {}\nf((x) => { f((y) => { y; }); });` → 0 诊断（嵌套回调调用终止，不栈溢出）| 无（递归守恒由"只类型化 callee"保证）|
| 3 守卫（无双报，genuine RED）| `unresolved_callee_with_callback_reports_2304_once` | `g((x) => { x; });`（`g` 未声明）→ 1×2304（红：实测 **3**——主路径 1 + 急性 1 + 惰性 1，上下文查找重报 callee）| `get_resolved_signature_for_contextual_argument` 重解析期间**快照+回滚** callee 诊断 |
| 单测 | `get_contextual_type_of_call_argument_is_signature_parameter_type` | `function f(cb: (x: number) => void) {}\nf((x) => {});` → `get_contextual_type(arrow)` 是 `(x: number) => void`（1 call 签名、参 0==`number`、返回==`void`）| call 实参臂 |
| 单测 | `get_contextual_type_of_callee_is_none` | callee `f` 与 call 同父但非实参 → None（`slices.Index==-1`）| `get_contextual_type_for_argument` -1 守卫 |
| 单测 | `get_contextual_type_of_overloaded_call_argument_is_none` | `declare function f(a: number): void;\ndeclare function f(a: string): void;\nf((x) => {});` → None（重载 DEFER）| `get_resolved_signature_for_contextual_argument` 多签名臂 |
| 单测 | `get_contextual_type_of_extra_call_argument_is_any` | `function f() {}\nf((x) => {});` → `Some(any)`（越界 `getTypeAtPosition` fallback）| `get_contextual_type_for_argument_at_index` any fallback |
| 单测 | `get_contextual_type_of_new_expression_argument_without_construct_signature_is_none` | `declare const C: any;\nnew C((x) => {});` → None（construct 签名 DEFER；`get_signatures_of_type` 仅 call）| NewExpression 臂 + `call_arguments` NewExpression 分支 |

> 红→绿证据：slice 1 **genuine RED**（初始无 call 实参臂：`x`=`TypeId(1)` any ≠ `TypeId(8)` number）；slice 2 **genuine RED**（临时 `if true { return None }` 中和 call 实参臂 → body 无 2322，`0≠1`）；slice 3 双报守卫 **genuine RED**（实测 `3≠1`——上下文查找重解析 callee 重报 2304，修复为重解析期间快照+回滚 callee 诊断）。slice 1/2 测**不同通道**（惰性/急性）互补。其余 slice 3 守卫（无回归 / 无 crash / 递归终止）+ 单测为 **green-on-arrival 守卫**——**如实记录非伪造红**（同 4bc–4bk 口径）。

**关键修正（红的过程中发现）**：上下文查找经 `get_resolved_signature_for_contextual_argument` 重新 `check_expression(callee)`，而本端口的表达式检查尚无缓存（Go 经 `getResolvedSignature`/`checkExpressionCached` memo），故未解析 callee（`g((x)=>{x;})`）的 2304 被**重报 3 次**（主路径 + 急性 + 惰性）。修复：重解析期间**快照当前文件诊断长度 → 重解析后 truncate 回滚**，丢弃上下文专用重解析产生的 callee 诊断（call-checking 主路径已报一次）——与 Go memo 化二次解析"零重报"等价。有效 callee 不报诊断，故 happy-path 无影响。

**本轮交付**：
- `core/contextual.rs`：`get_contextual_type` 加 `CallExpression | NewExpression` 实参臂；新私有方法 `get_contextual_type_for_argument`/`get_contextual_type_for_argument_at_index`/`get_resolved_signature_for_contextual_argument`（含诊断回滚）；新私有自由 fn `call_arguments`；更新 `get_contextual_type` 文档（call/new 实参不再 DEFER，余项移至 4bm+）。
- 测试：`contextual_test.rs`（+12：导航 helper `call_expression_of_statement`/`call_argument` + slice 1 tracer + slice 2 mismatch/正控 + slice 3 四守卫（无回归/无 crash/递归/无双报）+ 5 单测）。

**新公开 API 形状**：**无新 `pub` 项、无既有 `pub fn` 签名变更、无 `lib.rs` 改动、无新依赖、无 `.go` 改动**。新增项全为私有方法（3）+ 私有自由 fn（1）；`get_contextual_type`（既有 `pub(crate)` 方法）新增 `CallExpression`/`NewExpression` 臂——**additive 行为扩展**（先前该 parent kind 落 default→None，无既有读者依赖；全套件零回归）。`cargo build -p tsgo_compiler` 绿（实测）。

**测试增量**：**499 单测**（+12，相对 4bk 基线 487）+ **134 doctest**（**±0**：新增项全为私有，不挂 doctest）。

**gate（实测，均已 RUN）**：`cargo test -p tsgo_checker --lib`（499 绿）+ `--doc`（134 绿）；`cargo clippy -p tsgo_checker --all-targets -- -D warnings` 干净；`cargo fmt -p tsgo_checker -- --check` 干净；`cargo build -p tsgo_compiler` 绿。未 `--workspace`（无其它 lane）。未 `git commit`。

**本轮 DEFER（带 blocked-by）**：
- **重载-目标上下文实参**（Go 经 `getResolvedSignature` 消歧选出重载，且推断期 candidate loop union 各候选参数类型）：本轮重载 callee（>1 签名）→ None。blocked-by: 重载解析的 contextual 通道（`resolvedSignature` 缓存 + resolvingSignature 哨兵）+ 推断期 candidate 参数类型 union。
- **`import(...)` call 实参类型**（`getContextualTypeForArgumentAtIndex` 的 `ast.IsImportCall` 臂 → `string`/`getGlobalImportCallOptionsType`/`any`）：未做。blocked-by: dynamic-import 类型化 + lib globals(P6)。
- **JSX 首实参签名**（`getEffectiveFirstArgumentForJsxSignature`）：未做。blocked-by: JSX 属性类型解析。
- **rest 参数 indexed-access**（`getContextualTypeForArgumentAtIndex` 的 `signatureHasRestParameter && argIndex >= restIndex` → `getIndexedAccessTypeEx`；`tryGetTypeAtPosition` 的 rest 臂）：仅非-rest 位（越界落 any）。blocked-by: rest/tuple 类型 + indexed-access。
- **`getEffectiveCallArguments` 合成实参**（spread 元素从 tuple 展开、tagged-template 的 strings-array + substitution、decorator 实参、binary-instanceof 左操作数、JSX 属性实参）：仅原实参列。blocked-by: spread/tuple 类型化 + tagged-template + decorators + JSX。
- **construct 签名（`new` 的 `__new`）**：`new C(arrow)` 经 `get_signatures_of_type`（仅 call 签名）→ 通常 0 → None；NewExpression 自身在 `check_expression` 也未类型化（无 `check_new_expression`）。blocked-by: construct 签名收集 + `new` 表达式检查。
- **泛型签名从上下文/实参推断**（`getResolvedSignature` 的推断上下文 + `instantiateSignature`）：仅非泛型单一签名。blocked-by: 推断上下文 + 签名实例化（call-site 推断）。
- **`resolvingSignature` 哨兵 + `signatureLinks.resolvedSignature` 缓存**：本轮以"只类型化 callee、不重查实参"达成等价的无环保证，未建签名缓存/哨兵。blocked-by: 签名解析缓存 + 重入哨兵（与重载/泛型推断一并）。

**conformance 切片（登记，端到端对拍仍在 P10）**：`tests/cases/conformance/expressions/contextualTyping/`（call 实参回调参数上下文类型最基础形态）+ `tests/cases/conformance/functions/`（回调参数从被调签名取类型）最基础子集——无重载/泛型推断/import-call/JSX/rest/spread/construct 签名。
...
## C-B2 落地记录（worklog 摘要）—— 类型实参**推断**引擎：从调用实参推断类型参数

**目标**：把**无显式类型实参**的泛型调用端到端打通——`id(1)` 推出 `T=number`、`[arr].map(x=>...)` 的回调参数 `x` 经推断的 `T` 上下文定型。在 `check_call_expression` 为「泛型签名 + 无显式类型实参」新增**推断分支**：建 `InferenceContext` → 从实参类型对参数类型 `infer_types` → `get_inferred_types_for_call` 解析（covariant common-supertype + 约束 + 默认/unknown 兜底）→ `get_signature_instantiation` 实例化（清类型参数）→ 把**实例化签名**记忆到调用节点（供 context-sensitive 回调实参的上下文定型读到实例化参数类型）。逐切片红→绿；单 lane；cargo 仅 `-p tsgo_checker`（+ `cargo build -p tsgo_compiler` 验 additive）；未 `git commit`。

**Go 真值（ground truth，`go build -o /tmp/tsgo ./cmd/tsgo` 实跑 `--noEmit --strict` 逐一快照；与任务书设想不符处一律以 cmd/tsgo 为准）**：
- 恒等 `function id<T>(x:T):T{return x;} const r=id(1); const s:string=r;` → `s1.ts(3,7) 2322`「Type 'number' is not assignable to type 'string'.」。揭示：`const n:1=id(1)` **0**、`const n:number=id(1)` **0** ⇒ `r` 实为字面量 `1`（赋给 `1` 与 `number` 皆可），消息把字面量泛化为 `number`。
- 同型参两实参 `function pick<T>(a:T,b:T):T{...}`：`pick(1,"x")` → `s2b.ts(2,19) 2345`「Argument of type '"x"' is not assignable to parameter of type '1'.」（**异基底字面量 ⇒ `getCommonSupertype` 取最左 `1`，第二实参对 `1` 不可赋 ⇒ 2345；非任务书设想的 `string|number` 联合**）；`pick(1,2)` → `T=1|2`（`const n:1=pick(1,2)` → `2322`「Type '1 | 2' is not assignable to type '1'.」；`const n:number=pick(1,2)` → **0**）（**同基底字面量 ⇒ 联合**）。
- 对象属性 `function f<T>(o:{v:T}):T{return o.v;} const r=f({v:1}); const s:string=r;` → `s4.ts(3,7) 2322`「...'number'...'string'.」（`const n:1=f({v:1})` → **0** ⇒ `r` 为 `1`，对象字面量属性以未泛化字面量参与推断）。
- 约束 `function f<T extends number>(x:T):T{...}`：`f(1)` → **0**；`f("x")` → `s5b.ts(2,3) 2345`「Argument of type 'string' is not assignable to parameter of type 'number'.」（推断 `"x"` 违反约束 ⇒ `getInferredType` 改推**约束** `number`，实参 `"x"` 对 `number` 不可赋 ⇒ 2345，源泛化为 `string`）。
- 回调 `interface Array<T>{[n:number]:T;length:number} function map<T,U>(a:T[],f:(x:T)=>U):U[]{...}`：`map([1,2],(x):string=>x)` → `(3,28) 2322`「Type 'number' is not assignable to type 'string'.」（`T=number` 从首实参定，回调 `x` 经实例化参数 `(x:number)=>U` 定型为 `number`；cmd/tsgo 因加载真 lib + 合成 `Array` 另报 2374 duplicate-index，本 port 无真 lib 故无）；`map([1,2],(x):number=>x)` → 仅 2374（即回调体 **0**）。`U` 从回调返回类型推断（结果 `string[]`）属 **Go-only**，本 port DEFER。
- 无候选 `function f<T>():T{...} const r=f();` → **0**；`f().toFixed()` → `2571`「Object is of type 'unknown'.」⇒ `T=unknown`。（注：`const n:number=f()` 在 Go 因 **contextual-return 推断** 推出 `T=number` 故 **0**——该路径 C-B3 DEFER；本 port 无 contextual-return ⇒ `T=unknown`。）
- 显式仍优先 `id<string>(1)` → `s7.ts(2,12) 2345`「Argument of type 'number' is not assignable to parameter of type 'string'.」（走 C-B1 显式分支，不推断）。

**Go 函数镜像（`// Go:` 锚）**：`inferTypeArguments`（实参循环 + `getTypeAtPosition` + context-sensitive 跳过）、`resolveCall`/`chooseOverload`（推断分支 + `signatureLinks.resolvedSignature` 记忆）、`getInferredTypes`/`getInferredType`（covariant + 约束臂 + 无候选默认/unknown）、`getCovariantInference`、`getCommonSupertype`/`getSingleCommonSupertype`/`findLeftmostType`/`literalTypesWithSameBaseType`、`isContextSensitive`、`instantiateType`（anonymous object 臂）/`instantiateAnonymousType`、既有 4e `inferTypes`/`inferFromTypes`/`inferFromObjectTypes`/`inferFromProperties`（type-param 槽 / object 属性逐对 / 引用 type-args 逐对——本轮直接复用，未改）。

**严格 TDD（逐切片 red→green；每切片先实测红的对的原因，再最小绿）**：

| # | 切片 | 最小 input → observable（实测红）| 实现触点 |
|---|---|---|---|
| 1 | `generic_call_infers_type_argument_from_argument` | `id(1)` 红：`2345`(arg vs 裸 `T`)+`2322`(`unknown`)，应仅 `2322`(`number`) | `check_call_expression` 推断分支 + `infer_type_arguments_for_call` + `get_inferred_types_for_call` + `get_signature_instantiation` |
| 2 | `..._cross_base_candidates_infer_leftmost` / `..._same_base_candidates_infer_union` / `..._same_base_union_assignable_to_base` | `pick(1,"x")` 应 `2345`('"x"' vs `1`)；`pick(1,2)` 应 `T=1\|2` | `get_common_supertype`（`literalTypesWithSameBaseType` ? union : `getSingleCommonSupertype`/`findLeftmostType`）|
| 3 | `..._callback_parameter_inferred_from_array_argument` / `..._accepts_matching_return` | `map([1,2],(x):string=>x)` 应 `2322`(`number`/`string`)（回调 `x` 须为 `number`）| 两遍（非 context-sensitive 先推断）+ 实例化签名记忆到节点 + `instantiate_param_type`（anonymous object/函数类型深实例化）+ `get_resolved_signature_for_contextual_argument` 读缓存 |
| 4 | `..._infers_through_object_property` / `..._assignable` | `f({v:1})` 红：`2345`(`{v:number}` vs **未实例化** `{v:T}`) | `instantiate_param_type`/`instantiate_anonymous_object_type`（程序感知，按缓存成员类型重建对象）|
| 5 | `..._inference_satisfies_constraint` / `..._violates_constraint_infers_constraint` | `f("x")`（`T extends number`）应 `2345`(`string` vs `number`) | `get_inferred_type_for_call` 约束臂（违反 ⇒ 推约束）|
| 6 | `..._no_candidate_infers_unknown` / `..._result_is_unknown` | `f()` 无候选 ⇒ `unknown` | 无候选默认/`unknown` 兜底 |
| 7 | `..._explicit_type_argument_wins_over_inference` | `id<string>(1)` → `2345`（不推断）| C-B1 显式分支不受影响（推断分支在 `else if 泛型`）|
| 单测 | `inference_test.rs` +7 | 见 tests.md | `literal_types_with_same_base_type`/`find_leftmost_supertype`/`get_common_supertype`/`get_covariant_inference`/`get_inferred_type(s)_for_call` |

> 红→绿证据：切片 1 **genuine RED**（把实参当裸 `T` 报 2345 且返回 `unknown`）；切片 4 **genuine RED**（实参对**未实例化** `{v:T}` 报 2345——暴露 anonymous object 未深实例化，驱动 `instantiate_param_type`）。切片 2/3/5/6 在切片 1/4 建立的引擎与 anonymous-object 实例化之上验证 covariant 解析 / 回调上下文 / 约束 / 无候选兜底（其新代码路径——多候选 `getCommonSupertype`、回调记忆缓存、约束臂、无候选臂——均首次被各自切片触发）。

**可达裁剪（faithful-but-reachable）**：
- **两遍 by-context-sensitivity（非 by-position）**：推断遍只喂**非** context-sensitive 实参（函数/箭头表达式实参跳过），用回滚诊断的 `check_expression_for_inference` 取其类型（应用相位会重检并如实报一次，避免重复诊断）；实例化后把实例化签名记忆到调用节点（`Checker.resolved_signatures`），应用相位检查回调实参时其上下文签名经 `get_resolved_signature_for_contextual_argument` 读到**实例化**参数 `(x:number)=>U` ⇒ 回调 `x` 定型 `number`。`isContextSensitive` 仅取「箭头/函数表达式」可达子集（全注解函数实际非敏感，DEFER）。
- **程序感知参数类型实例化 `instantiate_param_type`**：C-B1 的参数 mapper-on-read 用程序无关 `instantiate_type`，对 anonymous object 返原值（无法解析成员符号类型）。本轮 `get_type_at_position`/`try_get_type_at_position` 改走 `instantiate_param_type(program, base, mapper)`：anonymous object/函数类型经 `instantiate_anonymous_object_type` 用 program 读成员类型（`get_type_of_symbol`）、逐成员代入、铸合成属性符号（`new_object_literal_property`）、实例化 call/construct 签名，重建 anonymous object；type reference/非对象仍委托程序无关 `instantiate_type`。这解锁 `{v:T}`→`{v:number}`（切片 4）与 `(x:T)=>U`→`(x:number)=>U`（切片 3）。
- **covariant 解析 = Go `getCommonSupertype`**：单候选直接返回；同基底字面量（`literalTypesWithSameBaseType`）⇒ `getUnionType`；否则 `getSingleCommonSupertype`（`findLeftmostType` 取「其后无更上类型」的最左者）。再 `getWidenedType`（对可达字面量/原始类型为恒等）。**不**做 `widenLiteralTypes`（可达 `:T` 返回位置该条件恒 false）与 `PriorityImpliesCombination`。
- **约束臂**：covariant 结果若 `!is_type_assignable_to(inferred, constraint)` ⇒ 改用约束（Go 的简化：略 `getTypeWithThisArgument` / 返回类型 `filteredByConstraint` 投机 / `nonFixingMapper` 约束实例化）。
- **无候选兜底**：默认（`getDefaultFromTypeParameter`）→ 否则 `unknownType`（`InferenceFlagsNone`，非 `AnyDefault`）。
- **4e 低层 `infer_type_arguments`/`get_inferred_type`/`get_best_common_type` 不动**：保留其既有语义与单测（其 `get_best_common_type` 对异基底候选给联合，**与 Go `getCommonSupertype` 不同**——属 4e 既有简化，本轮调用路径改用忠实的 `get_inferred_types_for_call`，不复用低层解析；两者并存且各自有测）。

**InferencePriority 标志（实现 vs 推迟）**：
- **实现（可达）**：基础候选累积 + covariant 解析（`InferencePriority::NONE` 单一优先级；`InferenceContext`/`InferenceInfo` 每签名类型参数一槽）。
- **推迟**：contravariant 候选/位（`contraCandidates`/`getContravariantInference`/`inferFromContravariantTypes`）、优先级格（`NAKED_TYPE_VARIABLE`/`RETURN_TYPE`/`HomomorphicMappedType`/`LiteralKeyof`/`SubstituteSource`/...）与 `inferToMappedType`/reverse-mapped、`priority < inference.priority` 时清候选、`topLevel`/`isFixed` 翻转、`PriorityImpliesCombination`、circularity/expanding-flags/visited 深度（仅保留 4e 既有 visited 破环）。blocked-by: 函数类型/签名推断（`inferFromSignatures`）、conditional/mapped/keyof（C-C）、contextual-return（C-B3）。

**本轮交付**：
- `core/inference.rs`：新增 `get_inferred_types_for_call`/`get_inferred_type_for_call`（`pub(crate)`，调用路径解析 = covariant + 约束 + 默认/unknown）+ `get_covariant_inference`/`get_common_supertype`/`get_single_common_supertype`/`find_leftmost_supertype`/`literal_types_with_same_base_type`（私有）；import `get_constraint_of_type_parameter`/`get_default_from_type_parameter`。4e 低层不改。
- `core/check.rs`：`check_call_expression` 加「泛型 + 无显式类型实参」推断分支 → `resolve_inferred_type_argument_signature`（推断 + 实例化 + 记忆）+ `infer_type_arguments_for_call`（两遍）+ `check_expression_for_inference`（回滚诊断）；`get_type_at_position` 改走 `instantiate_param_type` + 新 `instantiate_anonymous_object_type`/`instantiate_signature_list`；`get_base_type_of_literal_type` 由私有改 `pub(crate)`；自由函数 `is_context_sensitive_argument`；import `InferenceContext`。
- `core/contextual.rs`：`get_resolved_signature_for_contextual_argument` 先查 `resolved_signatures` 缓存（命中返实例化签名）；`try_get_type_at_position` 改走 `instantiate_param_type`。
- `core/mod.rs`：`Checker` 加 `resolved_signatures: FxHashMap<NodeId, SignatureId>` 字段（调用节点 → 实例化签名）。
- 测试：`check_test.rs`（+13 端到端 C-B2）、`inference_test.rs`（+7 引擎单测）。

**新公开 API 形状（additive-only 确认）**：无既有 `pub fn` 签名变更、无 `lib.rs`/依赖/`.go`/`internal/ast` 改动。新增项全 `pub(crate)`/私有/字段：`get_inferred_types_for_call`/`get_inferred_type_for_call`/`instantiate_param_type`（`pub(crate)`）、`get_base_type_of_literal_type`（私有→`pub(crate)`，仅放宽 crate 内可见性）、`Checker.resolved_signatures`（私有字段）、其余私有 `fn`。`cargo build -p tsgo_compiler` 绿（消费 crate 无感）。

**测试增量**：**620 单测**（+20，相对 C-B1 基线 600：`check_test`+13、`inference_test`+7）+ **144 doctest**（不变；新 `pub(crate)` 方法不带 runnable doctest）。**无既有测试弱化/删除**（C-B1 600 全绿；新行为均新增测试）。

**gate（实测，均已 RUN）**：`cargo test -p tsgo_checker`（620 lib + 144 doc 绿）；`cargo clippy -p tsgo_checker --all-targets -- -D warnings` 干净；`cargo fmt -p tsgo_checker -- --check` 干净；`cargo build -p tsgo_compiler` 绿。

**与 Go 的已知偏离（本轮，记录在案）**：
- **回调结果类型 `U` 未推断**：`map([1,2],(x):string=>x)` 本 port 仅推 `T`（回调 `x:number` 正确），`U` 无候选 ⇒ 结果 `U[]`=`unknown[]`（Go 从回调返回类型推 `U=string` ⇒ `string[]`）。blocked-by: 匿名箭头函数自身类型 + 函数体返回类型推断 + `inferFromSignatures`（C-B3）。
- **contextual-return 推断缺失**：`const n:number=f<T>()` 在 Go 经 contextual 返回类型推 `T=number`（无错），本 port `T=unknown`（`const n:number=f()` 会报 2322）。blocked-by: contextual-return 推断（C-B3）。
- **`unknown` 属性访问诊断码**：`f().toFixed()`（`f()`=`unknown`）cmd/tsgo 报 `2571`「Object is of type 'unknown'.」，本 port 进到属性查找报 `2339`——既有「属性访问于 `unknown`」差异，非本轮推断范畴（被推断类型确为 `unknown`，切片 6 仅断言此）。

**本轮 DEFER（带 blocked-by）**：
- **contextual-return 类型推断**（从调用的上下文类型对签名返回类型推断）+ **实例化缓存**（`(type,mapper)` / 签名 type-list key）：C-B3。blocked-by: lazy `InferenceTypeMapper` + 可哈希 mapper key + `getContextualType`→返回类型推断。
- **从 context-sensitive 实参（回调）的返回类型推断 + 函数类型推断 `inferFromSignatures`/`inferFromSignature`**（参数 contravariant、返回 covariant）：C-B3。blocked-by: 匿名函数自身类型 + 体返回推断。
- **contravariant 推断 + 优先级格 + `inferToMappedType`/reverse-mapped + conditional/mapped/keyof 推断**：C-C。blocked-by: 这些类型构造子 + 4bp variance marker-probe。
- **精确 `isContextSensitive`**（全注解函数非敏感；object/array 字面量含敏感元素亦敏感）、**this/rest/spread 实参推断**、**嵌套 anonymous object 深实例化**（成员类型本身为 anonymous object 时仍走程序无关 `instantiate_type` 返原值）、**成员可选性保留**（重建合成属性仅复制 meaning flags）：blocked-by 见各 `// DEFER` 锚。
- **JSX 推断**（`inferJsxTypeArguments`）：blocked-by JSX 签名。

**conformance 切片（登记，端到端对拍仍在 P10）**：`tests/cases/conformance/types/typeInference/` 与 `generics/genericFunctionInference` 的**单态、从实参推断**最小子集（恒等 / 同型参联合 / 对象属性 / 数组+回调参数定型 / 约束应用 / 无候选）——无 contextual-return、无 `U`-从回调、无 conditional/mapped/keyof、无 contravariant 优先级。

**推荐下一轮（C-B3）**：(a) **contextual-return 推断**（`getContextualType`→签名返回类型，`InferencePriority::RETURN_TYPE`）；(b) **回调返回类型推断 `U`**（匿名函数自身类型 + 体返回推断 + `inferFromSignatures`）；(c) **实例化缓存**（lazy `InferenceTypeMapper` + type-list key）。

## C-B3 落地记录（worklog 摘要）—— 泛型调用解析收口：回调结果推断 + 上下文-return 推断 + lazy InferenceTypeMapper + 实例化缓存（**C-B 泛型基石可达子集完成**）

**目标**：把 C-B2 显式 DEFER 的四件事在一轮内逐切片红→绿打通，收口泛型调用解析（C-B 基石）：(1) **回调结果 `U` 推断**——`map<T,U>(a:T[],f:(x:T)=>U):U[]` 调 `map([1,2], x=>x+"")` 从回调**体返回类型**推 `U=string` ⇒ 结果 `string[]`（C-B2 只推 `T`、`U=unknown` ⇒ `unknown[]`）；(2) **上下文-return 推断**——`make<T>():T[]` 调 `const xs:number[]=make()` 从上下文类型推 `T=number`（无实参）；(3) **lazy `InferenceTypeMapper`** 可达实现——回调参数 `(x:T)=>U` 用已推断的 `T` 定型为 `(x:number)=>U`、`U` 保持开放；(4) **实例化缓存**——`create_type_reference` 按 `(target, type-args)` interning。逐切片红→绿；单 lane；cargo 仅 `-p tsgo_checker`（+ `cargo build -p tsgo_compiler` 验 additive）；未 `git commit`。

**Go 真值（ground truth，`go build -o /tmp/tsgo ./cmd/tsgo` 实跑 `--noEmit --strict` 逐一快照；本 port 无 lib，回调体改用等价 lib-free 写法并记录对应）**：
- 头条 `map([1,2], x=>x.toFixed())` 后 `const n:number=r` → `(3,7) 2322`「Type 'string[]' is not assignable to type 'number'.」。lib-free 等价 `x=>x+""`（`number+string`⇒`string`）同样 `string[]`（实测 s7）。
- `map([1,2], x=>x)` 后 `const s:string[]=map([1,2],x=>x)` → `(3,7) 2322`「Type 'number[]' is not assignable to type 'string[]'.」(+「Type 'number' is not assignable to type 'string'.」elaboration)（`U=T=number`）。
- `make<T>():T[]`：`const xs:number[]=make()` → **0**（contextual-return 推 `T=number`）；`const ys:string[]=make<number>()` → `(4,7) 2322`「Type 'number[]' is not assignable to type 'string[]'.」（显式实参仍胜）。
- 嵌套 `map(map([1,2],x=>x), y=>y.toFixed())` → `string[]`；lib-free `y=>y+""` 同样 `string[]`（实测 s8）。
- 字面量体加宽 `map([1,2], x=>"s")` → `string[]`（`"s"` 加宽为 `string`，实测 s5）。

**Go 函数镜像（`// Go:` 锚）**：`inferTypeArguments`（contextual-return 块 + 实参循环 context-sensitive 臂）、`inferFromTypes`（`priority < inference.priority` 清候选）、`inferFromSignatures`/`inferFromSignature`/`applyToParameterTypes`/`applyToReturnTypes`、`newInferenceTypeMapper`(fixing)、`getReturnTypeFromBody`、`couldContainTypeVariables`、`getTypeReferenceType`(interning)、`newInferenceInfo`（`priority=InferencePriorityMaxValue` 种子）。

**严格 TDD（逐切片 red→green；每切片先实测红的对的原因，再最小绿）**：

| # | 切片 | 最小 input → observable（实测红）| 实现触点 |
|---|---|---|---|
| 1 | `generic_call_infers_callback_result_from_body` | `map([1,2],x=>x+"")` 红：`r`=`Array<unknown>`（C-B2 `U=unknown`），应 `Array<string>` | phase-3 context-sensitive 推断 + `infer_from_signatures`（return covariant）+ `get_return_type_from_body` + `get_fixing_inference_mapper` |
| 2 | `generic_call_callback_identity_infers_param_and_result` | `map([1,2],x=>x)`→`const s:string[]=` 验证 `U=number` ⇒ `Array<number>` | 同上（`U=T` 路径）|
| 3 | `generic_call_contextual_return_inference` | `make<T>():T[]`，`const xs:number[]=make()` 红：报 `Array<unknown>` vs `Array<number>`，应 **0** | phase-1 `infer_from_contextual_return_type`（`RETURN_TYPE` 优先级）+ 优先级种子 `MAX_VALUE` |
| 4 | `generic_call_nested_callbacks_infer_result` | `map(map([1,2],x=>x),y=>y+"")`→`Array<string>` | 嵌套解析复用 lazy mapper |
| 5 | `create_type_reference_interns_by_target_and_arguments` | 同实例化返同 id | `create_type_reference` interning 缓存 |
| 单测 | `infer_from_signatures_infers_parameter_and_return` / `infer_with_priority_stronger_replaces_weaker` / `could_contain_type_variables_classifies_types` / `fixing_inference_mapper_maps_inferred_and_skips_empty` / `get_return_type_from_body_widens_concise_literal` | 见 tests.md | 新机器直测 |

> 红→绿证据：切片 1/3 **genuine RED**（切片 1 `r=Array<unknown>` 暴露 `U` 未从回调体推断；切片 3 `make()` 报 `Array<unknown>` 暴露无 contextual-return 推断；尤其切片 3 在加 `MAX_VALUE` 种子前**仍红**——`RETURN_TYPE(128)` 既非 `<NONE(0)` 也非 `==NONE(0)` ⇒ 候选不入，定位到 Go `newInferenceInfo` 的 `MaxValue` 种子）。切片 2/4 在切片 1 机器上验证 `U=T` 与嵌套。切片 5 直测 interning。

**可达裁剪（faithful-but-reachable）**：
- **三相位 `infer_type_arguments_for_call`**：phase-1 contextual-return（`RETURN_TYPE` 优先级，低于实参）→ phase-2 非 context-sensitive 实参（`NONE`）→ phase-3 context-sensitive 回调（用至此推断结果实例化参数类型定型回调，再 `infer_types` 回调函数类型 vs 泛型参数类型）。Go 单遍循环靠 lazy mapper 在每实参处即时定型；本 port 显式分两相以贴合既有「实例化签名记忆到节点」架构，等价于 Go 数组先于回调的顺序（可达用例数组恒先于回调）。
- **lazy `InferenceTypeMapper` 可达实现 `get_fixing_inference_mapper`**：对**有候选**的槽映射到当前推断类型（`get_inferred_type_for_call`，即「fix」）、**无候选**槽省略（保持自身），故 `(x:T)=>U` 实例化为 `(x:number)=>U`、`U` 不被提前解析为 `unknown`（否则阻断回调结果推断）。Go 的真·按需 fixing mapper（首次访问才 fix + `clearCachedInferences`）DEFER。
- **`inferFromSignature` 参数臂**：Go 走 `inferFromContravariantTypesIfStrictFunctionTypes`（contra 候选）；本 port 可达子集用 covariant `infer_from_types`——回调参数处理时其类型变量已被数组实参 fix，故参数臂是「`number→T`」的幂等再推断（无新信息）。full contravariant 候选格 DEFER(C-C)。
- **优先级 lattice 最小子集**：`infer_types_with_priority` 实现 `priority < inference.priority ⇒ 清候选 + 设优先级`、`== ⇒ 加候选`、`> ⇒ 忽略`；`InferenceInfo` 种子 `MAX_VALUE`（Go `newInferenceInfo`）。仅用 `NONE`/`RETURN_TYPE` 两档：实参（`NONE`）覆盖 contextual-return（`RETURN_TYPE`），故 `id(1)`/`pick(1,2)` 在有上下文注解时仍由实参定型（不回归）。完整 lattice（mapped/keyof/literal/substitution 位 + `topLevel`/`isFixed` 翻转）DEFER(C-C)。
- **`get_return_type_from_body` 可达子集**：concise 体 = 体表达式类型（加宽：`get_widened_literal_type` 字面量→基底 + `get_widened_type` 对象字面量去鲜）；block 体 = `return` 表达式类型并集（无 `return`⇒`void`，单层 + if/else 分支）。回调体在推断相位检查时回滚诊断（应用相位重检并报一次）。async/generator/never/contextual-`undefined`/literal-preservation DEFER(C-C)。
- **`could_contain_type_variables` 可达子集**：类型参数 / union·intersection 成员 / 泛型引用按**类型实参**递归 / anonymous object 按 call 签名参数·返回。Go 的 objectFlags 缓存 + mapped/conditional 构造子 DEFER(C-C)。
- **实例化缓存 = 类型引用 interning**：`create_type_reference` 按 `(target, type-args)` 缓存返稳定 id（Go `getTypeReferenceType` interning，即 `(type,mapper)` 实例化缓存的可达形）。引用共享 target 成员、创建后不改自身对象数据（已核对全部调用点不 mutate），故共享 id 安全。`(type, 任意 mapper)` 通用缓存（mapper 不廉价可哈希）DEFER(C-C)。

**本轮交付**：
- `core/inference.rs`：`InferencePriority::MAX_VALUE`；`InferenceInfo::new` 种子 `MAX_VALUE`；`infer_types_with_priority`（`pub(crate)`，优先级 lattice 子集）；`infer_from_types` 加 `priority` 形参 + object 臂调 `infer_from_signatures`；新 `infer_from_signatures`/`infer_from_signature`/`could_contain_type_variables`（`pub(crate)`）；`infer_types` 委托 `infer_types_with_priority(NONE)`。
- `core/check.rs`：`infer_type_arguments_for_call` 改三相位（加 `node` 形参）；新 `infer_from_contextual_return_type`/`infer_from_context_sensitive_argument`/`get_type_of_context_sensitive_arrow`/`get_return_type_from_body`/`get_fixing_inference_mapper`；`get_type_at_position` 改 `pub(crate)`；自由函数 `function_like_parameters`/`function_like_body`/`arrow_return_type_node`/`is_optional_parameter`/`collect_return_expressions(_into)`；import `InferencePriority`/`Signature`/`SignatureFlags`。
- `core/relations.rs`：`signature_return_type` 改 `pub(crate)`（供 inference 复用）。
- `core/mod.rs`：`Checker.type_reference_cache` 字段；`create_type_reference` interning。
- 测试：`check_test.rs`（+4 端到端 + 2 单测）、`inference_test.rs`（+3 单测，1 既有改）、`mapper_test.rs`（+1 单测）。

**新公开 API 形状（additive-only 确认）**：无既有 `pub fn` 签名变更、无 `lib.rs`/依赖/`.go`/`internal/ast` 改动。新增/放宽项：`InferencePriority::MAX_VALUE`（新 `pub const`）、`infer_types_with_priority`/`infer_from_signatures`/`could_contain_type_variables`（`pub(crate)`）、`get_type_at_position`/`signature_return_type`（私有→`pub(crate)` 放宽）、`Checker.type_reference_cache`（私有字段）、其余私有 `fn`。`infer_types` 公开签名不变（行为不变，仍 `NONE` 优先级）。`cargo build -p tsgo_compiler` 绿。

**测试增量**：**630 单测**（+10，相对 C-B2 基线 620：`check_test`+6【slice1/2/3/4 + fixing-mapper + return-from-body】、`inference_test`+3【infer_from_signatures + priority + could_contain】、`mapper_test`+1【interning】）+ **144 doctest**（不变；新 `pub(crate)` 方法不带 runnable doctest；`InferenceInfo` doc 例 +1 断言 `MAX_VALUE`）。

**既有测试更新（Go-verified justification）**：`inference_test.rs::inference_context_and_info_construction` 由断言 `info.priority == NONE` 改为 `== MAX_VALUE`——旧值是本 port 的简化（C-B2 无优先级 lattice），Go `newInferenceInfo` 实际种子 `InferencePriorityMaxValue`(1<<11)；不改种子则低优先级的 contextual-return 候选无法入槽（切片 3 红）。无既有测试弱化/删除（C-B2 620 全绿，新行为均新增测试）。

**gate（实测，均已 RUN）**：`cargo test -p tsgo_checker`（630 lib + 144 doc 绿）；`cargo clippy -p tsgo_checker --all-targets -- -D warnings` 干净；`cargo fmt -p tsgo_checker -- --check` 干净；`cargo build -p tsgo_compiler` 绿。

**与 Go 的已知偏离（本轮，记录在案）**：
- **数组类型印名 `Array<X>` vs `X[]`**：cmd/tsgo 印 `string[]`/`number[]`，本 port nodebuilder 无 `X[]` 糖 ⇒ 印底层引用 `Array<string>`/`Array<number>`。属既有 nodebuilder 局限（非 C-B3 推断范畴）；被推断的**元素类型**（`string`/`number`/`boolean`）一致——即 C-B3 确立的语义。blocked-by: nodebuilder array 印形。
- **数组↔数组 2322 elaboration chain**：cmd/tsgo 在 `number[]`/`string[]` 不可赋时附「Type 'number' is not assignable to type 'string'.」嵌套链；本 port 顶层 2322 文本一致、元素级 message-chain 对数组引用尚未物化（既有 relation chain 局限）。blocked-by: 数组引用关系的 message-chain elaboration。
- **lib 依赖回调体**：头条 `x.toFixed()` 需真 `Number.toFixed`（lib，P6）；本 port 用 lib-free 等价 `x+""`（⇒`string`）测 callback-return 推断；语义（U 从回调体）一致。

**本轮 DEFER（带 blocked-by）**：
- **full contravariant 推断 + 完整 `InferencePriority` lattice**（`contraCandidates`/`getContravariantInference`/`inferFromContravariantTypes`、mapped/keyof/literal/substitution 优先级位、`topLevel`/`isFixed` 翻转、`PriorityImpliesCombination`）：C-C。blocked-by: 变型 marker-probe + 这些构造子。
- **mapped/conditional/keyof/template/reverse-mapped 推断**：C-C。blocked-by: 这些类型构造子。
- **嵌套泛型上下文 `outerMapper`/`returnMapper` + intra-expression inference sites + 真·按需 fixing mapper**：C-C。blocked-by: 外层推断穿线 + context-capturing mapper 变体。
- **精确 `isContextSensitive`**（全注解函数非敏感；object/array 字面量含敏感元素亦敏感）、**this/rest/spread 实参推断**、**async/generator/never 体返回推断**：blocked-by 见各 `// DEFER` 锚。
- **`(type, mapper)` 通用实例化缓存**：仅做了引用 interning；通用缓存需可哈希 mapper key。blocked-by: 可哈希 mapper key。
- **JSX 推断**（`inferJsxTypeArguments`）：blocked-by JSX 签名。

**conformance 切片（登记，端到端对拍仍在 P10）**：`tests/cases/conformance/types/typeInference/genericCallWithObjectTypeArgs*` 与 `generics/genericFunctionInference` 的**回调结果推断 + contextual-return**最小子集（map-风格回调 / make-风格 contextual-return / 嵌套调用 / 字面量体加宽）——无 conditional/mapped/keyof、无 full contravariant lattice、无嵌套外层 mapper。

**C-B（泛型基石）状态**：**可达子集完成**。C-B1（显式类型实参 + 实例化/TypeMapper）+ C-B2（从实参推断类型实参）+ C-B3（回调结果推断 + contextual-return + lazy InferenceTypeMapper + 实例化缓存）三轮收口泛型调用解析的可达路径；剩余高级推断（conditional/mapped/keyof、full variance/priority lattice、嵌套外层 mapper、JSX）归 C-C/later。

## C-C1 落地记录（worklog 摘要）—— keyof/index 类型 + 索引访问 `T[K]` 深化（高级类型 part 1）

> 在 C-B（泛型基石）之上落地 **keyof（index 类型算子）+ 索引访问 `T[K]` 深化**。每个行为一条 `/tdd` 红→绿垂直切片。**先建类型基建** → 再 `get_index_type` → 再 `get_indexed_access_type` 深化 → 再实例化臂 → 再节点接线 → 再印名 → 最后端到端串联（keyof + 索引访问 + C-B 推断）。

**Go ground truth（`cmd/tsgo --strict` 实测，断言前捕获）**：
- 切片1 `type K = keyof { a: number; b: string }; const k: K="a"; const k2: K="c";` → 仅 `(3,7): error TS2322: Type '"c"' is not assignable to type '"a" | "b"'.`（行2 接受）。证 `keyof` 具体对象 = `"a" | "b"`。
- 切片2 `type T = { a: number }; const x: T["a"]=1; const y: T["a"]="s";` → 仅 `(3,7): error TS2322: Type 'string' is not assignable to type 'number'.`。证 `T["a"]` = `number`。
- 切片3（头条）`function get<T, K extends keyof T>(o: T, k: K): T[K] { return o[k]; } const r = get({ a: 1, b: "x" }, "a"); const s: string = r;` → 仅 `(3,7): error TS2322: Type 'number' is not assignable to type 'string'.`。证 `r` = `number`（T 推为 `{a:number,b:string}`、K 保字面量 `"a"`、`T[K]` 实例化为 `number`）。
- 切片5 `... const r = get({ a: 1 }, "b");` → `(2,25): error TS2345: Argument of type '"b"' is not assignable to parameter of type '"a"'.`。证非键索引：约束实例化为 `keyof {a:number}="a"`，K 钳到 `"a"`，实参 `"b"` 不可赋 `"a"`。
- 空体 `function get<T,K extends keyof T>(o:T,k:K):T[K]{}` → `2355`（故各切片函数体用 `{ return o[k]; }`）。

**RED→GREEN 证据（每切片实测）**：
- `get_index_type`（具体）：函数缺失 ⇒ 编译红 → 实现具体臂 `get_literal_type_from_properties` 绿。
- `get_index_type`（泛型）：同函数延迟臂；`keyof T`(类型参数) → 延迟 `Index` + interning。
- `get_indexed_access_type`（泛型延迟）：`T[K]` 旧返回 `None`（K 非 string/number-like）→ RED；加 `should_defer_indexed_access_type` 后返回延迟 `IndexedAccess` 绿。
- `get_indexed_access_type`（union-index 分配）：`I["a"|"b"]` 旧 `None` RED → 加 union 分配 + 命名属性查找 `get_property_type_for_index_type` 绿（`number | string`）。
- 实例化臂：`instantiate_type(keyof T)` / `instantiate_type(T[K])` 旧恒等返回延迟类型 → 加 `Index`/`IndexedAccess` 臂绿。
- `could_contain_type_variables(keyof T)`：旧返回 false（无 INSTANTIABLE 臂）RED → 加 `INSTANTIABLE` 臂绿。
- 节点接线：`type K = keyof I` 旧得 `error`（TypeId(3)）RED → 接 keyof 臂绿；`type A = I["a"]` 同。
- 端到端切片1：**genuine RED**——首跑报 `Type 'string' is not assignable to type '"a" | "b"'.`（源字面量 `"c"` 被错误加宽为 `string`），定位到 `type_could_have_top_level_singleton_types` 不迭代 union 成员（4m 子集遗留），修正后绿。
- 端到端切片3（头条）：**genuine RED**——首跑报 **2 条**诊断（`2345: Argument of type 'string' ... parameter of type 'keyof T'` + `2322: Type '{ a: number; b: string; }[keyof T]' ...`），暴露 `get_inferred_type_for_call` 用**未实例化**约束 `keyof T` 钳 K（K 被钳成延迟 `keyof T`）；加 `inference_context_mapper` 实例化约束后 K 保 `"a"`，`T[K]` 实例化为 `number`，回单条 2322 绿。
- 端到端切片5：切片3 修复后直绿（约束实例化为 `"a"`，K 钳 `"a"`，实参 `"b"`→2345，源字面量 `"b"` 经 `generalized_source_for_error` 因 target `"a"` 是 unit 而**不加宽**，印 `'"b"'` 对齐 Go）。

**类型基建（`core/types.rs`+`core/mod.rs`）**：
- 新 `TypeData::Index(IndexType{ target, index_flags })` / `TypeData::IndexedAccess(IndexedAccessType{ object_type, index_type, access_flags })`（`TypeFlags::INDEX`/`INDEXED_ACCESS` 已存在于 4a 位表）。
- 新 `IndexFlags`/`AccessFlags` bitflags（1:1 Go `types.go`，可达只用 `NONE`，其余为忠实保真）。
- 新访问器 `Type::as_index`/`as_indexed_access`；构造器 `Checker::new_index_type`/`new_indexed_access_type`。
- interning 缓存 `Checker.index_types`（按 target）/`indexed_access_types`（按 `(object,index)`），使 `keyof T`/`T[K]` 跨解析 id 稳定（关系恒等 + 实例化幂等所需）。

**可达裁剪（faithful-but-reachable）**：
- **`should_defer_index_type`**：仅 `flags & INSTANTIABLE_NON_PRIMITIVE`（类型参数/索引访问/条件/替换延迟）。泛型 tuple/mapped、reducible union、instantiable intersection 臂 DEFER(C-C3)。
- **`should_defer_indexed_access_type` 统一形**：`is_generic_index_type(index) || is_generic_object_type(object)`，对齐**类型节点**路径。Go 的 accessNode-kind 区分（element-access **表达式**对「泛型对象 + 非泛型键」经约束**急切**解析、类型节点延迟）+ 泛型 tuple 定长急切 DEFER：可达用例（已移植 element-access 测试）要么操作数具体、要么索引泛型，两路均得 Go 结果（`get_indexed_access_type` 公开签名 additive 不可改，故不引 accessNode 形参）。
- **`get_property_type_for_index_type`**：apparent → any/never 短路 → tuple 定长/number → **命名属性**（string 字面量索引 via `get_type_of_property_of_type`）→ index 签名。numeric 字面量命名属性（`{0:T}[0]`，非 tuple）、`Contextual`/`Writing`/`IncludeUndefined` access flags、accessNode 错误臂（2536/2538/7053）DEFER。
- **`get_literal_type_from_properties`**：`getPropertiesOfType` 名 + index-info 键（`string`+Number⇒`string|number`）。`includeOrigin` 别名索引、properties-types 缓存、enum-number index 排除 DEFER(C-C3)。
- **`get_literal_type_from_property`**：可达只 string 字面量名。`nameType`（computed/late-bound）、numeric 字面量名、`default`、非公开过滤 DEFER。
- **union/boolean 守卫**：本 port `boolean` = 纯 `false|true` union 无 `BOOLEAN` 位，故 union-index 分配守卫用 `index != boolean_type`；`type_could_have_top_level_singleton_types` 同理顶层加 `t == boolean_type ⇒ false`。

**推断约束实例化（`core/inference.rs`，解头条）**：`get_inferred_type_for_call` 经新 `inference_context_mapper`（context 各类型参数→至今推断类型 / 自身）把约束实例化后再钳（Go `nonFixingMapper`）。槽按序解（T 先于 K），故 K 的 `keyof T` 约束实例化时 T 已定。`getTypeWithThisArgument` + 返回类型 `filteredByConstraint` 推测 DEFER(C-C2)。

> **字面量加宽说明**：本 port `get_covariant_inference` 用 `get_widened_type`（仅去对象字面量鲜度，**不**做 Go 的 `getWidenedLiteralType` 字面量→基底加宽），故 K 的字面量候选 `"a"` 天然保留——Go 用 `hasPrimitiveConstraint(keyof T)` 抑制加宽达同效果。完整 `getWidenedLiteralType` + `hasPrimitiveConstraint`/`widenLiteralTypes` 守卫归 C-C（与 inference 字面量加宽一并），本轮无需新增守卫（既有 `get_widened_type` 行为即满足切片）。

**本轮交付**：
- `core/types.rs`：`IndexFlags`/`AccessFlags` bitflags；`IndexType`/`IndexedAccessType` struct；`TypeData::Index`/`IndexedAccess`；`Type::as_index`/`as_indexed_access`。
- `core/mod.rs`：`Checker.index_types`/`indexed_access_types` 字段 + init；`new_index_type`/`new_indexed_access_type`；程序无关 `type_to_string` 加 `Index`/`IndexedAccess` 臂。
- `core/declared_types.rs`：`get_index_type`（`pub`）+ `get_index_type_ex`/`should_defer_index_type`/`get_index_type_for_generic_type`/`get_literal_type_from_properties`/`get_literal_type_from_property`/`is_key_type_included`；`get_indexed_access_type` 重构（延迟 + union 分配）+ `get_property_type_for_index_type`/`should_defer_indexed_access_type`/`is_generic_index_type`/`is_generic_object_type`/`union_or_intersection_members`/`get_or_create_indexed_access_type`/`get_property_name_from_index`；`get_type_from_type_node` 加 `IndexedAccessType` 臂 + `get_type_from_indexed_access_type_node`；`get_type_from_type_operator_node` 加 keyof 臂。
- `core/mapper.rs`：`instantiate_type_worker` 加 `Index`/`IndexedAccess` 臂（后者经 `retained_program` 解析属性）。
- `core/inference.rs`：`could_contain_type_variables` 改 `INSTANTIABLE` 臂；`get_inferred_type_for_call` 约束实例化 + `inference_context_mapper`。
- `core/nodebuilder.rs`：程序感知 `type_to_string` 加 `keyof X`/`X[Y]` 臂。
- `core/check.rs`：`type_could_have_top_level_singleton_types` 加 union/intersection 迭代 + boolean 守卫（修字面量-union target 的源加宽 bug）。
- `lib.rs`：re-export `get_index_type`、`AccessFlags`/`IndexFlags`/`IndexType`/`IndexedAccessType`（additive）。
- 测试：`declared_types_test.rs`（+6）、`mapper_test.rs`（+2）、`inference_test.rs`（+1）、`nodebuilder_test.rs`（+1）、`check_test.rs`（+4 端到端）。

**新公开 API 形状（additive-only 确认）**：无既有 `pub fn` 签名变更、无 `lib.rs` 既有项移除、无依赖/`.go`/`internal/ast` 改动。新增 additive：`TypeData::Index`/`IndexedAccess`（枚举变体）、`IndexType`/`IndexedAccessType`/`IndexFlags`/`AccessFlags`（新 `pub`，lib.rs re-export）、`Type::as_index`/`as_indexed_access`/`Checker::new_index_type`/`new_indexed_access_type`/`get_index_type`（新 `pub`）。其余新增 `fn` 均私有；新字段私有。`get_indexed_access_type`/`get_type_from_type_node`/`instantiate_type`/`could_contain_type_variables` 公开签名不变（行为增量）。`cargo build -p tsgo_compiler` 绿。

**测试增量**：**644 单测**（+14，相对 C-B3 基线 630：`declared_types_test`+6【keyof 具体/泛型、indexed-access 泛型延迟/union 分配、keyof 节点、indexed-access 节点】、`mapper_test`+2【Index/IndexedAccess 实例化臂】、`inference_test`+1【could_contain Index/IndexedAccess】、`nodebuilder_test`+1【keyof/索引访问印名】、`check_test`+4【端到端切片 1/2/3/5】）+ **151 doctest**（+7：`IndexFlags`/`AccessFlags`/`IndexType`/`IndexedAccessType`/`new_index_type`/`new_indexed_access_type`/`get_index_type` 各 1）。

**既有测试更新（Go-verified justification）**：无既有测试弱化/删除。`type_could_have_top_level_singleton_types` 改为迭代 union 成员属**修 bug**（4m 子集遗留），无既有测试断言其旧（Go-错误）行为——C-B3 的 630 单测全绿不变。

**gate（实测，均已 RUN）**：`cargo test -p tsgo_checker`（644 lib + 151 doc 绿）；`cargo clippy -p tsgo_checker --all-targets -- -D warnings` 干净；`cargo fmt -p tsgo_checker -- --check` 干净；`cargo build -p tsgo_compiler` 绿。

**本轮 DEFER（带 blocked-by）**：
- **条件类型** `T extends U ? X : Y`（`TypeData::Conditional`、`getConditionalType`、`getConditionalTypeInstantiation`、`getDefaultConstraintOfConditionalType`）：**C-C2**。blocked-by: 条件类型构造子 + distributive 语义 + infer 位置。
- **mapped 类型 + template literal**（`{ [K in keyof T]: ... }`、`getIndexTypeForMappedType`、`` `${T}` ``、`getTemplateLiteralType`、`getStringMappingType`、reverse-mapped 推断）：**C-C3**。blocked-by: mapped/template/string-mapping 构造子。
- **keyof over mapped 类型**：blocked-by mapped 类型（C-C3）。
- **索引访问 on intersection / index-signature-only 类型**（超可达）、**`noUncheckedIndexedAccess`**（`AccessFlagsIncludeUndefined`）、**reverse-mapped 推断**：blocked-by 各构造子 + option 接线。
- **`getConstraintOfIndexedAccessType` / `getSimplifiedType` 化简机**（超切片所需）、**accessNode-kind 急切解析 + 泛型 tuple 定长索引**、**numeric 命名属性 / computed-name 字面量**：blocked-by 化简机 + accessNode 接线 + numeric/computed 名类型。
- **full literal 加宽 + `hasPrimitiveConstraint`/`widenLiteralTypes` 守卫**（`getWidenedLiteralType`）：归 C-C 与 inference 字面量加宽一并。blocked-by: inference 字面量加宽路径。
- **`keyof any/never` ⇒ `string|number|symbol`、wildcard 臂**：blocked-by 全局 `stringNumberSymbolType` + wildcard 类型。

**conformance 切片（登记，端到端对拍仍在 P10）**：`tests/cases/conformance/types/keyof/keyofAndIndexedAccess*` 与 `types/members/indexSignatures` 的**具体 keyof / 具体 `T["a"]` / 泛型 `get<T,K extends keyof T>` 推断 / 非键索引 2345** 最小子集——无条件/mapped/template、无 `noUncheckedIndexedAccess`、无化简机。
...
## C-D1 落地记录（worklog 摘要）—— strictNullChecks 完成（可选属性/参数 `| undefined` 注入 + TypeFacts 补全）+ 枚举（成员字面量类型 + 联合 + 关系 + 求值 + 印名）

> 在 4am–4bb（strict-null 起步：赋值性门控 / `get_non_null_type` / `x!` / EQ-NE-nullable facts / `??` 精化 / possibly-null 诊断）之上，**完成可达的 strictNullChecks 面**（可选属性/参数注 `| undefined`、TypeFacts symbol/object 补全），并**新增枚举**（此前 checker 几乎无枚举支持）。每个行为一条 `/tdd` 红→绿垂直切片。**所有 Go 真值经 `go build -o /tmp/tsgo ./cmd/tsgo` 实跑 `--noEmit --strict`/`--strictNullChecks false` 逐一捕获后才断言**；与任务书设想不符处一律以 cmd/tsgo 为准并记录。单 lane；cargo 仅 `-p tsgo_checker`（+ `cargo build -p tsgo_compiler` 验 additive）；未 `git commit`。

**Go 真值（ground truth，已逐一实跑捕获）**：
- 可选属性：`const o: { a?: number } = {}; const n: number = o.a;` → strict `(2,7) TS2322: Type 'number | undefined' is not assignable to type 'number'.`（+ elaboration `Type 'undefined' ...`）；`--strictNullChecks false` → **0**；`const o:{a?:number}={}` 赋 `{}` → **0**（可选属性非必填）。
- 可选参数：`function f(x?: number) { const n: number = x; }` → strict `(1,32) TS2322: Type 'number | undefined' ...`；非 strict → **0**。
- 枚举（数值）：`enum E { A, B } const a: E = E.A; const c: E = 2;` → 仅 `(3,7) TS2322: Type '2' is not assignable to type 'E'.`（`E.A`→`E` OK；`2` 不匹配任一成员值 → 不可赋）。`const c0: E = 0; const c1: E = 1;` → **0**（匹配成员值的数字字面量可赋枚举，位标志规则）。
- 枚举（auto-increment）：`enum E { A = 1, B } const b1: 2 = E.B; const b2: 1 = E.B;` → 仅 `Type 'E.B' is not assignable to type '1'.`（`E.B`=2）。`enum E { A = 10, B, C = 20 } const b2: 12 = E.B;` → `Type 'E.B' is not assignable to type '12'.`（`E.B`=11）。
- 枚举（字符串）：`enum E { A = "x" } const s: string = E.A; const e: E = "x";` → 仅 `Type '"x"' is not assignable to type 'E'.`（单成员枚举印 `E`；`E.A`→`string` OK；`"x"`→`E` 失败）。
- 枚举成员引用：`enum E { A, B } const n: number = E.A; const e: E = E.B; const bad: 5 = E.A;` → 仅 `Type 'E.A' is not assignable to type '5'.`（`E.A`→`number`/`E` OK；`E.A`(0)→`5` 失败）。多成员枚举成员印 `E.A`；单成员枚举成员印 `E`（实测 `enum E{A=1}` 的 `E.A` 印 `E`，对应 Go `getDeclaredTypeOfSymbol(parent)==t` 规则）。
- const enum：`const enum E { A, B } const bad: 5 = E.B;` → `Type 'E.B' is not assignable to type '5'.`（成员定型与普通枚举一致；emit 内联是 transformer 关注点，DEFER）。
- 求值器常量成员：`enum E { A = 1 << 1, B = A | 1 } const b2: 4 = E.B;` → `Type 'E.B' is not assignable to type '4'.`（`A`=2、`B`=`2|1`=3，经 `tsgo_evaluator` 折叠 + 实体引用解析）。
- definite assignment：`let x: number; const y = x;` → `(2,11) TS2454: Variable 'x' is used before being assigned.`（**本轮 DEFER**，见下）。

**Go 函数镜像（`// Go:` 锚）**：`addOptionalityEx`/`getOptionalType`/`getTypeForVariableLikeDeclaration`(可选注入)、`getTypeFactsWorker`(ESSymbolLike/NonPrimitive 臂)、`getDeclaredTypeOfEnum`/`getDeclaredTypeOfEnumMember`/`getTypeOfEnumMember`/`getTypeOfFuncClassEnumModule`(enum)/`getEnumLiteralType`/`createComputedEnumType`/`computeEnumMemberValues`/`computeEnumMemberValue`/`computeConstantEnumMemberValue`/`evaluateEntity`、`isSimpleTypeRelatedTo`(enum 臂)、`resolveMappedTypeMembers`(stripOptional)、nodebuilderimpl.go:`typeToTypeNodeWorker`(EnumLike 印名)。

**严格 TDD（逐切片 red→green；每片先实测红的对的原因，再最小绿）**：

| # | 切片 | 最小 input → observable（实测红） | 实现触点 |
|---|---|---|---|
| 1 可选属性 | `optional_property_read_is_number_or_undefined_under_strict` | `const o:{a?:number}={};const n:number=o.a;` → **2322**（红：`o.a` 此前为裸 `number`，0 诊断；实测红消息 `number | undefined` vs `undefined | number` 暴露端口 union 序，按端口实际断言）| `type_facts.rs`：`add_optionality_ex`/`get_optional_type`（pub(crate)）；`declared_types.rs`：`get_type_of_variable_or_property` 注解臂调 `add_optionality_ex(t, is_property, is_optional)` + 新私有 `is_optional_declaration` |
| 1 守卫×2 | `assign_empty_object_to_optional_property_is_ok` / `optional_property_read_is_bare_type_without_strict_null_checks` | strict `{}`→`{a?}` 0；非 strict `o.a`=裸 `number` 0 | 无（gate 在 `strict_null_checks()`）|
| 2 可选参数 | `optional_parameter_is_number_or_undefined_in_body`（+ 非 strict 守卫）| `function f(x?:number){const n:number=x;}` → **2322**（红：`x` 此前裸 `number`）| 同臂（参数声明经同 `is_optional_declaration` 的 `question_token` 判定）|
| TypeFacts | `type_facts_of_symbol_is_truthy_non_nullable` / `type_facts_of_non_primitive_object_is_truthy_non_nullable` | `get_type_facts(symbol/object)` → `NE_*\|TRUTHY`（红：此前落 `UNKNOWN_FACTS` 含 `FALSY`/`EQ_*`）| `type_facts.rs`：`get_type_facts` 加 `ES_SYMBOL_LIKE`/`NON_PRIMITIVE` 臂（Go `SymbolStrictFacts`/`ObjectStrictFacts`）|
| 3 枚举数值 | `numeric_enum_member_and_union_assignability` | `enum E{A,B};const c:E=2;` → 仅 1×2322 `'2'`/`'E'`（红：枚举此前无声明类型机制 → `E.A` 落 error/`E` 落对象）| `declared_types.rs`：重写 `get_declared_type_of_enum`（成员值→字面量→联合 + `mark_enum_union`）+ `compute_enum_member_values`；`get_type_of_symbol` 加 `ENUM_MEMBER`/`ENUM` 臂（`get_type_of_enum_member`/`get_type_of_enum_object`）；`mod.rs`：`new_enum_literal_type`/`mark_enum_union`/`new_computed_enum_type`；`relations.rs`：`is_simple_type_related_to` 加 enum 臂 + `literal_values_equal`；`nodebuilder.rs`：EnumLike 印名臂 |
| 3 守卫 | `matching_numeric_literal_is_assignable_to_enum` | `const c0:E=0;const c1:E=1;` → 0（数字字面量匹配成员值可赋枚举）| 无（`relations.rs` assignable 分支的 number-literal→enum-literal 臂）|
| 4/5 auto-incr | `enum_auto_increment_after_explicit_initializer` / `..._between_explicit_initializers` | `E{A=1,B}` `E.B`=2；`E{A=10,B,C=20}` `E.B`=11 → 各 1×2322 印 `E.B` | `compute_enum_member_values`（autoValue 链）|
| 6 字符串枚举 | `string_enum_member_and_assignability` | `enum E{A="x"};const e:E="x";` → 1×2322 `'"x"'`/`'E'`（单成员印 `E`）| `get_declared_type_of_enum`（字符串成员 + 单成员不 mark union → 印名走 `getDeclaredTypeOfSymbol(parent)==t`）|
| 7 成员引用 | `enum_member_value_reference_assignability` | `E.A`→`number`/`E` OK，`E.A`(0)→`5` 失败 → 1×2322 印 `E.A`（多成员）| `get_type_of_enum_member` + 印名臂 |
| 8 const enum | `const_enum_member_typing` | `const enum E{A,B};const bad:5=E.B;` → 1×2322 印 `E.B` | 无（const/regular 同路径）|
| 9 求值器 | `enum_constant_expression_members_via_evaluator` | `enum E{A=1<<1,B=A\|1};const b2:4=E.B;` → 1×2322（`B`=3）| `compute_enum_member_values` 经 `tsgo_evaluator::new_evaluator` + 实体回调按名解析前序成员 |
| 单测 | `declared_types_test`：`numeric_enum_declared_type_is_union_of_member_literals` / `enum_member_value_computation`；`type_facts_test`×2 | 见 tests.md | 直测联合构造 + 成员值（explicit/auto/string）+ symbol/object facts |

> 红→绿证据：切片 1/2/3/4/5/6/7/9 均 **genuine RED**（可选注入前属性/参数为裸类型 0 诊断；枚举前无声明类型机制）。切片 3-守卫/8 为 green-on-arrival 覆盖守卫（位标志规则随 relations 臂落地、const/regular 同路径），如实记录。**关键发现**：可选属性消息端口印 `undefined | number`（端口 union 按 id 排序、`undefined` id 在前），cmd/tsgo 印 `number | undefined`——**既有端口 union 序偏离**（与 4ay/4az 一致），断言端口实际值并登记。

**本轮交付（全部 `internal/checker/**`）**：
- `core/type_facts.rs`：`add_optionality_ex`/`get_optional_type`（pub(crate)）；`get_type_facts` 加 `ES_SYMBOL_LIKE`/`NON_PRIMITIVE` 臂。
- `core/declared_types.rs`：`get_type_of_variable_or_property` 注解臂注可选性 + `is_optional_declaration`；重写 `get_declared_type_of_enum`（联合）+ `compute_enum_member_values`/`enum_member_name_text`/`get_type_of_enum_object`/`get_type_of_enum_member`/`get_declared_type_of_enum_member`；`get_type_of_symbol` 加 `ENUM_MEMBER`/`ENUM` 臂；`resolve_mapped_type_members_eager` 加 `-?` stripOptional（`get_type_with_facts(NE_UNDEFINED)`）。
- `core/mod.rs`：`new_enum_literal_type`/`new_computed_enum_type`/`mark_enum_union`（pub(crate)）。
- `core/relations.rs`：`is_simple_type_related_to` 加 4 个 enum 臂（string/number enum-literal→非-enum 同值；number/number-literal→enum）+ `literal_values_equal`。
- `core/nodebuilder.rs`：`type_to_string` 顶部加 EnumLike 印名臂（成员→`E.A`/单成员→`E`；联合/computed→`E`）。
- 测试：`check_test.rs`（+13：可选 5 + 枚举 8）、`type_facts_test.rs`（+2）、`declared_types_test.rs`（+2 新 + 1 更新）、`relations_test.rs`（1 更新）。

**新公开 API 形状（additive-only 确认）**：**本轮无新增任何真 `pub` 项**——所有新方法/函数均 `pub(crate)`/私有；新增的 `add_optionality_ex`/`get_optional_type`/`new_enum_literal_type`/`new_computed_enum_type`/`mark_enum_union` 全 `pub(crate)`，enum declared-type 函数全私有 `fn`，`is_optional_declaration`/`literal_values_equal`/`enum_member_name_text` 私有。无 `lib.rs` 改动、无依赖/`.go`/`internal/ast` 改动（复用既有 `tsgo_evaluator` 依赖）。公开 API 形状与 C-C3 完全一致。`cargo build -p tsgo_compiler` 绿（实测）。

**测试增量**：**696 单测**（+17，相对 C-C3 基线 679：`check_test`+13、`type_facts_test`+2、`declared_types_test`+2）+ **165 doctest**（**±0**：无新 `pub` 项；`new_enum_literal_type` 初版误带 `# Examples` doctest——pub(crate) 方法外部不可调致 doctest 编译失败 E0624，已移除该 `# Examples`，回到无新 doctest）。

**既有测试更新（Go-verified justification，4 处）**：
1. `declared_types_test::declared_type_of_enum_exposes_members`：原断言 `get_declared_type_of_symbol(E)` 为对象（4c 简化把枚举声明类型建成成员对象）。Go 区分值侧（`getTypeOfFuncClassEnumModule`=成员对象）与类型侧（`getDeclaredTypeOfEnum`=成员字面量联合）。改为：值侧 `get_type_of_symbol(E)` 是对象暴露成员 `A`，类型侧 `get_declared_type_of_symbol(E)`（单成员）是带 `ENUM_LITERAL` 的成员字面量（值 0）。
2. `check_test::assignability_chain_optional_source_required_target_reports_2327`：原在 strict 默认下期望 2327（optional-vs-required）。实测 cmd/tsgo：strict 下因 `addOptionalityEx` 使源 `a?:string`→`string | undefined`，属性类型比较先失败 → 报 "Types of property 'a' are incompatible" 链；2327 仅在**非 strict** 出现。该测试意在锁定 2327 臂，故改为非 strict 运行（Go-faithful）。
3. `relations_test::comparable_is_lenient_about_optional_vs_required`：原 `Checker::new()`（默认 strict）断言 `S` comparable `T`。strict 下源可选属性注 `undefined` 使属性类型比较失败（与 `skipOptional` 无关）。该测试意在隔离 `skipOptional`，故改非 strict 运行（成员保持裸 `string`，仅 optional/required 差异）。
4. `check_test::mapped_required_strips_optional_e2e` + `declared_types_test::instantiate_mapped_type_required_strips_optional`：可选注入后 `Required<{a?:number}>` 的 `T[K]` 读为 `number | undefined`；按 Go `resolveMappedTypeMembers` 的 `stripOptional`（`strict && !isOptional && srcOptional` → `getTypeWithFacts(NEUndefined)`）在 `-?` 时剥 `undefined`，回到 `number`。这是**完成既有 C-C3 DEFER 的 `?`-undefined 处理**（非弱化）。
**无任何测试被弱化/删除**（断言改的都是匹配 Go 实测的更强/更正确行为）。

**gate（实测，均已 RUN）**：`cargo test -p tsgo_checker --lib`（696 绿）+ `--doc`（165 绿）；`cargo clippy -p tsgo_checker --all-targets -- -D warnings` 干净；`cargo fmt -p tsgo_checker -- --check` 干净；`cargo build -p tsgo_compiler` 绿。

**与 Go 的已知偏离（本轮，记录在案）**：
- **union 成员印序**：端口按 TypeId 排序、`undefined` 在前 → `undefined | number`；cmd/tsgo 印 `number | undefined`（既有，非本轮引入）。
- **无 fresh 枚举字面量**：端口枚举成员值侧/联合共用同一 regular 字面量（不建 `getFreshTypeOfLiteralType`）。可观察等价（赋值性/印名一致）；枚举字面量加宽（`let x = E.A` → `E`）DEFER。
- **联合 reduction / alias attribution**：`getUnionTypeEx(_, UnionReductionLiteral, alias)` 的 reduction 与 alias 归属未建；单成员枚举经 nodebuilder `getDeclaredTypeOfSymbol(parent)==t` 规则印 `E`（与 Go 等价可观察）。
- **实体引用解析**：求值器实体回调按名解析枚举**自身**前序成员（非完整 `resolveEntityName`/`resolveAlias`/跨枚举/变量引用）。

**本轮 DEFER（带 blocked-by）**：
- **definite assignment / `2454`**（`let x:number;` use-before-assign）：需 `markNodeAssignments`（`markedAssignmentSymbolLinks`/`getAssignmentTargetKind`/`extendAssignmentPosition`/`getResolvedSymbol`）+ `getControlFlowContainer` + `getFlowTypeOfReferenceEx(initialType)` 种子 + `isMutableLocalVariableDeclaration`（依赖 `isGlobalSourceFile`/模块检测，StubProgram 单文件脚本与 cmd/tsgo 模块检测可能不一致 → 易误报/分歧）。属任务允许的 DEFER（"flow 基建不足"）。blocked-by: `markNodeAssignments` 子系统 + `getControlFlowContainer` + flow `initialType` 种子 + 模块检测/`isGlobalSourceFile`。
- **`exactOptionalPropertyTypes`**：`getOptionalType` 的 `undefinedOrMissingType`（property 用 `missingType`）+ `removeMissingType`：本轮 base 行为用 `undefinedType`。blocked-by: `missingType` 内建 + `exactOptionalPropertyTypes` 选项消费端。
- **类型位枚举成员引用 `E.A`**（`let x: E.A`）：qualified-name 类型引用解析到枚举成员声明类型。blocked-by: `get_type_from_type_reference` 的 qualified-name 臂 + namespace/enum 成员类型解析。
- **TypeFacts typeof 位（`1<<0..1<<15`）+ object empty/function 精化 + intersection facts + bigint `0n` 字面量 falsiness**：typeof narrowing 仍走关系引擎；bigint 字面量值未建模（`LiteralValue` 无 `BigInt` 变体）。blocked-by: typeof 位建模 + lib `Function` 全局（P6）+ `LiteralValue::BigInt`/`get_bigint_literal_type`。
- **枚举：fresh 字面量 + 联合 reduction/alias attribution + computed（非常量）成员 + 枚举与 namespace 合并 + `isEnumTypeRelatedTo`（跨同名枚举）+ 枚举专属诊断（`2553`/`2567`/computed-name/numeric-name/"member must have initializer"/const-enum NaN/isolatedModules）+ emit 内联（transformer/P5）**：blocked-by 各自基建（fresh 类型 / alias 归属 / 完整 `evaluate`+`resolveEntityName` / 合并 / 跨枚举关系 / grammar 诊断 / P5）。

**conformance 切片（登记，端到端对拍仍在 P10）**：`tests/cases/conformance/types/strictNullChecks*`（可选属性/参数 `| undefined`）、`compiler/enum*`、`conformance/enums/*`（数值/字符串/auto-increment/const enum/常量表达式成员、成员引用赋值性、字面量→枚举位标志）最小子集——无 fresh/alias 归属、无 computed 成员、无枚举合并、无 emit 内联。
...
## C-E 落地记录（worklog 摘要）—— EmitResolver 剩余方法（`GetConstantValue`/`GetEnumMemberValue` + `IsDeclarationVisible` 扩展）+ lib.d.ts globals 双侧（`getGlobalPromiseType`/`getGlobalPromiseConstructorSymbol`）（**P4 checker 最后一轮**）

> 在 4k/4an–4aw（EmitResolver 引用/别名/导出容器/类型序列化查询）与 4z/P6（checker 侧 `get_global_symbol`/`get_global_type` + 编译器侧 lib 加载）之上，落地任务点名的 C-E 两块：**EmitResolver 剩余可达方法** + **lib.d.ts globals 双侧（checker 注入）**。逐行为 `/tdd` 红→绿垂直切片，一次一个。const-enum 真值经 `go build -o /tmp/tsgo ./cmd/tsgo` 实跑 `--target esnext --module preserve` 捕获后才断言。单 lane；cargo 仅 `-p tsgo_checker`（+ `cargo build -p tsgo_compiler`/`-p tsgo_transformers` 验 additive）；未 `git commit`。

**Go 真值（ground truth，cmd/tsgo 实跑捕获）**：
- const enum 内联：`const enum E { A = 10, B = A + 5 }\nvar x = E.A;\nvar y = E.B;` → `var x = 10 /* E.A */;` + `var y = 15 /* E.B */;`（`E.A`=10、`E.B`=A+5=15）。
- 非-const enum 不内联：`enum E { A = 10 }\nvar x = E.A;` → 运行时对象 `var E; (function(E){E[E["A"]=10]="A";})(E||(E={}));\nvar x = E.A;`（`E.A` 保留 ⇒ `GetConstantValue` 返 nil）。

**Go 函数镜像（`// Go:` 锚）**：`services.go:Checker.GetConstantValue(819)`、`checker.go:Checker.getEnumMemberValue(23773)`/`computeEnumMemberValues(23799)`、`emitresolver.go:EmitResolver.GetEnumMemberValue(89)`/`IsDeclarationVisible(104)`/`determineIfDeclarationIsVisible(131)`、`utilities.go:IsEnumConst(1834)`/`GetCombinedModifierFlags(1162)`/`getCombinedFlags(1146)`/`GetDeclarationContainer(2556)`/`GetRootDeclaration(1139)`、`checker.go:getGlobalPromiseType(getGlobalTypeResolver "Promise" arity1)`/`getGlobalPromiseConstructorSymbol(getGlobalValueSymbolResolver "Promise")`。

**严格 TDD（逐切片 red→green；每片先实测红的对的原因，再最小绿）**：

| # | 切片 | 最小 input → observable（实测红）| 实现触点 |
|---|---|---|---|
| S1 tracer | `get_constant_value_of_enum_member_node` | `const enum E { A = 10 }` 的 `EnumMember` `A` → `EvalValue::Num(10)`（红：`get_constant_value` 方法不存在，编译错误）| `EmitResolver::get_constant_value`（`EnumMember` 臂）+ `EmitResolver::get_enum_member_value` + `declared_types::get_enum_member_value`（复用既有 `compute_enum_member_values`）|
| S2 | `get_constant_value_of_const_enum_property_access` / `..._non_const_..._is_none` | `const enum E{A=10,B=A+5}` 的 `E.A`→10、`E.B`→15；非-const `enum E{A=10}` 的 `E.A`→`None`（红：`E.A` 返 `None` 应 `Num(10)`——先把 `get_constant_value` 缩成仅 `EnumMember` 臂看红，再补属性访问解析）| `EmitResolver::resolve_enum_member_reference`（`E.A` 解析 `E`→enum 符号→exports 查成员；bare ident 臂）+ `is_enum_const`（`IsEnumConst` = 父 enum decl 带 `const` 修饰）|
| S3 | `is_declaration_visible_for_exported_variable_declaration` | `export const x = 1;` 的 `VariableDeclaration` → visible=true；`const y = 2;` → false（红：旧 `is_declaration_visible` 仅查 node 自身 `export` 修饰，`VariableDeclaration` 无修饰 ⇒ false，应 true）| `determine_if_declaration_is_visible`（忠实 Go switch 可达子集）+ `combined_modifier_flags`（折叠 VariableStatement 的 `export`）+ `get_declaration_container`/`get_root_declaration` |
| S4 | `get_global_promise_type_resolves_and_degrades` | 合成 `interface Promise<T>{}` → 解析到 object 类型；无 Promise/无 program → `None`（红：`get_global_promise_type` 方法不存在）| `Checker::get_global_promise_type`（委托 `get_global_type("Promise")`，优雅降级 `None`）|
| S5 | `get_global_promise_constructor_symbol_resolves_and_degrades` | 合成 `declare var Promise: ...;`（值）→ 解析到 VALUE 符号；无 → `None`（红：方法不存在）| `Checker::get_global_promise_constructor_symbol`（委托 `get_global_symbol("Promise", VALUE)`，优雅降级 `None`）|

> 红→绿证据：S1/S4/S5 **genuine RED（编译错误：方法不存在）**；S2 **genuine RED**——先把 `get_constant_value` 缩成仅 `EnumMember` 臂（属性访问返 `None`）跑红（`E.A` 返 `None` ≠ `Num(10)`），再补 `resolve_enum_member_reference` + `is_enum_const` 转绿；S3 **genuine RED**（旧 `is_declaration_visible` 对 `VariableDeclaration` 返 false，应 true）。S2 的非-const guard + S4/S5 的"无 program → None"分支为同切片内的双侧断言（present↔absent），如实记录。

**可达裁剪与受限处（faithful-but-reachable）**：
- **`GetConstantValue` 可达子集**：`EnumMember` 节点直接返值（Go 首臂，与 const 无关）；引用臂仅内联 **const** enum 成员（Go `ast.IsEnumConst(member.Parent)`），覆盖 `E.A`（PropertyAccess）+ bare identifier 两形态。Go 经 `checkExpressionCached`+`symbolNodeLinks.resolvedSymbol`（回退 `resolveEntityName`）解析；可达 port 用单文件 `resolve_name` 直解实体名。element-access（`E["A"]`）/ qualified / 跨模块引用 DEFER。
- **`GetEnumMemberValue` 返值而非 `evaluator.Result`**：复用既有 `compute_enum_member_values`（auto-increment + 常量折叠 + enum 内实体引用），返 `EvalValue`（Go `Result.Value` 的可达 `.value`）；`isSyntacticallyString`/`resolvedOtherFiles` bookkeeping + 逐成员诊断 + 跨文件 DEFER（同 `EvalValue` 生产者）。
- **`IsDeclarationVisible` 忠实 switch 可达子集**：var/module/class/interface/typealias/function/enum/`import=` 组 → 导出（按 **combined** 修饰，折叠 VariableStatement `export`）则递归容器；`SourceFile`/`NamespaceExportDeclaration`/`TypeParameter` 恒可见；import clause/specifier/namespace import/`export=` 不可见。`combined_modifier_flags`/`get_declaration_container`/`get_root_declaration` 1:1 端口 Go（放 checker crate，未改 `internal/ast`）。**非导出 → `IsGlobalSourceFile(parent)` 近似为 `false`**（保守视作 module，全局脚本文件案 DEFER，与既有 DEFER 一致 ⇒ 既有 `function g` 不可见断言不回归）。empty-binding-pattern carve-out / ambient module / JS typedef / property·method·accessor 臂（需 `GetEffectiveDeclarationFlags`）DEFER。
- **lib globals 双侧**：checker 侧 `get_global_promise_type`/`get_global_promise_constructor_symbol` 命名便捷物——**有**（合成 global 或 P6 真 lib）则解析、**无**（StubProgram 无 lib / `Checker::new()` 无 program）则**优雅降级 `None` 不 panic**；这正是 metadata 序列化（async `design:returntype` 认 `Promise<T>`、`design:type` Promise 臂比对值符号）的地基。Go `getGlobalPromiseType` 用 arity 1 且缺时回退 `emptyGenericType`；可达 port 返 `None`（DEFER empty-generic-type + arity 检查 + 诊断 wiring）。真 lib 解析端到端经 P6 编译器测试兜底。

**本轮交付（全部 `internal/checker/**` + 一处 `lib.rs` re-export）**：
- `core/emit_resolver.rs`：新增 pub 方法 `EmitResolver::get_enum_member_value`/`get_constant_value`；私有 helper `resolve_enum_member_reference`/`is_enum_const`；**重写** `is_declaration_visible`（委托新私有 `determine_if_declaration_is_visible` + `combined_modifier_flags`/`get_declaration_container`/`get_root_declaration`）。
- `core/declared_types.rs`：新增 `pub(crate) fn get_enum_member_value`（复用既有 `compute_enum_member_values`）。
- `core/mod.rs`：新增 pub 方法 `Checker::get_global_promise_type`（委托 `get_global_type`）/`get_global_promise_constructor_symbol`（委托 `get_global_symbol`）。
- `lib.rs`：`pub use tsgo_evaluator::EvalValue;`（加法式 re-export，使 `get_constant_value`/`get_enum_member_value` 返回类型可被消费者命名）。
- 测试：`emit_resolver_test.rs`（+4：S1、S2×2、S3）、`mod_test.rs`（+2：S4、S5）。

**新公开 API 形状（additive-only 确认）**：无既有 `pub fn` 签名变更、无 `lib.rs` 既有项移除、无依赖/`.go`/`internal/ast`/`internal/binder` 改动。新增物全为加法：`EmitResolver` 上 pub 方法 `get_enum_member_value`/`get_constant_value`（`EmitResolver` 已 re-export）、`Checker` 上 pub 方法 `get_global_promise_type`/`get_global_promise_constructor_symbol`、`lib.rs` 新 re-export `EvalValue`、`declared_types::get_enum_member_value`（`pub(crate)`）；其余 helper 私有 `fn`。`is_declaration_visible` 签名不变（行为增量：从仅 `export`-修饰扩到忠实 Go switch 可达子集）。`cargo build -p tsgo_compiler` + `cargo build -p tsgo_transformers` 绿。

**测试增量**：**720 单测**（+6，相对 C-D2 基线 714：`emit_resolver_test`+4、`mod_test`+2）+ **169 doctest**（+4：`get_enum_member_value`/`get_constant_value`/`get_global_promise_type`/`get_global_promise_constructor_symbol` 各一 runnable doctest）。**无既有测试弱化/删除**（C-D2 714 全绿不变；`is_declaration_visible` 重写后既有 `exported_declaration_is_visible` 仍绿）。

**既有测试更新（Go-verified justification）**：**无**。`is_declaration_visible` 重写为加法式行为扩展——旧实现对 `export function f`/`function g` 返 true/false，新实现经 combined-flags + 容器递归仍返 true/false（`export function f` combined `export` → 容器 SourceFile → true；`function g` 非导出 → `IsGlobalSourceFile` 近似 false）；既有断言不变。

**gate（实测，均已 RUN）**：`cargo test -p tsgo_checker --lib`（720 绿）+ `--doc`（169 绿）；`cargo clippy -p tsgo_checker --all-targets -- -D warnings` 干净；`cargo fmt -p tsgo_checker -- --check` 干净；`cargo build -p tsgo_compiler` 绿；`cargo build -p tsgo_transformers` 绿。

**与 Go 的已知偏离（本轮，记录在案）**：
- **`GetConstantValue`/`GetEnumMemberValue` 返 `EvalValue` 而非 `any`/`evaluator.Result`**：`EvalValue::None` = Go nil（不可折叠）；bookkeeping flags DEFER（见上）。
- **`get_global_promise_type` 缺时返 `None`（非 `emptyGenericType`）**：DEFER empty-generic-type/arity/诊断（与既有 `get_global_type` DEFER 一致）。
- **`IsDeclarationVisible` 非导出 → `IsGlobalSourceFile(parent)`**（C-E 保守 `false`；**D-F3 de-defer**：`is_global_source_file` 查 `external_module_indicator`，脚本可见 / module 不可见。CommonJS indicator 仍 DEFER，blocked-by P6 JS 文件支持）。

**本轮 DEFER（带 blocked-by）**：
- **EmitResolver 剩余方法（D-F/P6 拥有更合适者）**：`CreateTypeOfDeclaration`/`CreateReturnTypeOfSignatureDeclaration`/`CreateTypeOfExpression`/`CreateTypeParametersOfSignatureDeclaration`/`CreateLiteralConstValue`/`CreateLateBoundIndexSignatures`（**声明 type-node 合成**，输出类型*节点*而非字符串，需完整 node builder + `EmitContext` + `SymbolTracker`）→ **交接 D-F（declarations transformer）**：D-F 落地声明 emit 时由其驱动这套 node-builder-backed 合成（本轮 `serialize_type_of_declaration` 返字符串的桩仍在，D-F 升级为节点）。blocked-by: 完整 node builder + `SymbolTracker` + `printer.EmitContext`（P5/P6）。
- **`IsEntityNameVisible`/`hasVisibleDeclarations`/`PrecalculateDeclarationEmitVisibility`/`markLinkedAliases`**（声明 emit 别名可见性标记 + `SymbolAccessibilityResult`）：blocked-by `printer.SymbolAccessibilityResult` + alias 可见性 upfront walk（P5 declarations）。
- **`GetReferencedValueDeclaration(s)`/`GetReferencedImportDeclaration`/`SetReferencedImportDeclaration`/`GetReferencedMemberValueDeclaration`/`GetElementAccessExpressionName`**（`binder.ReferenceResolver` 委托面）：blocked-by 完整 `ReferenceResolver` hooks（4as 已落 `get_referenced_export_container` 单点）+ jsx importRef link。
- **`IsLateBound`/`RequiresAddingImplicitUndefined(Unsafe)`/`IsLiteralConstDeclaration`/`IsExpandoFunctionDeclaration`/`IsImportRequiredByAugmentation`/`IsDefinitelyReferenceToGlobalSymbolObject`/`IsTopLevelValueImportEqualsWithEntityName`/`MarkLinkedReferencesRecursively`/`GetTypeReferenceDirectivesFor*`**：blocked-by 各自基建（CheckFlagsLate / reverse-mapped + 可选参数 undefined / fresh-literal / expando 属性 / `getExportsOfModule` 跨模块 / globalThis 符号 / `markLinkedReferences` / type-reference directives）。
- **`get_type_reference_serialization_kind` 的 Promise 臂 wiring**：本轮已备 `get_global_promise_constructor_symbol`，但接入序列化（需 `resolveEntityName` 的 value/type 符号一致性 + lib 真 Promise）仍 DEFER（P6）。

**C-E 状态**：**可达子集完成**。EmitResolver const-value（`GetConstantValue`/`GetEnumMemberValue`：EnumMember + const-enum `E.A` 内联）+ `IsDeclarationVisible`（忠实 Go switch 可达子集）+ lib globals 双侧（`getGlobalPromiseType`/`getGlobalPromiseConstructorSymbol`，present↔absent 优雅降级）落地；声明 type-node 合成交接 D-F、其余 EmitResolver 面按各 DEFER。
