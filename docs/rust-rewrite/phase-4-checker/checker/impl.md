...
## checker 移植状态（4a–4k 收口 · 给下一 phase 看的"接缝"）

> **状态：checker 移植子阶段（4a–4k）已收口、gate 全绿（127 单测 + 109 doctest，C1–C8 + 整 workspace 绿）。** 这是一个**可达核心**的纵向切片，不是全量 checker；剩余深化 + 端到端正确性由 **P10 conformance** 兜底，跨 phase 依赖标注于下。
...
- [x] `Checker::new_checker(program)` 前向兼容入口（4k 落地，`core/mod.rs`）　`// Go: checker.go:NewChecker`　blocked-by: 全局/lib 绑定 + 保留 program（P6）
