...
- [ ] 推断深化（C-C）：full contravariant 候选格（`contraCandidates`/`getContravariantInference`）+ 完整 `InferencePriority` lattice（mapped/keyof/literal/substitution 位）、mapped/conditional/template/keyof 推断、reverse-mapped、`outerMapper`/`returnMapper` 嵌套泛型上下文、intra-expression inference sites、JSX 推断　`// Go: inference.go`（C-C/later）
