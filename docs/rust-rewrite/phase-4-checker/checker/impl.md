...
- [x] `is_declaration_visible`（4k：顶层声明带 `export` 即可见 → **C-E 扩为忠实 Go `determineIfDeclarationIsVisible` switch 可达子集**：var/module/class/interface/typealias/function/enum/`import=` 组按 **combined** 修饰（折叠 VariableStatement `export`）+ 容器递归；`SourceFile`/`NamespaceExportDeclaration`/`TypeParameter` 恒可见；import clause/specifier/namespace import/`export=` false。非导出 → `IsGlobalSourceFile(parent)` 保守 false（全局脚本案 DEFER）。新私有 helper `determine_if_declaration_is_visible`/`combined_modifier_flags`/`get_declaration_container`/`get_root_declaration`）（4k 落地 + C-E 扩，`core/emit_resolver.rs`）　`// Go: emitresolver.go:IsDeclarationVisible(104)/determineIfDeclarationIsVisible(131) + utilities.go:GetCombinedModifierFlags/GetDeclarationContainer/GetRootDeclaration`
...
- **`IsDeclarationVisible` 非导出 → 保守 `false`**（全局脚本文件案 blocked-by external-module detection，P6）。
