...
| `internal/checker/core/contextual.rs`（4bj 新） | `ContextFlags`(pub(crate)) + `get_contextual_type`(var-decl 初始化器臂 + binary `=` RHS 臂；余 DEFER)/`get_contextual_type_for_initializer_expression`/`get_contextual_type_for_variable_like_declaration`/`get_contextual_type_for_binary_operand`/`get_contextual_type_for_assignment_expression`(Identifier 目标)/`get_apparent_type_of_contextual_type`(薄包装)/`get_contextual_signature`/`get_contextual_call_signature`/`is_arity_smaller`/`get_contextually_typed_parameter_type`/`assign_contextual_parameter_types`/`try_get_type_at_position`/`contextually_check_function_expression` + 自由 fn `declaration_initializer`/`declaration_type_node`/`is_function_expression_or_arrow`/`function_like_parameters`/`parameter_index`/`parameter_is_optional_for_arity` | `checker.go:` |
