...
| Rust 测试 | 验证内容 | input → expected | 完成 |
|---|---|---|---|
| `inference_test.rs::inference_context_and_info_construction` | 上下文/槽/优先级构造 | `InferenceContext::new([T1,T2])`→2 槽；`InferenceInfo::new`空候选；`priority==MAX_VALUE`（C-B3：Go `newInferenceInfo` 种子，旧 `NONE` 已按 Go 修正） | ✓ |
...
| `object_literal_property_mismatch_reports_2322`（`check_test.rs`，slice 2，slice 1 前 genuine red；**4bo 改 element-wise**）| 成员 widen 类型不关联注解属性类型报 2322（红：slice 1 前对象=`error` 可赋任意 0 诊断）；4bo 后字面量源 element-wise 阐释→叶消息（旧整对象链是 4bn 记录的偏离，非弱化）| `const o: { a: number } = { a: "x" };` → 1×2322「Type 'string' is not assignable to type 'number'.」（指字面量 `a`）| ✓ |
...
| `object_literal_property_mismatched_literal_kind_still_reports_2322`（`check_test.rs`，slice 1 守卫，green-on-arrival；**4bo 改 element-wise**）| kind 不匹配仍 widen + 报 2322；4bo 后 element-wise + 6500 related-info | `const o: { a: "x" } = { a: 1 };` → 1×2322「Type 'number' is not assignable to type '"x"'.」（指字面量 `a`）+ 6500 "...on type '{ a: "x"; }'" | ✓ |
...
## 4bo 对象/数组**字面量** RHS 的 element-wise 阐释（`elaborateError`/`elaborateObjectLiteral`/`elaborateArrayLiteral`/`elaborateElement`）行为单测（§8.6）

> 全部 `check_test.rs`。Go 真值用 `go build ./cmd/tsgo` 实跑逐一核对（码 / 节点 span / 6500 related-info）。

| Rust 测试 | 验证行为 | 输入 → 期望（Go 实测）| 完成 |
|---|---|---|---|
| `elaborate_object_literal_wrong_property_type_points_at_property_name`（slice 1 tracer，genuine red→green）| 对象字面量错类型 element-wise，报在**属性名节点** + 6500（红：实测整对象链消息 `Type '{ a: string; }'...`）| `const o:{a:number}={a:"x"}` → `2322`「Type 'string' is not assignable to type 'number'.」指字面量 `a`，+ 6500「The expected type comes from property 'a' which is declared here on type '{ a: number; }'」指注解 `a` | ✓ |
| `elaborate_nested_object_literal_points_at_innermost_property`（slice 2，recursion，green-on-arrival）| 嵌套字面量递归到**最内层**成员（前/后：4bn 走 dotted 2200 "a.b"）| `const o:{a:{b:number}}={a:{b:"x"}}` → `2322` 指内层 `b` + 6500 'b' "...on type '{ b: number; }'" | ✓ |
| `elaborate_array_literal_wrong_element_points_at_element`（slice 3，genuine red→green，direct-driver）| 数组字面量 force-tuple + element-wise，报在 `"x"` 元素、**无** related（红：禁 `elaborate_array_literal` → 0 诊断，bool 路径误判数组可赋值）| `const xs:number[]=[1,"x"]` → `2322`「Type 'string' is not assignable to type 'number'.」指 `"x"`（`1` 静默），无 related | ✓ |
| `elaborate_object_literal_wrong_type_suppresses_excess_property`（slice 4 守卫）| 错类型 + excess → **仅** element-wise `2322`、无 `2353`（elaborate 先行短路 excess）| `const o:{a:number}={a:"x",b:1}` → 仅 1×`2322` 指 `a` | ✓ |
| `object_literal_missing_property_falls_back_to_2741_chain`（slice 4 守卫）| 缺失必需属性 → elaborate 不报 → 4bn 通用链 `2741`（兜底未变）| `const o:{a:number;b:number}={a:1}` → `2741`「Property 'b' is missing in type '{ a: number; }' but required in type '{ a: number; b: number; }'.」无 related | ✓ |
| `non_literal_rhs_keeps_4bn_generic_chain`（slice 4 守卫）| 非字面量 RHS（identifier）→ elaborate 默认臂 false → 4bn 链保留 | `interface A{a:number} interface B{a:string}; declare const b:B; const o:A=b;` → `2322` 头 + `message_chain`[2326]，无 related | ✓ |
| `elaborate_error_unwraps_parenthesized_object_literal`（附，direct-driver）| `elaborateError` 解包 `KindParenthesizedExpression` → 内层字面量阐释 | `const o:{a:number}=({a:"x"})` → `2322` 指内层 `a` + 6500（direct-driver：`check_expression` 未给括号定型，var-decl 不可达）| ✓ |
| `elaborate_simple_assignment_object_literal_rhs`（附）| 简单赋值 RHS 字面量在赋值站也 element-wise（`checkAssignmentOperator` 接线）| `let x:{a:number}={a:1}; x={a:"y"}` → `2322` 指 RHS `a` + 6500 指 `x` 声明 | ✓ |
| `elaborate_error_returns_false_for_non_literal_expression`（单测）| dispatch 默认臂：identifier → false、无诊断 | `x`（identifier 节点）+ string→number → `elaborate_error`=false | ✓ |
| `elaborate_object_literal_primitive_target_returns_false`（单测）| primitive/never target 守卫 → false | 对象字面量 vs `number` target → `elaborate_error`=false、无诊断 | ✓ |
| `is_tuple_like_type_recognizes_tuples_and_zero_indexed_objects`（单测）| `isTupleLikeType` 两臂：TUPLE flag + `"0"` 属性；空对象 false | `create_tuple_type_ex`→true / 空对象→false / `{0:1}`→true | ✓ |

> 红→绿证据：slice 1 **genuine RED**（实测整对象链消息 ≠ element-wise 叶）；slice 3 **genuine RED**（临时 `if true {return false}` 禁 `elaborate_array_literal` → 实测 **0** 诊断，因 bool 路径误判 `(number|string)[]` 可赋值给 `number[]`）。两处 RED 实测后已恢复复绿。slice 2 是 slice 1 递归臂、附加/单测为 green-on-arrival——**如实记录**（同 4bc–4bn 口径）。改 2 既有测试（`object_literal_property_mismatch_reports_2322`/`..._mismatched_literal_kind_still_reports_2322`）断言为 element-wise——对齐 Go 真值（旧整对象链是 4bn 记录的偏离，**非弱化**）。**测试增量**：**537 单测**（+11，相对 4bn 基线 526，全在 check_test）+ **135 doctest**（+0）。gate：`cargo test -p tsgo_checker`（537 lib + 135 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿（**未触 `internal/ast`，故未跑 `--workspace`**）。**与 Go 实测一致**：错类型 element-wise 报**属性名/元素节点**（NOT 整赋值）+ 6500 related-info（指目标属性声明的**名字**节点）；数组无 related（lib index 签名被 `IsSourceFileDefaultLibrary` 抑制）；错类型 + excess 仅报 element-wise（elaborate 短路 excess）——均按 `cmd/tsgo` 实测匹配。**DEFER（带 blocked-by）**：数组/括号 end-to-end 不可达（关系引擎 array/reference 类型实参变型；`ParenthesizedExpression` 定型）/ `elaborateError` 其余臂（条件类型早退·call·construct 建议·binary·as·JSX·arrow）/ `elaborateObjectLiteral` spread·shorthand·method·accessor·computed 名 / `elaborateArrayLiteral` spread·omitted·contextual·optional·rest / `elaborateElement` union best-match·exactOptional·JSX 工厂·`removeMissingType`·6501·`target.symbol` 兜底（需默认库文件判定）/ `isTupleLikeType` array-like+length 臂。详见 impl.md「4bo 落地记录」。

## 4bp type-reference **类型实参变型**（`typeArgumentsRelatedTo`）+ 括号定型 + 引用成员实例化 行为单测（§8.6）

> Go 真值用 `go build ./cmd/tsgo` 实跑逐一核对（码 / 节点 span / message / 嵌套链）。`check_test.rs` 走 `get_diagnostics` end-to-end；`relations_test.rs` 直接驱动私有/公开关系函数。

| Rust 测试 | 验证行为 | 输入 → 期望（Go 实测）| 完成 |
|---|---|---|---|
| `array_literal_wrong_element_reports_2322_end_to_end`（`check_test.rs`，slice 1 tracer，genuine red→green）| 数组字面量错元素 end-to-end（4bo elaborate 现可达）；红：实测 **0** 诊断（bool 误判 `Array<number\|string>`↔`Array<number>`）| `const xs:number[]=[1,"x"]` → `2322`「Type 'string' is not assignable to type 'number'.」指 `"x"`，无 related | ✓ |
| `array_literal_matching_elements_reports_nothing`（`check_test.rs`，slice 1 正控）| 全可赋值元素 → 0 诊断 | `const xs:number[]=[1,2]` → 0 | ✓ |
| `array_reference_covariance_is_assignable`（`check_test.rs`，slice 2）| `number[]` IS 可赋值给 `(number\|string)[]`（covariant）| `const a:number[]=[1]; const b:(number\|string)[]=a` → 0 | ✓ |
| `array_reference_reverse_reports_2322`（`check_test.rs`，slice 2，genuine red→green）| `(number\|string)[]` 不可赋值给 `number[]`（identifier RHS 走关系链）；红：实测 **0** 诊断 | `const c:(number\|string)[]=["x"]; const d:number[]=c` → `2322` 指 `d` 声明 + 嵌套链「Type 'string \| number' is not assignable to type 'number'.」（头打印 `Array<...>` vs Go `(...)[]`、链深 2 vs 3 = 记录偏离；CODE/span/union 序对齐 Go）| ✓ |
| `generic_reference_target_wrong_property_reports_2322`（`check_test.rs`，slice 3）| `Box<number>` 错属性（成员实例化为 number）→ 4bo element-wise 报在 `v` + 6500 | `interface Box<T>{v:T} const x:Box<number>={v:"s"}` → `2322`「Type 'string' is not assignable to type 'number'.」指 `v` + 6500「...property 'v'...on type 'Box<number>'」| ✓ |
| `generic_reference_target_matching_property_reports_nothing`（`check_test.rs`，slice 3，genuine red→green）| `Box<number>` 对属性 → 0；红：实测 **2741**「Property 'T' is missing」（类型参数泄漏 + 未实例化比对）| `const y:Box<number>={v:1}` → 0 | ✓ |
| `parenthesized_expression_takes_inner_type`（`check_test.rs`，slice 4，genuine red→green）| `(expr)` 定型为内层；红：实测 `(1)` 返 error type → 0 诊断 | `const n:number=(1)` → 0；`const s:string=(1)` → `2322`「Type 'number' is not assignable to type 'string'.」| ✓ |
| `type_arguments_related_to_relates_covariantly`（`relations_test.rs`，单测）| covariant 逐对：`source[i]` relates `target[i]` | `[number]`→`[number\|string]` true；反向 false；`[number]`→`[number]` true；`[]`→`[]` true | ✓ |
| `type_arguments_related_to_identity_length_guard`（`relations_test.rs`，单测）| identity 不等长 → false；等长等型 → true | `[n,n]`≢`[n]` false；`[n]`≡`[n]` true | ✓ |
| `reference_type_arguments_related_to_requires_same_target`（`relations_test.rs`，单测）| 仅同 target 引用触发（同→Some / 异 target→None / 非引用→None）| `Box<number>`/`Box<number\|string>`→Some(true)/反向 Some(false)；`Box`/`Bag`→None；`Plain`→None | ✓ |
| `array_references_relate_by_element_covariance`（`relations_test.rs`，单测，公开 API）| `Array<T>` 同 target 经元素 covariant | `Array<number>`→`Array<number\|string>` true；反向 false；同→自 true | ✓ |
| `reference_member_types_are_instantiated_for_relation`（`relations_test.rs`，单测）| 引用成员经 type-arg mapper 实例化 + 类型参数非属性 | `{v:number}`→`Box<number>` true；`{v:string}`→`Box<number>` false | ✓ |

> 红→绿证据：slice 1 **genuine RED**（实测 0 诊断，bool 误判 `(number\|string)[]`↔`number[]`）；slice 2 反向 **genuine RED**（实测 0 诊断）；slice 3 对属性 **genuine RED**（实测 `2741`「Property 'T' is missing」——类型参数泄漏进接口属性表 + `v` 比对未实例化 `T`）；slice 4 **genuine RED**（`(1)` 返 error type ⇒ 0 诊断）。四处均用临时 probe 实测红后产最终测试复绿。单测为 green-on-arrival（关系机器一体）——**如实记录非伪造红**（同 4bc–4bo 口径）。**仅更新 2 处 4bo 直驱测试的过时注释**（`elaborate_array_literal_wrong_element_points_at_element`/`elaborate_error_unwraps_parenthesized_object_literal` 现 end-to-end 可达）——测试体/断言未改、**无弱化**。**测试增量**：**549 单测**（+12，相对 4bo 基线 537：relations_test +5 / check_test +7）+ **135 doctest**（+0）。gate：`cargo test -p tsgo_checker`（549 lib + 135 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿（**未触 `internal/ast`，故未跑 `--workspace`**）。**与 Go 实测一致**：同 target 引用 covariant 类型实参关系修 `(number\|string)[]`↔`number[]` 误判、数组字面量 element-wise 现 end-to-end、`Box<number>` 成员实例化为 `number`、`(expr)`/`(T)` 定型——均按 `cmd/tsgo` 实测匹配。**已知打印偏离（非关系 bug）**：`Array<T>` vs `T[]` shorthand（`typeToString` array-shorthand 臂未 port）、cov_bad 链深 2 vs 3（最内层 union 阐释 4bo DEFER）。**DEFER（带 blocked-by）**：完整 `getVariances`（contravariant/invariant/bivariant/independent + variance 注解 + marker-probe + 结构兜底）/ signature·index-signature 关系 / tuple·alias 变型 / intersection-of-references / union 实参 per-constituent 叶 / 数组 `[]` shorthand 打印 / 递归深度·`maybeKeys` 限制。详见 impl.md「4bp 落地记录」。

## C-A 函数类型 **signature 关系**（参数 contravariant + arity + 返回 covariant/void + 方法 bivariance + construct 签名）+ 函数类型打印 行为单测（§8.6）

> Go 真值用 `go build ./cmd/tsgo` 实跑逐一核对（码 / 节点 span / message / 嵌套链 / 链深）。`check_test.rs` 走 `get_diagnostics` end-to-end；`relations_test.rs` 直接驱动私有/公开关系函数。

| Rust 测试 | 验证行为 | 输入 → 期望（Go 实测）| 完成 |
|---|---|---|---|
| `function_parameters_relate_contravariantly`（`relations_test.rs`，slice 1 tracer，genuine red→green）| 参数 contravariant；红：实测反向 **true**（裸函数类型仅按属性比较误判）| `(x:number\|string)=>void`✓`(x:number)=>void`；反向 ✗ | ✓ |
| `function_parameter_mismatch_reports_2328_chain`（`check_test.rs`，slice 1 端到端）| 参数错链 + arrow 打印；红：头印 `{}`、无链 | `(x:string)=>void` 赋 `(x:number)=>void` → `2322`「Type '(x: string) => void' is not assignable to type '(x: number) => void'.」→ `2328`「Types of parameters 'x' and 'x' are incompatible.」→ `2322`「Type 'number' is not assignable to type 'string'.」指 `b` 声明 | ✓ |
| `function_return_types_relate_covariantly`（`relations_test.rs`，slice 2，genuine red→green）| 返回 covariant + void 特例；红：实测 `s✗n` **true** | `()=>string`✗`()=>number`；`()=>number`✓`()=>void`；同→true | ✓ |
| `function_return_mismatch_reports_collapsed_chain`（`check_test.rs`，slice 2 端到端）| 返回错链 + 2204 marker elision（链深 2）| `()=>string` 赋 `()=>number` → `2322`「Type '() => string' is not assignable to type '() => number'.」→ `2322`「Type 'string' is not assignable to type 'number'.」（无更深 next）| ✓ |
| `function_void_return_accepts_any_source_return`（`check_test.rs`，slice 2 正控）| void target 接受任意源返回 | `()=>number` 赋 `()=>void` → 0 | ✓ |
| `function_arity_tolerance`（`relations_test.rs`，slice 3，genuine red→green）| arity 容忍；红：实测 `many✗few` **true** | few✓many；many✗few | ✓ |
| `function_arity_too_few_target_args_reports_2849`（`check_test.rs`，slice 3 端到端）| arity 错 → 2849（非 elided）| `(x:number,y:number)=>void` 赋 `(x:number)=>void` → `2322` → `2849`「Target signature provides too few arguments. Expected 2 or more, but got 1.」| ✓ |
| `function_fewer_source_params_is_assignable`（`check_test.rs`，slice 3 正控）| 源参数更少 → 可赋值 | `(a:number)=>void` 赋 `(a:number,b:number)=>void` → 0 | ✓ |
| `method_parameters_are_bivariant`（`check_test.rs`，slice 4，genuine red→green）| 方法参数恒 bivariant；红：临时强制 strict → 实测 **1×2322** | `interface A{f(x:number):void} interface B{f(x:number\|string):void}; A 赋 B` → 0 | ✓ |
| `arrow_property_parameters_are_contravariant`（`check_test.rs`，slice 4）| 箭头属性 strict contravariant | 同形 `f:(x:T)=>void` 属性 → `2322`「Type 'A2'...」→ `2326` 'f' → `2322` 函数类型 → `2328` → `2322`「Type 'string \| number' is not assignable to type 'number'.」| ✓ |
| `arrow_property_parameters_are_bivariant_without_strict_function_types`（`check_test.rs`，slice 4）| 关 `strictFunctionTypes` → 箭头属性也 bivariant | `--strict --strictFunctionTypes false`：上例 → 0 | ✓ |
| `construct_signatures_relate_by_return_type`（`relations_test.rs`，slice 5，genuine red→green）| construct 签名按返回 covariant；红：实测 `new()=>Other`↔`new()=>Base` **true**（未 populate）| `new()=>Base`✓自身；`new()=>Other`✗`new()=>Base` | ✓ |
| `construct_signature_matching_return_reports_nothing`（`check_test.rs`，slice 5 正控）| construct 对返回 → 0 | `new()=>Base` 赋 `new()=>Base` → 0 | ✓ |
| `construct_signature_incompatible_return_reports_2322`（`check_test.rs`，slice 5 端到端）| construct 错返回 + 2205 elision | `new()=>Other` 赋 `new()=>Base` → `2322`「Type 'new () => Other' is not assignable to type 'new () => Base'.」→ `2741`「Property 'x' is missing in type 'Other' but required in type 'Base'.」| ✓ |
| `array_of_function_types_relates_by_element_signature`（`check_test.rs`，slice 6）| 变型 + signature 复合（非回归）| `Array<(x:number)=>void>` 赋 `Array<(x:string)=>void>` → `2322` → 元素 `2322` → `2328` → `2322`「Type 'string' is not assignable to type 'number'.」| ✓ |
| `array_of_function_types_contravariant_element_is_assignable`（`check_test.rs`，slice 6 正控）| 元素函数 contravariant 协调 | `Array<(x:number\|string)=>void>` 赋 `Array<(x:number)=>void>` → 0 | ✓ |
| `signatures_of_type_is_kind_aware`（`relations_test.rs`，单测）| kind-aware 取 call/construct | 含 call+construct 对象各取对应；非对象→空 | ✓ |
| `signatures_related_to_matches_some_source_signature`（`relations_test.rs`，单测）| N*M「each target matched by some source」| target `()=>number` 被 `[()=>string,()=>number]` 匹配；`()=>boolean` 无；空 target→true | ✓ |
| `compare_signatures_related_return_covariance_and_void`（`relations_test.rs`，单测）| 返回 covariant + void 短路 | `()=>string`✗`()=>number`；`()=>number`✓`()=>void`；同→true | ✓ |
| `compare_signatures_related_arity_guard`（`relations_test.rs`，单测）| arity 守卫（real `x:number` 填充参数）| more✗fewer；fewer✓more | ✓ |

> 红→绿证据：slice 1/2/3/5 均 **genuine RED**（实测裸函数/构造类型误判可赋值，因无 signature 比较）；slice 4 用临时 `strict_variance = true || (...)` 实测方法测试 **1×2322** 红后恢复复绿；端到端链测试接报告孪生前实测**头印 `{}`、无 signature 链**。单测为 green-on-arrival（关系机器一体）——**如实记录非伪造红**（同 4bc–4bp 口径）。**无既有测试弱化/删除**（无既有断言改动；新增 arrow 打印路径未触动现有 nodebuilder 测试）。**测试增量**：**569 单测**（+20，相对 4bp 基线 549：`relations_test`+8 / `check_test`+12）+ **136 doctest**（+1：`strict_function_types`）。gate：`cargo test -p tsgo_checker`（569 lib + 136 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿（**未触 `internal/ast`，故未跑 `--workspace`**）。**与 Go 实测一致**：参数 contravariant `2328`、返回 covariant + void、arity `2849`、方法 bivariance vs 箭头属性 strict（`strictFunctionTypes` 门控）、construct 签名 `2741` + marker elision、变型+signature 复合——均按 `cmd/tsgo` 实测匹配。**已知偏离（记录）**：m_prop/combo 链最内层少一层 union 阐释（4bo/4bp DEFER）、combo 头 `Array<...>` vs Go `(...)[]`（4bp 打印偏离）。**DEFER（带 blocked-by）**：完整 `getVariances` marker-probe（contravariant/invariant/函数参数位变型，blocked-by 泛型实例化 C-B）/ callback-covariance / `this`·rest·泛型签名 / top-signature / identity·overload·abstract·visibility / interface·type-literal 级签名收集 / `x()`/`x(...)` 返回属性折叠 / type predicate·index-signature 关系。详见 impl.md「C-A 落地记录」。

## C-B1 泛型基石 —— 类型参数 + 约束 + 显式类型实参 + 默认 + arity + 实例化 行为单测（§8.6）

> Go 真值用 `go build ./cmd/tsgo`（`--noEmit --strict --target es2020`）实跑逐一核对（码 / message / 链）。`check_test.rs` 走 `get_diagnostics` end-to-end；`mapper_test.rs`/`declared_types_test.rs` 直接驱动私有/公开机器。

| Rust 测试 | 验证行为 | 输入 → 期望（Go 实测）| 完成 |
|---|---|---|---|
| `generic_function_signature_carries_type_parameters`（`declared_types_test.rs`，slice A，genuine red→green）| 泛型函数签名携带类型参数表 + 与 `x:T` 同 id；红：`type_parameters` 空（0 应 1）| `function id<T>(x:T):T{...}` → sig.type_parameters=[T]、param/return 同 id | ✓ |
| `non_generic_function_signature_has_no_type_parameters`（`declared_types_test.rs`，正控）| 非泛型签名类型参数表空（不回归）| `function f(x:number):number{}` → 空 | ✓ |
| `generic_function_explicit_type_argument_instantiates_return`（`check_test.rs`，slice B1，genuine red→green）| `id<number>(1)` 实例化为 `number`；红：`2345`(arg vs `T`)+`2322`(`unknown`)| `...const r=id<number>(1); const s:string=r;` → `2322`「'number'...'string'.」| ✓ |
| `generic_function_explicit_type_argument_assignable_to_match`（`check_test.rs`，正控）| `r` 实为 `number`（赋 number 通过）| `const n:number=r;` → 0 | ✓ |
| `generic_interface_member_reads_instantiated_type`（`check_test.rs`，slice B2）| `Box<number>` 经 `b.v` 读出 `number`| `...const n:string=b.v;` → `2322`「'number'...'string'.」| ✓ |
| `generic_interface_member_assignable_to_match`（`check_test.rs`，正控）| `b.v` 实为 `number`| `const n:number=b.v;` → 0 | ✓ |
| `explicit_type_argument_violates_constraint_reports_2344`（`check_test.rs`，slice C约束 call，genuine red→green）| call-site 约束违反 2344 + 中止（无 2345）| `function f<T extends number>(x:T){} f<string>("a");` → `2344`「Type 'string' does not satisfy the constraint 'number'.」| ✓ |
| `failing_constraint_suppresses_argument_error`（`check_test.rs`，slice C约束）| 约束失败抑制 2345（Go `checkTypeArguments` `return nil`）| `f<string>(1);` → 仅 `2344` | ✓ |
| `explicit_type_argument_satisfies_constraint_ok`（`check_test.rs`，正控）| 满足约束 → 0 | `f<1>(1);` → 0 | ✓ |
| `type_alias_reference_violates_constraint_reports_2344`（`check_test.rs`，slice C约束 ref，genuine red→green）| 类型引用站约束违反 2344；红：检查相位不访问类型节点 ⇒ 0 | `type G<T extends number>=T; type X=G<string>;` → `2344` | ✓ |
| `type_alias_reference_satisfies_constraint_ok`（`check_test.rs`，正控）| `G<1>` → 0 | `type X=G<1>;` → 0 | ✓ |
| `default_type_argument_applied_to_member`（`check_test.rs`，slice C默认，genuine red→green）| 省略实参用默认；红：`c.v` 为 `T` 非 `number`| `interface C<T=number>{v:T} const c:C={v:1}; const w:string=c.v;` → `2322`「'number'...'string'.」| ✓ |
| `default_type_argument_overridden_by_explicit`（`check_test.rs`，slice C默认）| 显式覆盖默认 | `const c2:C<string>={v:"x"}; const w:number=c2.v;` → `2322`「'string'...'number'.」| ✓ |
| `interface_too_many_type_arguments_reports_2314`（`check_test.rs`，slice C-arity，genuine red→green）| interface 太多 2314（印 `Box<T>`）；红：0 诊断 | `type X=Box<number,string>;` → `2314`「Generic type 'Box<T>' requires 1 type argument(s).」| ✓ |
| `interface_too_few_type_arguments_reports_2314`（`check_test.rs`，slice C-arity）| interface 太少 2314（印 `Pair<A, B>`）| `type X=Pair<number>;` → `2314`「...'Pair<A, B>' requires 2...」| ✓ |
| `type_alias_too_many_type_arguments_reports_2314`（`check_test.rs`，slice C-arity）| alias 印裸名 `G`| `type G<T>=T; type X=G<number,string>;` → `2314`「...'G' requires 1...」| ✓ |
| `interface_type_arguments_out_of_range_reports_2707`（`check_test.rs`，slice C-arity）| 默认致区间 → 2707| `interface C<T=number,U=string>{...} type X=C<number,string,boolean>;` → `2707`「...'C<T, U>' requires between 0 and 2...」| ✓ |
| `generic_call_too_many_type_arguments_reports_2558`（`check_test.rs`，slice C-arity）| call 太多类型实参 → 2558| `const r=id<number,string>(1);` → `2558`「Expected 1 type arguments, but got 2.」| ✓ |
| `required_type_parameter_after_optional_reports_2706`（`check_test.rs`，slice C-arity grammar）| 必选跟默认后 → 2706| `interface C<T=number,U>{...}` → `2706`「Required type parameters may not follow optional type parameters.」| ✓ |
| `generic_call_with_nested_reference_type_argument`（`check_test.rs`，slice 组合）| `id<Box<number>>` 嵌套引用代入 → `r.v` `number`| `...const bad:string=r.v;` → `2322`「'number'...'string'.」| ✓ |
| `generic_function_returning_reference_instantiates_argument`（`check_test.rs`，slice 组合）| `wrap<T>:Box<T>` 返回引用实例化 → `r.v` `number`| `...const bad:string=r.v;` → `2322` | ✓ |
| `instantiate_type_maps_intersection_members`（`mapper_test.rs`，单测）| intersection 成员逐个实例化 | `{tp & A}` with `tp→string` → `string & A` | ✓ |
| `instantiate_type_intersection_without_type_vars_is_identity`（`mapper_test.rs`，单测）| 无类型变量 intersection 恒等 | `{A & B}` → 不变 | ✓ |
| `mapper_combinators_compose`（`mapper_test.rs`，单测）| `unary`/`merge`/`combine`(None/Some 复合)/`append_mapping` | 见测试断言 | ✓ |
| `instantiate_signature_composes_mappers_and_keeps_parameters`（`mapper_test.rs`，单测）| 链式实例化复合 mapper + 参数保基符号 | `a→b` 再 `b→number` ⇒ `a→number`、返回 `number` | ✓ |
| `constraint_of_type_parameter_resolves_extends_type`/`..._is_none`（`declared_types_test.rs`，单测）| `getConstraintOfTypeParameter`（有/无）| `T extends number`→number；`T`→None | ✓ |
| `default_of_type_parameter_resolves_default_type`（`declared_types_test.rs`，单测）| `getDefaultFromTypeParameter`（有/无）| `T=string`→string；`U`→None | ✓ |
| `min_type_argument_count_excludes_trailing_defaults`（`declared_types_test.rs`，单测）| `getMinTypeArgumentCount`| `A<T,U=string>`→1 | ✓ |
| `fill_missing_type_arguments_uses_defaults_then_unknown`（`declared_types_test.rs`，单测）| `fillMissingTypeArguments`（默认→unknown）| `A<T,U=string>` fill `[number]`→`[number,string]`；`B<X,Y>`→`[number,unknown]` | ✓ |
| `generic_type_alias_records_type_parameters`（`declared_types_test.rs`，单测）| 泛型 alias 记录类型参数 + 约束 | `type G<T extends number>=T` → tp 数 1、约束 number | ✓ |

> 红→绿证据：slice A/B1/C约束(call+ref)/C-arity 均 **genuine RED**——A 类型参数表空；B1 实参被当 `T` 报 2345 且返回 `unknown`；C约束 call 失败时尚报 2345、ref/arity 因检查相位**不访问类型节点** ⇒ 0 诊断；C默认 `c.v` 为 `T` 非 `number`。各以临时 probe 实测红后产最终测试复绿。B2/组合切片建立在 4bp 成员实例化 + 本轮 reference/signature 实例化之上（B2 为既有机制验证）。单测覆盖新机器，green-on-arrival——**如实记录非伪造红**（同 4bc–C-A 口径）。**无既有测试弱化/删除**（C-A 569 全绿；新行为均新增测试）。**测试增量**：**600 单测**（+31，相对 C-A 基线 569：check_test +19 / mapper_test +5 / declared_types_test +7）+ **144 doctest**（+8）。gate：`cargo test -p tsgo_checker`（600 lib + 144 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿（**未触 `internal/ast`，故未跑 `--workspace`**）。**与 Go 实测一致**：显式类型实参实例化、约束 2344（call `return nil` 抑制 2345）、默认填充、arity 2314/2707/2558、grammar 2706、嵌套/返回引用实例化——均按 `cmd/tsgo` 实测匹配。**已知偏离（记录）**：错 arity 引用类型形成 `Box<unknown>`（Go errorType，诊断对齐）、泛型 alias 实例化 DEFER。**DEFER（带 blocked-by）**：从实参推断类型实参（C-B2）/ `getVariances` marker-probe（C-C）/ 实例化缓存 / anonymous object 深实例化 / `this`·rest·泛型签名实例化 / 2315·qualified-name·`checkTypeParametersNotReferenced`。详见 impl.md「C-B1 落地记录」。

## C-B2 类型实参**推断** —— 从调用实参推断类型参数 行为单测（§8.6）

> Go 真值用 `go build -o /tmp/tsgo ./cmd/tsgo`（`--noEmit --strict`）实跑逐一核对（码 / message）。`check_test.rs` 走 `get_diagnostics` end-to-end；`inference_test.rs` 直接驱动私有/`pub(crate)` 解析机器。

| Rust 测试 | 验证行为 | 输入 → 期望（Go 实测）| 完成 |
|---|---|---|---|
| `generic_call_infers_type_argument_from_argument`（`check_test.rs`，slice 1，genuine red→green）| 无显式实参泛型调用从实参推断 T；红：`2345`(arg vs 裸 `T`)+`2322`(`unknown`)| `function id<T>(x:T):T{...} const r=id(1); const s:string=r;` → `2322`「Type 'number' is not assignable to type 'string'.」| ✓ |
| `generic_call_cross_base_candidates_infer_leftmost`（`check_test.rs`，slice 2）| 异基底字面量候选 → `getCommonSupertype` 取最左 `1`（**非联合**）| `function pick<T>(a:T,b:T):T{...} const r=pick(1,"x");` → `2345`「Argument of type '"x"' is not assignable to parameter of type '1'.」| ✓ |
| `generic_call_same_base_candidates_infer_union`（`check_test.rs`，slice 2）| 同基底字面量候选 → 联合 `1\|2`| `const n:1=pick(1,2);` → `2322`「Type '1 \| 2' is not assignable to type '1'.」| ✓ |
| `generic_call_same_base_union_assignable_to_base`（`check_test.rs`，slice 2 正控）| `1\|2` 可赋 `number`| `const n:number=pick(1,2);` → 0 | ✓ |
| `generic_call_infers_through_object_property`（`check_test.rs`，slice 4，genuine red→green）| 对象属性逐对推断 T；红：实参对**未实例化** `{v:T}` 报 2345 | `function f<T>(o:{v:T}):T{...} const r=f({v:1}); const s:string=r;` → `2322`「'number'...'string'.」| ✓ |
| `generic_call_infers_through_object_property_assignable`（`check_test.rs`，slice 4 正控）| `r` 实为 `number`| `const n:number=r;` → 0 | ✓ |
| `generic_call_callback_parameter_inferred_from_array_argument`（`check_test.rs`，slice 3，keystone）| 回调参数经推断 T 上下文定型为 `number`| `interface Array<T>{...} function map<T,U>(a:T[],f:(x:T)=>U):U[]{...} map([1,2],(x):string=>x);` → `2322`「'number'...'string'.」（cmd/tsgo 另报合成 Array vs 真 lib 的 2374，本 port 无真 lib 故无）| ✓ |
| `generic_call_callback_parameter_inferred_accepts_matching_return`（`check_test.rs`，slice 3 正控）| 回调返回匹配推断参数 → 0| `map([1,2],(x):number=>x);` → 0 | ✓ |
| `generic_call_inference_satisfies_constraint`（`check_test.rs`，slice 5 正控）| 推断满足约束 → 0| `function f<T extends number>(x:T):T{...} f(1);` → 0 | ✓ |
| `generic_call_inference_violates_constraint_infers_constraint`（`check_test.rs`，slice 5）| 推断违反约束 ⇒ 改推约束 `number`| `f("x");` → `2345`「Argument of type 'string' is not assignable to parameter of type 'number'.」| ✓ |
| `generic_call_no_candidate_infers_unknown`（`check_test.rs`，slice 6）| 无候选 ⇒ `unknown`，裸调用非错| `function f<T>():T{...} const r=f();` → 0 | ✓ |
| `generic_call_no_candidate_result_is_unknown`（`check_test.rs`，slice 6 揭示）| 结果确为 `unknown`| `f().toFixed();` → 1 诊断含「type 'unknown'」（port 2339 vs Go 2571，属性访问差异）| ✓ |
| `generic_call_explicit_type_argument_wins_over_inference`（`check_test.rs`，slice 7 正控）| 显式实参优先于推断（C-B1 路径不受扰）| `function id<T>(x:T):T{...} id<string>(1);` → `2345`「Argument of type 'number' is not assignable to parameter of type 'string'.」| ✓ |
| `literal_types_with_same_base_type_classifies_candidates`（`inference_test.rs`，单测）| `literalTypesWithSameBaseType`| `[1,2]`→true；`[1,"x"]`/`[1,number]`/`[number,number]`→false | ✓ |
| `find_leftmost_supertype_keeps_leftmost_when_unrelated`（`inference_test.rs`，单测）| `findLeftmostType`| `[1,"x"]`→`1`；`[1,number]`→`number` | ✓ |
| `get_common_supertype_unions_same_base_and_picks_leftmost_otherwise`（`inference_test.rs`，单测）| `getCommonSupertype`| `[1]`→`1`；`[1,2]`→`1\|2`；`[1,"x"]`→`1` | ✓ |
| `get_covariant_inference_widens_and_combines`（`inference_test.rs`，单测）| `getCovariantInference`| `[1]`→`1`；`[1,2]`→`1\|2` | ✓ |
| `get_inferred_type_for_call_no_candidate_is_unknown`（`inference_test.rs`，单测）| 无候选 → `unknown` + 缓存 | 空候选槽 → `unknown` | ✓ |
| `get_inferred_type_for_call_resolves_covariant_candidates`（`inference_test.rs`，单测）| covariant 候选解析 | `[1,2]`→`1\|2` | ✓ |
| `get_inferred_types_for_call_resolves_each_slot`（`inference_test.rs`，单测）| 每槽独立解析 | `[T:[1], U:[]]`→`[1, unknown]` | ✓ |

> 红→绿证据：slice 1 **genuine RED**（实参被当裸 `T` 报 2345 且返回 `unknown`）；slice 4 **genuine RED**（实参对**未实例化** `{v:T}` 报 2345——暴露 anonymous object 未深实例化，驱动 `instantiate_param_type`）。slice 2/3/5/6 在 slice 1/4 引擎 + anonymous-object 实例化之上验证多候选 `getCommonSupertype` / 回调记忆缓存 / 约束臂 / 无候选臂（各自首触其新代码路径）。slice 7 为 C-B1 显式路径不回归的正控。`inference_test` 单测直接驱动 `pub(crate)`/私有解析机器。**无既有测试弱化/删除**（C-B1 600 全绿；新行为均新增测试）。**测试增量**：**620 单测**（+20，相对 C-B1 基线 600：check_test +13 / inference_test +7）+ **144 doctest**（不变；新 `pub(crate)` 方法不带 runnable doctest）。gate：`cargo test -p tsgo_checker`（620 lib + 144 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿（**未触 `internal/ast`，故未跑 `--workspace`**）。**与 Go 实测一致**：恒等/对象属性/约束推断、异基底取最左 vs 同基底联合（`getCommonSupertype`）、回调参数经推断 T 上下文定型、无候选→unknown、显式优先——均按 `cmd/tsgo` 实测匹配（任务书设想的 `pick(1,"x")`→联合 与 cmd/tsgo 不符，已按 Go 取最左 `1`+2345）。**已知偏离（记录）**：回调结果 `U` 未推断（`unknown[]` vs Go `string[]`，C-B3）、contextual-return 缺失（`const n:number=f<T>()` 本 port 报 2322，C-B3）、`unknown` 属性访问 2339 vs Go 2571。**DEFER（带 blocked-by）**：contextual-return + 回调返回 `U` 推断 + `inferFromSignatures`（C-B3）/ contravariant + 优先级格 + mapped/conditional/keyof（C-C）/ 精确 `isContextSensitive` / 实例化缓存 / 嵌套 anonymous object 深实例化 / JSX 推断。详见 impl.md「C-B2 落地记录」。

## C-B3 泛型调用收口 —— 回调结果推断 + contextual-return + lazy mapper + 实例化缓存 行为单测（§8.6）

> Go 真值用 `go build -o /tmp/tsgo ./cmd/tsgo`（`--noEmit --strict`）实跑逐一核对（码 / message）。本 port 无 lib，回调体改用等价 lib-free 写法（`x.toFixed()`→`x+""`，均 `string`），并记录 cmd/tsgo 对应。`check_test.rs` 走 `get_diagnostics` end-to-end；`inference_test.rs`/`mapper_test.rs` 直接驱动私有/`pub(crate)` 机器。
>
> **数组印名偏离**：cmd/tsgo 印 `string[]`/`number[]`，本 port nodebuilder 无 `X[]` 糖 ⇒ 印 `Array<string>`/`Array<number>`；元素类型一致（即 C-B3 确立的语义）。

| Rust 测试（文件 / slice）| 行为 | input → expected | 完成 |
|---|---|---|---|
| `generic_call_infers_callback_result_from_body`（`check_test.rs`，slice 1 头条，genuine red→green）| 回调**体返回类型**推 `U`；红：`r`=`Array<unknown>`（C-B2 `U=unknown`）| `interface Array<T>{...} function map<T,U>(a:T[],f:(x:T)=>U):U[]{...} const r=map([1,2],x=>x+""); const n:number=r;` → `2322`「Type 'Array<string>' is not assignable to type 'number'.」（cmd/tsgo: `string[]`，等价体 `x.toFixed()`）| ✓ |
| `generic_call_callback_identity_infers_param_and_result`（`check_test.rs`，slice 2）| 恒等回调 `U=T`；参数经定型 ⇒ `U=number`| `const s:string[]=map([1,2],x=>x);` → `2322`「Type 'Array<number>' is not assignable to type 'Array<string>'.」（cmd/tsgo: `number[]`/`string[]` + 元素链）| ✓ |
| `generic_call_contextual_return_inference`（`check_test.rs`，slice 3，genuine red→green）| 无实参泛型调用从上下文类型推类参；红：`make()` 报 `Array<unknown>` vs `Array<number>`| `function make<T>():T[]{...} const xs:number[]=make(); const ys:string[]=make<number>();` → 仅 `2322`(line 4)「Type 'Array<number>' is not assignable to type 'Array<string>'.」（line 3 **0**：contextual-return 推 `T=number`）| ✓ |
| `generic_call_nested_callbacks_infer_result`（`check_test.rs`，slice 4）| 嵌套泛型调用 lazy mapper 跨解析正确| `const r=map(map([1,2],x=>x),y=>y+""); const n:number=r;` → `2322`「Type 'Array<string>' is not assignable to type 'number'.」（cmd/tsgo: `string[]`）| ✓ |
| `create_type_reference_interns_by_target_and_arguments`（`mapper_test.rs`，slice 5）| 实例化缓存：同 `(target,args)` 返稳定 id| 两次 `create_type_reference(t,[number])` 同 id；`[string]` 异 id；`instantiate_type(Array<T>,{T→number})` 两次同 id 且 == 直建 | ✓ |
| `infer_from_signatures_infers_parameter_and_return`（`inference_test.rs`，单测）| `inferFromSignatures`/`inferFromSignature`：参数 + 返回| `(x:number)=>string` vs `(x:T)=>U` → `[number, string]` | ✓ |
| `infer_with_priority_stronger_replaces_weaker`（`inference_test.rs`，单测）| 优先级 lattice：强优先级清弱候选| `RETURN_TYPE` 记 `string` → `NONE` 清并记 `number` → 后续 `RETURN_TYPE` 被忽略 | ✓ |
| `could_contain_type_variables_classifies_types`（`inference_test.rs`，单测）| `couldContainTypeVariables` 可达子集| `T`/`number\|T`/`Array<T>`→true；`number`/`Array<number>`→false | ✓ |
| `fixing_inference_mapper_maps_inferred_and_skips_empty`（`check_test.rs`，单测）| lazy `InferenceTypeMapper`：有候选→推断、无候选→自身| `T`(候选 number)→`number`；`U`(无候选)→`U` | ✓ |
| `get_return_type_from_body_widens_concise_literal`（`check_test.rs`，单测）| `getReturnTypeFromBody` concise 体 + 字面量加宽| `(x:number)=>"s"` → `string`（非 `"s"`）| ✓ |
| `inference_context_and_info_construction`（`inference_test.rs`，**既有改**）| 槽种子优先级 `MAX_VALUE`（旧 `NONE` 按 Go `newInferenceInfo` 修正）| `InferenceInfo::new` → `priority==MAX_VALUE` | ✓ |

> 红→绿证据：slice 1 **genuine RED**（`r=Array<unknown>` 暴露 `U` 未从回调体推断）；slice 3 **genuine RED**（`make()` 报 `Array<unknown>` 暴露无 contextual-return；且在加 `MAX_VALUE` 种子前仍红——`RETURN_TYPE(128)` 既非 `<` 也非 `==` 初始 `NONE(0)` ⇒ 候选不入，定位 Go `newInferenceInfo` 的 `MaxValue` 种子）。slice 2/4 在 slice 1 机器上验证 `U=T` 与嵌套。slice 5 + `inference_test`/`mapper_test` 单测直接驱动 `pub(crate)`/私有机器。**既有测试更新（Go-verified）**：`inference_context_and_info_construction` 由 `priority==NONE` 改 `==MAX_VALUE`（Go 种子）；**无既有测试弱化/删除**（C-B2 620 全绿；新行为均新增测试）。**测试增量**：**630 单测**（+10，相对 C-B2 基线 620：check_test +6 / inference_test +3 / mapper_test +1）+ **144 doctest**（不变）。gate：`cargo test -p tsgo_checker`（630 lib + 144 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿。**与 Go 实测一致**：回调体推 `U`（map-风格）、`U=T` 恒等、contextual-return（make-风格，显式实参仍胜）、嵌套、字面量加宽——均按 `cmd/tsgo` 实测匹配。**已知偏离（记录）**：数组印名 `Array<X>` vs `X[]`、数组↔数组 2322 元素 message-chain、lib 依赖回调体（`x.toFixed()`→`x+""`）。**DEFER（带 blocked-by）**：full contravariant + 完整优先级格 + mapped/conditional/keyof（C-C）/ 嵌套外层 `outerMapper`·`returnMapper` / 精确 `isContextSensitive` / `(type,mapper)` 通用实例化缓存 / JSX 推断。详见 impl.md「C-B3 落地记录」。**C-B（泛型基石）可达子集完成**（C-B1+C-B2+C-B3）。

## C-C1 keyof/index 类型 + 索引访问 `T[K]` 深化 行为单测（§8.6）

> Go 真值用 `go build -o /tmp/tsgo ./cmd/tsgo`（`--noEmit --strict`）实跑逐一核对（码 / message / span）。`check_test.rs` 走 `get_diagnostics` 端到端；`declared_types_test.rs`/`mapper_test.rs`/`inference_test.rs`/`nodebuilder_test.rs` 直接驱动 `pub`/`pub(crate)`/私有机器。**无 lib 依赖**（切片用本地 interface/type-alias）。

| Rust 测试（文件 / slice）| 行为 | input → expected | 完成 |
|---|---|---|---|
| `keyof_concrete_object_union_of_name_literals_e2e`（`check_test.rs`，slice 1 端到端，genuine red→green）| `keyof` 具体对象 = 属性名字面量并集；红：源 `"c"` 被错误加宽为 `string`（`type_could_have_top_level_singleton_types` 不迭代 union）| `type K = keyof { a: number; b: string }; const k: K="a"; const k2: K="c";` → 仅 `2322`「Type '"c"' is not assignable to type '"a" \| "b"'.」(line 3；line 2 接受) | ✓ |
| `concrete_indexed_access_type_node_e2e`（`check_test.rs`，slice 2 端到端）| 具体 `T["a"]` 类型节点 = 命名属性类型 | `type T = { a: number }; const x: T["a"]=1; const y: T["a"]="s";` → 仅 `2322`「Type 'string' is not assignable to type 'number'.」(line 3) | ✓ |
| `generic_keyof_indexed_access_call_inference_e2e`（`check_test.rs`，slice 3 头条，genuine red→green）| keyof + 索引访问 + C-B 推断串联；红：首跑 **2** 诊断（K 被钳成延迟 `keyof T`，约束未实例化）| `function get<T, K extends keyof T>(o: T, k: K): T[K] { return o[k]; } const r = get({ a: 1, b: "x" }, "a"); const s: string = r;` → 仅 `2322`「Type 'number' is not assignable to type 'string'.」(line 3) | ✓ |
| `generic_keyof_index_with_non_key_reports_2345_e2e`（`check_test.rs`，slice 5 端到端）| 非键索引：约束实例化 `keyof {a:number}="a"`，K 钳 `"a"`，实参 `"b"` 2345 | `function get<T,K extends keyof T>(o:T,k:K):T[K]{return o[k];} const r = get({ a: 1 }, "b");` → `2345`「Argument of type '"b"' is not assignable to parameter of type '"a"'.」(line 2) | ✓ |
| `get_index_type_of_object_is_union_of_name_literals`（`declared_types_test.rs`，单测）| `getIndexType` 具体对象臂 | `interface I{a;b}` → `keyof I == "a" \| "b"` | ✓ |
| `get_index_type_of_type_parameter_is_deferred_index`（`declared_types_test.rs`，单测）| `getIndexTypeForGenericType` 延迟 + interning | `keyof T`(类型参数) → `Index{target:T}`；二次同 id | ✓ |
| `get_indexed_access_type_generic_is_deferred`（`declared_types_test.rs`，单测）| 泛型 `T[K]` 延迟 + interning | `T[K]`(均类型参数) → `IndexedAccess{T,K}`；二次同 id | ✓ |
| `get_indexed_access_type_distributes_union_index`（`declared_types_test.rs`，单测）| union-index 分配 | `I["a"\|"b"]` → `number \| string` | ✓ |
| `keyof_type_operator_node_resolves_to_index_type`（`declared_types_test.rs`，单测）| `getTypeFromTypeOperatorNode` keyof 臂 | `type K = keyof I` → `"a" \| "b"` | ✓ |
| `indexed_access_type_node_resolves_property`（`declared_types_test.rs`，单测）| `getTypeFromIndexedAccessTypeNode` | `type A = I["a"]` → `number` | ✓ |
| `instantiate_type_index_recomputes_keyof`（`mapper_test.rs`，单测）| `instantiateTypeWorker` Index 臂 | `keyof T`{T→I} → `"a" \| "b"` | ✓ |
| `instantiate_type_indexed_access_resolves_property`（`mapper_test.rs`，单测）| `instantiateTypeWorker` IndexedAccess 臂 | `T[K]`{T→I,K→"a"} → `number` | ✓ |
| `could_contain_type_variables_index_and_indexed_access`（`inference_test.rs`，单测）| `couldContainTypeVariables` INSTANTIABLE 臂 | `keyof T`/`T[keyof T]` → true | ✓ |
| `type_to_string_index_and_indexed_access`（`nodebuilder_test.rs`，单测）| `typeToString` Index/IndexedAccess 印名 | `keyof T` → `"keyof T"`；`T["a"]` → `"T[\"a\"]"` | ✓ |

> 红→绿证据：slice 1 **genuine RED**（源 `"c"` 加宽为 `string`，定位 `type_could_have_top_level_singleton_types` 不迭代 union → 修）；slice 3 **genuine RED**（首跑 2 诊断，暴露约束 `keyof T` 未实例化即钳 K → 加 `inference_context_mapper` 约束实例化）；单测各自首触新代码路径（缺函数编译红 / 旧延迟臂返回恒等或 `None`）。**既有测试更新（Go-verified）**：无（`type_could_have_top_level_singleton_types` 改 union 迭代属修 bug，无既有测试断言其旧行为）；**无既有测试弱化/删除**（C-B3 630 全绿）。**测试增量**：**644 单测**（+14，相对 C-B3 基线 630：declared_types_test +6 / mapper_test +2 / inference_test +1 / nodebuilder_test +1 / check_test +4）+ **151 doctest**（+7：`IndexFlags`/`AccessFlags`/`IndexType`/`IndexedAccessType`/`new_index_type`/`new_indexed_access_type`/`get_index_type`）。gate：`cargo test -p tsgo_checker`（644 lib + 151 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿。**与 Go 实测一致**：keyof 具体并集 / `T["a"]` 命名属性 / 泛型 `get<T,K extends keyof T>` 推断（K 保字面量、约束实例化、`T[K]` 实例化为 `number`）/ 非键 2345 —— 均按 `cmd/tsgo --strict` 实测匹配。**DEFER（带 blocked-by）**：条件类型（C-C2）、mapped/template-literal（C-C3）、keyof over mapped、index access on intersection、`noUncheckedIndexedAccess`、reverse-mapped、`getConstraintOfIndexedAccess`/`getSimplifiedType` 化简机、accessNode-kind 急切解析、numeric 命名属性、`keyof any/never`、full literal 加宽 + `hasPrimitiveConstraint` 守卫。详见 impl.md「C-C1 落地记录」。

## C-C2 条件类型 `T extends U ? X : Y` + `infer` + distributive 行为单测（§8.6）

> Go 真值用 `go build -o /tmp/tsgo ./cmd/tsgo`（`--noEmit --strict`）实跑逐一核对（码 / message）。`check_test.rs` 走 `get_diagnostics` 端到端；`declared_types_test.rs`/`mapper_test.rs`/`types_test.rs`/`mod_test.rs`/`nodebuilder_test.rs` 直接驱动 `pub`/`pub(crate)`/私有机器。切片用本地 `interface Array<T>`/type-alias（无 lib 依赖）。

| Rust 测试（文件 / slice）| 行为 | input → expected | 完成 |
|---|---|---|---|
| `conditional_concrete_true_branch_e2e`（`check_test.rs`，slice 1，genuine red→green）| 具体条件真支；红：条件桩成 `error` ⇒ 0 诊断 | `type IsString<T> = T extends string ? "yes" : "no"; const a: IsString<string>="yes"; const b: IsString<string>="no";` → 仅 `2322`「Type '"no"' is not assignable to type '"yes"'.」(line 3；line 2 接受) | ✓ |
| `conditional_concrete_false_branch_e2e`（`check_test.rs`，slice 1b）| 具体条件假支 | `... const c: IsString<number>="no"; const d: IsString<number>="yes";` → 仅 `2322`「Type '"yes"' is not assignable to type '"no"'.」(line 3) | ✓ |
| `conditional_distributive_over_union_e2e`（`check_test.rs`，slice 2）| distributive over union；`ToArray<string\|number>`=`string[]\|number[]` | `type ToArray<T> = T extends unknown ? T[] : never; ... const x:ToArray<string\|number>=sa; const y:ToArray<string\|number>=ba;` → 仅 `2322`（`ba=boolean[]`，port 印 `Array<false \| true>` not assignable to `Array<string> \| Array<number>`；`sa=string[]` 接受） | ✓ |
| `conditional_infer_element_type_e2e`（`check_test.rs`，slice 3，genuine red→green）| `infer U` 元素位；红：`U` 引用解析失败 ⇒ 结果 `unknown` ⇒ 0 诊断 | `type ElementType<T> = T extends (infer U)[] ? U : never; const n:ElementType<number[]>=1; const s:ElementType<number[]>="x";` → 仅 `2322`「Type 'string' is not assignable to type 'number'.」(line 4) | ✓ |
| `conditional_infer_return_type_e2e`（`check_test.rs`，slice 4，genuine red→green）| `infer R` 返回位；红：匿名函数实例化 DEFER ⇒ `R` 未代入 ⇒ 假支 `never` ⇒ 2 诊断 | `type Ret<T> = T extends (...args: any[]) => infer R ? R : never; const r:Ret<() => number>=1; const r2:Ret<() => number>="x";` → 仅 `2322`「Type 'string' is not assignable to type 'number'.」(line 4) | ✓ |
| `conditional_deferred_through_generic_function_e2e`（`check_test.rs`，slice 5）| 延迟条件过泛型函数实例化解析 | `function f<T>(x:T): T extends string ? 1 : 2 {return null as any;} const a=f<string>("a"); const bad:2=a;` → 仅 `2322`「Type '1' is not assignable to type '2'.」(line 3) | ✓ |
| `get_conditional_type_resolves_true_branch`（`declared_types_test.rs`，单测）| `getConditionalType` 具体真支 + 延迟标志 | `IsString<T>` 声明 = 延迟 `Conditional`；`{T→string}` 实例化 → `"yes"` | ✓ |
| `get_conditional_type_resolves_false_branch`（`declared_types_test.rs`，单测）| `getConditionalType` 具体假支 | `IsString<T>`{T→number} → `"no"` | ✓ |
| `get_conditional_type_distributes_over_union`（`declared_types_test.rs`，单测）| `getConditionalTypeInstantiation` 分配 | `ToArray<T>`{T→string\|number} → 2-成员 union，各为 `Array<...>` 引用 | ✓ |
| `get_conditional_type_infers_element_type`（`declared_types_test.rs`，单测）| `infer` 候选收集 | `ElementType<T>`{T→Array<number>} → `number` | ✓ |
| `instantiate_type_conditional_without_program_is_deferred`（`mapper_test.rs`，单测）| `instantiateTypeWorker` Conditional 臂无 program 留延迟 | `new_conditional_type` 后 `instantiate_type` → 同 id | ✓ |
| `arena_conditional_type_accessor`（`types_test.rs`，单测）| `Type::as_conditional` + `ConditionalRoot`/`ConditionalType` | 构造 `Conditional` → 访问器返 root/check/extends；非条件返 `None` | ✓ |
| `new_conditional_type_and_program_less_printing`（`mod_test.rs`，单测）| `newConditionalType` + 程序无关 `typeToString` | `new_conditional_type` → `CONDITIONAL` 标志；印 `"T extends string ? ... : ..."` | ✓ |
| `type_to_string_conditional_type`（`nodebuilder_test.rs`，单测）| 程序感知 `typeToString` 条件臂 | `IsString<T>` 声明 → `"T extends string ? \"yes\" : \"no\""` | ✓ |

> 红→绿证据：slice 1 **genuine RED**（条件桩 `error` ⇒ 0 诊断）；slice 3 **genuine RED**（`infer U` 引用经 binder 匿名绑定不可 resolve ⇒ 结果 `unknown` ⇒ 0 诊断 → `resolve_type_parameter_in_scope` 加 conditional `infer` 作用域）；slice 4 **genuine RED**（匿名函数类型实例化 DEFER ⇒ `R` 未代入 ⇒ 假支 `never` ⇒ 2 诊断 → `get_conditional_type` 改走程序感知 `instantiate_param_type`）；单测各自首触新代码路径（缺函数编译红 / 旧路径返恒等/`error`）。**既有测试更新（Go-verified）**：无；新增的泛型 alias 实例化（`get_type_from_type_reference`）与 conditional `infer` 作用域（`resolve_type_parameter_in_scope`）均加法式（旧路径对泛型 alias 返 `create_type_reference`、对 `infer` 名返 `error`，无既有测试断言其旧值）；**无既有测试弱化/删除**（C-C1 644 全绿）。**测试增量**：**658 单测**（+14，相对 C-C1 基线 644：check_test +6 / declared_types_test +4 / mapper_test +1 / types_test +1 / mod_test +1 / nodebuilder_test +1）+ **155 doctest**（+4：`ConditionalRoot`/`ConditionalType`/`new_conditional_type`/`get_conditional_type`）。gate：`cargo test -p tsgo_checker`（658 lib + 155 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿。**与 Go 实测一致**：具体真/假支、distributive over union（`string[]\|number[]`，行为匹配；印名因 array-shorthand + `boolean`=`false\|true` 偏离）、infer 元素/返回（=`number`）、延迟过泛型函数（=`1`）—— 均按 `cmd/tsgo --strict` 实测匹配。**DEFER（带 blocked-by）**：mapped/template-literal/string-mapping（C-C3）、条件 tail-recursion 循环、`getConstraintOfConditionalType`/`getDefaultConstraintOfConditionalType` 化简机、conditional-vs-conditional 关系（`combinedMapper`/`getTrueTypeFromConditionalType`）、`any` check-type `X\|Y` 分配（`extraTypes`）+ `forConstraint` + wildcard、`infer ... extends`（constrained infer）+ `getInferredTypeParameterConstraint`、simple-tuple 延迟、alias attribution、匿名 object 深实例化全局化、印名偏离（array shorthand / `false\|true`→`boolean`）。详见 impl.md「C-C2 落地记录」。

## C-C3 mapped types `{[K in keyof T]: V}` + template literal types + intrinsic string mapping 行为单测（§8.6）

> 逐行为 red→green（见 impl.md「C-C3 落地记录」）。先建类型基建（`MappedTypeModifiers`/`StringMappingKind`/`TemplateLiteralType`/`StringMappingType` + 2 `TypeData` 变体 + mapped 用 `Object`+`MAPPED`+侧表）→ 模板字面量 → 内置字符串映射 → 映射类型（Partial/Readonly/Required/`as`）。expected 全部取自 `cmd/tsgo --noEmit --strict` 实测（断言前捕获）。

| Rust 测试（文件 / 切片）| 被测 Go 函数 | input → observable | 完成 |
|---|---|---|---|
| `template_literal_concrete_e2e`（`check_test.rs`，s4，genuine red→green）| `getTemplateLiteralType`（all-literal → string literal）；红：`TemplateLiteralType` 节点落 `_` 臂 ⇒ `error` ⇒ 0 诊断 | `type T = \`a${"x"}b\`; const t: T="axb"; const u: T="nope";` → 仅 `2322`「Type '"nope"' is not assignable to type '"axb"'.」(line 3) | ✓ |
| `template_literal_union_distribution_e2e`（`check_test.rs`，s5）| `getTemplateLiteralType`（union 分配）| `type T = \`x${"a" \| "b"}\`; const t: T="xa"; const u: T="xc";` → 仅 `2322`「Type '"xc"' is not assignable to type '"xa" \| "xb"'.」(line 3) | ✓ |
| `intrinsic_uppercase_e2e`（`check_test.rs`，s6，genuine red→green）| `getStringMappingType`/`applyStringMapping`；红：`Uppercase<"abc">` 解成桩 body `string` ⇒ 0 诊断 | `type Uppercase<S extends string>=string; type U=Uppercase<"abc">; const u: U="ABC"; const v: U="abc";` → 仅 `2322`「Type '"abc"' is not assignable to type '"ABC"'.」(line 4)（divergence: alias body `string` 占位 Go `intrinsic`；按名派发） | ✓ |
| `mapped_partial_optional_e2e`（`check_test.rs`，s1，genuine red→green）| `instantiateMappedType`/`resolveMappedTypeMembers`（`?`）；红：解成空 mapped 对象 ⇒ 0 诊断 | `type Partial2<T>={[K in keyof T]?:T[K]}; type P=Partial2<{a:number}>; const p:P={}; const q:P={a:"x"};` → 仅 `2322`「Type 'string' is not assignable to type 'number'.」(line 4；`{}` 接受) | ✓ |
| `mapped_required_strips_optional_e2e`（`check_test.rs`，s3，genuine red→green）| `resolveMappedTypeMembers`（`-?` 去可选）；红：合成属性名喂 `program.symbol` panic ⇒ 加 `resolved_symbol_name` | `type Required2<T>={[K in keyof T]-?:T[K]}; type R=Required2<{a?:number}>; const r:R={};` → 仅 `2741`「Property 'a' is missing in type '{}' but required in type '{ a: number; }'.」(line 3)（divergence: 印名无 alias 归属/strict-null off） | ✓ |
| `mapped_as_identity_remap_e2e`（`check_test.rs`，s7）| `resolveMappedTypeMembers`（nameType remap）| `type Id<T>={[K in keyof T as K]:T[K]}; type R=Id<{a:number}>; const bad:R={a:"x"};` → 仅 `2322`「Type 'string' is not assignable to type 'number'.」(line 3) | ✓ |
| `instantiate_mapped_type_partial_makes_optional`（`declared_types_test.rs`，单测）| `instantiateMappedType`（`+?`）| `Partial2<{a:number;b:string}>` → 2 属性均 OPTIONAL，类型保 `number`/`string` | ✓ |
| `instantiate_mapped_type_readonly_adds_modifier`（`declared_types_test.rs`，单测；s2 替代 2540 断言）| `resolveMappedTypeMembers`（IncludeReadonly）| `Readonly2<{a:number}>` → 印 `{ readonly a: number; }`，属性带 `CheckFlags::READONLY`、非 OPTIONAL | ✓ |
| `instantiate_mapped_type_required_strips_optional`（`declared_types_test.rs`，单测）| `resolveMappedTypeMembers`（ExcludeOptional）| `Required2<{a?:number}>` → `a` 非 OPTIONAL，类型 `number` | ✓ |
| `instantiate_mapped_type_as_remaps_keys`（`declared_types_test.rs`，单测）| nameType remap（模板）| `Prefixed<{a:number}>`（`as \`p_${K}\``）→ `{ p_a: number; }`，旧键 `a` 不存在（divergence: Go 额外报 symbol 占位符诊断；`string & K` 化简 DEFER）| ✓ |
| `get_template_literal_type_concrete_folds_to_string_literal`（`declared_types_test.rs`，单测）| `getTemplateLiteralType`（折叠）| `\`a${"x"}b\`` → `"axb"` | ✓ |
| `get_template_literal_type_distributes_over_union`（`declared_types_test.rs`，单测）| `getTemplateLiteralType`（union）| `\`x${"a"\|"b"}\`` → `"xa" \| "xb"` | ✓ |
| `get_template_literal_type_generic_is_deferred`（`declared_types_test.rs`，单测）| `getTemplateLiteralType`（generic 延迟）| `\`p_${T}\`` → deferred `TEMPLATE_LITERAL`（texts/types 保留）| ✓ |
| `get_string_mapping_type_transforms_concrete_literals`（`declared_types_test.rs`，单测）| `applyStringMapping`（4 种）| `Uppercase<"abc">`=`"ABC"`/`Capitalize`=`"Abc"`/`Lowercase<"ABC">`=`"abc"`/`Uncapitalize`=`"aBC"` | ✓ |
| `get_string_mapping_type_distributes_and_defers`（`declared_types_test.rs`，单测）| `getStringMappingType`（union/generic）| `Uppercase<"a"\|"b">`=`"A"\|"B"`；`Uppercase<T>` → deferred `STRING_MAPPING` | ✓ |
| `instantiate_template_literal_folds_when_placeholder_resolved`（`mapper_test.rs`，单测）| `instantiateTypeWorker`（TEMPLATE_LITERAL 臂）| `\`p_${T}\``{T→"x"} → `"p_x"` | ✓ |
| `instantiate_string_mapping_folds_when_target_resolved`（`mapper_test.rs`，单测）| `instantiateTypeWorker`（STRING_MAPPING 臂）| `Uppercase<T>`{T→"abc"} → `"ABC"` | ✓ |
| `new_template_literal_type_and_program_less_printing`（`mod_test.rs`，单测）| `newTemplateLiteralType` + 程序无关 `typeToString`| `new_template_literal_type` → `TEMPLATE_LITERAL` 标志 + interned；印 `` `a${T}b` `` | ✓ |
| `new_string_mapping_type_and_program_less_printing`（`mod_test.rs`，单测）| `newStringMappingType` + 程序无关 `typeToString`| `new_string_mapping_type` → `STRING_MAPPING` 标志 + interned；印 `Uppercase<T>` | ✓ |
| `template_literal_and_string_mapping_accessors`（`types_test.rs`，单测）| `Type::as_template_literal`/`as_string_mapping`| 构造两变体 → 访问器返 payload；非对应返 `None` | ✓ |
| `string_mapping_kind_and_mapped_type_modifiers`（`types_test.rs`，单测）| `StringMappingKind::from_name`/`intrinsic_name` + `MappedTypeModifiers` 位 | 4 名映射 + 位位置（1<<0..1<<3）| ✓ |

> 红→绿证据：s4 **genuine RED**（`TemplateLiteralType` 落 `_` 臂 ⇒ `error` ⇒ 赋 error 0 诊断）；s6 **genuine RED**（`Uppercase<"abc">` 解成桩 `string` ⇒ 0 诊断）；s1 **genuine RED**（空 mapped 对象 ⇒ 0 诊断）；s3 首跑 **panic**（合成属性 id 喂 `program.symbol` 越界 → `resolved_symbol_name` synthesized-aware 修复）。s5（union 分配臂首次触发）/s7（identity nameType 臂）+ 各单测各自首触新代码路径（缺函数编译红 / 旧 `_` 臂返恒等）——**如实记录非伪造红**（同 C-C1/C-C2 口径）。**既有测试更新（Go-verified）**：无；`report_unmatched_property` 等 4 处改 `resolved_symbol_name` 属 synthesized-aware **bug 修复**（合成属性名此前从未走该报告路径，无既有测试断言其旧—会 panic 的—行为）；**无既有测试弱化/删除**（C-C2 658 全绿）。**测试增量**：**679 单测**（+21，相对 C-C2 基线 658：check_test +6 / declared_types_test +9 / mapper_test +2 / mod_test +2 / types_test +2）+ **165 doctest**（+10：`MappedTypeModifiers`/`StringMappingKind`/`TemplateLiteralType`/`StringMappingType` + `new_template_literal_type`/`new_string_mapping_type`/`new_mapped_type` + `get_template_literal_type`/`get_string_mapping_type`/`instantiate_mapped_type`）。gate：`cargo test -p tsgo_checker`（679 lib + 165 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿。**与 Go 实测一致**：模板具体折叠/union 分配、`Uppercase`/`Lowercase`/`Capitalize`/`Uncapitalize`、Partial（`?`）/Readonly（`readonly`）/Required（`-?`）/`as` remap —— 均按 `cmd/tsgo --noEmit --strict` 实测匹配。**已知偏离（记录）**：内置按名派发（vs `intrinsic` marker）/ 映射结果印名无 alias 归属 / 2540 未接线（改断言修饰符）/ 模板占位符约束未检查 + `string & K` 不化简（模板-`as` 用 identity + 直测）/ 急切成员解析（vs Go lazy）。**DEFER（带 blocked-by）**：reverse-mapped 推断、非同态/数组·元组同态、条件-`as` 过滤、keyof over mapped、strict-null `?`-undefined、模板关系/推断、wildcard。详见 impl.md「C-C3 落地记录」。**C-C（高级类型）= 可达子集完成**（C-C1 keyof/indexed-access + C-C2 conditional/infer + C-C3 mapped/template/string-mapping）。

## C-D1 测试（strictNullChecks 完成 + 枚举）

> 全部经 `cmd/tsgo --noEmit --strict`（或 `--strictNullChecks false`）实测捕获后断言。端到端经 `StubProgram`(`parse_and_bind`/`parse_and_bind_with_options`) + `new_checker` + `get_diagnostics`；单测直驱类型 API。

| 测试 | Go 锚 | input → expected（实测）| 完成 |
|---|---|---|---|
| `optional_property_read_is_number_or_undefined_under_strict`（`check_test.rs`）| `getTypeForVariableLikeDeclaration`/`addOptionalityEx` | `const o:{a?:number}={};const n:number=o.a;` → 1×2322「Type 'undefined \| number' is not assignable to type 'number'.」（端口 union 序；Go 印 `number \| undefined`）| ✓ |
| `assign_empty_object_to_optional_property_is_ok`（`check_test.rs`）| relater（optional target 非必填）| `{}`→`{a?:number}` → 0 | ✓ |
| `optional_property_read_is_bare_type_without_strict_null_checks`（`check_test.rs`）| `addOptionalityEx`（!strict no-op）| 非 strict `o.a`=裸 `number` → 0 | ✓ |
| `optional_parameter_is_number_or_undefined_in_body`（`check_test.rs`）| `getTypeForVariableLikeDeclaration`（param）| `function f(x?:number){const n:number=x;}` → 1×2322「'undefined \| number'/'number'」| ✓ |
| `optional_parameter_is_bare_type_without_strict_null_checks`（`check_test.rs`）| 同上（!strict）| 非 strict → 0 | ✓ |
| `type_facts_of_symbol_is_truthy_non_nullable`（`type_facts_test.rs`，单测）| `getTypeFactsWorker`（ESSymbolLike）| `get_type_facts(symbol)`=`NE_*\|TRUTHY`（无 FALSY/Is/EQ）| ✓ |
| `type_facts_of_non_primitive_object_is_truthy_non_nullable`（`type_facts_test.rs`，单测）| `getTypeFactsWorker`（NonPrimitive）| `get_type_facts(object)`=`NE_*\|TRUTHY` | ✓ |
| `numeric_enum_member_and_union_assignability`（`check_test.rs`）| `getDeclaredTypeOfEnum`/`isSimpleTypeRelatedTo` | `enum E{A,B};const a:E=E.A;const c:E=2;` → 仅 1×2322「Type '2' is not assignable to type 'E'.」| ✓ |
| `matching_numeric_literal_is_assignable_to_enum`（`check_test.rs`）| `isSimpleTypeRelatedTo`（num-lit→enum-lit 同值）| `const c0:E=0;const c1:E=1;` → 0 | ✓ |
| `enum_auto_increment_after_explicit_initializer`（`check_test.rs`）| `computeEnumMemberValue` | `enum E{A=1,B};const b2:1=E.B;` → 1×2322「'E.B'/'1'」（`E.B`=2）| ✓ |
| `enum_auto_increment_between_explicit_initializers`（`check_test.rs`）| `computeEnumMemberValues` | `enum E{A=10,B,C=20};const b2:12=E.B;` → 1×2322「'E.B'/'12'」（`E.B`=11）| ✓ |
| `string_enum_member_and_assignability`（`check_test.rs`）| `getEnumLiteralType`(string) | `enum E{A="x"};const s:string=E.A;const e:E="x";` → 仅 1×2322「Type '"x"' is not assignable to type 'E'.」（单成员印 `E`）| ✓ |
| `enum_member_value_reference_assignability`（`check_test.rs`）| `getTypeOfEnumMember` | `const n:number=E.A;const e:E=E.B;const bad:5=E.A;` → 仅 1×2322「'E.A'/'5'」| ✓ |
| `const_enum_member_typing`（`check_test.rs`）| `getDeclaredTypeOfEnum`（const）| `const enum E{A,B};const bad:5=E.B;` → 1×2322「'E.B'/'5'」| ✓ |
| `enum_constant_expression_members_via_evaluator`（`check_test.rs`）| `computeConstantEnumMemberValue`/`evaluate` | `enum E{A=1<<1,B=A\|1};const b2:4=E.B;` → 1×2322（`B`=3）| ✓ |
| `numeric_enum_declared_type_is_union_of_member_literals`（`declared_types_test.rs`，单测）| `getDeclaredTypeOfEnum` | `enum E{A,B}` 声明类型 = `UNION\|ENUM_LITERAL`，成员值 {0,1}| ✓ |
| `enum_member_value_computation`（`declared_types_test.rs`，单测）| `computeEnumMemberValue` | `E{A=1,B}`→`A`=1/`B`=2；`S{A="x"}`→`A`="x"| ✓ |
| `declared_type_of_enum_exposes_members`（`declared_types_test.rs`，单测，**更新**）| `getTypeOfFuncClassEnumModule`/`getDeclaredTypeOfEnum` | 值侧对象暴露成员 `A`；类型侧（单成员）= `ENUM_LITERAL` 字面量（值 0）| ✓ |

> 红→绿证据：可选属性/参数（裸类型→注 `undefined`）、枚举数值/字符串/auto-incr/求值器/成员引用均 **genuine RED**（注入前 0 诊断；枚举前无声明类型机制）；matching-numeric/const-enum 为 green-on-arrival 守卫（位标志臂 / const-regular 同路径），如实记录。**既有测试更新（Go-verified，4 处，无弱化）**：`declared_type_of_enum_exposes_members`（值/类型侧分离）、`assignability_chain_optional_source_required_target_reports_2327`（2327 仅非 strict，改非 strict）、`comparable_is_lenient_about_optional_vs_required`（隔离 skipOptional，改非 strict）、`mapped_required_strips_optional_e2e`+`instantiate_mapped_type_required_strips_optional`（`-?` stripOptional 完成 C-C3 DEFER）。**测试增量**：**696 单测**（+17，相对 C-C3 基线 679：check_test +13 / type_facts_test +2 / declared_types_test +2）+ **165 doctest**（±0）。gate：`cargo test -p tsgo_checker`（696 lib + 165 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿。**DEFER（带 blocked-by）**：2454 definite-assignment（markNodeAssignments + 模块检测基建不足）、exactOptionalPropertyTypes、类型位 `E.A`、typeof facts 位 / bigint `0n`、枚举 fresh/alias/computed/合并/诊断/emit 内联。详见 impl.md「C-D1 落地记录」。

## C-D2 测试（namespaces / this-type / abstract / overload-elaboration / switch exhaustiveness）

> 全部经 `cmd/tsgo --noEmit --strict` 实测捕获后断言。端到端经 `StubProgram::parse_and_bind` + `new_checker` + `get_diagnostics`；单测直驱 `pub`/`pub(crate)` 机器。

| Rust 测试（文件 / slice）| Go 锚 | input → expected（实测）| 完成 |
|---|---|---|---|
| `namespace_value_member_read_is_number`（`check_test.rs`，A1，genuine red→green）| `getTypeOfFuncClassEnumModuleWorker`(module) | `namespace N { export const x = 1; } const n: string = N.x;` → 1×2322「Type 'number' is not assignable to type 'string'.」（红：2339，module 值类型无成员）| ✓ |
| `namespace_value_missing_member_prints_typeof_n`（`check_test.rs`，A1b，genuine red→green）| `typeToTypeNodeWorker`(typeof query) | `... const y = N.y;` → 1×2339「Property 'y' does not exist on type 'typeof N'.」（红：印 `'N'`）| ✓ |
| `namespace_type_member_resolves_qualified_name`（`check_test.rs`，A2，genuine red→green）| `resolveEntityName`/`resolveQualifiedName` | `namespace N { export type T = number; } const t: N.T = 1; const u: N.T = "x";` → 仅 1×2322「...'string'...'number'.」（红：0 诊断）| ✓ |
| `nested_namespace_value_member_read_is_number`（`check_test.rs`，A3）| `getTypeOfFuncClassEnumModuleWorker`(nested) | `namespace A { export namespace B { export const x = 1; } } const n: string = A.B.x;` → 1×2322 | ✓ |
| `merged_namespace_accumulates_exports`（`check_test.rs`，A4）| `getExportsOfSymbol`(merged) | `namespace N{export const x=1} namespace N{export const y=2} ...const c:string=N.x;` → 仅 1×2322（x/y 均可达）| ✓ |
| `this_in_method_is_class_instance_type`（`check_test.rs`，B1，genuine red→green）| `checkThisExpression`/`tryGetThisTypeAtEx` | `class C { x = 1; m() { const n: string = this.x; } }` → 1×2322（红：2339→0→2322 两阶段）| ✓ |
| `this_type_return_annotation_is_class_instance_type`（`check_test.rs`，B2）| `getThisType`(this-type node) | `class C { x = 1; m(): this { return this; } } const c = new C(); const d: string = c.m();` → 1×2322「Type 'C' is not assignable to type 'string'.」| ✓ |
| `new_abstract_class_reports_2511`（`check_test.rs`，C1，genuine red→green）| `resolveNewExpression`(abstract) | `declare abstract class A {} new A();` → 1×2511「Cannot create an instance of an abstract class.」（divergence: 用 `declare abstract` 因 parser 不识别 bare `abstract class`）| ✓ |
| `new_concrete_class_is_ok`（`check_test.rs`，C1b）| `resolveNewExpression`(非 abstract) | `class C {} new C();` → 0 | ✓ |
| `abstract_method_in_non_abstract_class_reports_1244`（`check_test.rs`，C2，genuine red→green）| `checkGrammarModifiers`(abstract) | `class B { abstract n(): void; }` → 1×1244「Abstract methods can only appear within an abstract class.」（红：0 诊断）| ✓ |
| `overload_failure_reports_2769_with_elaboration_chain`（`check_test.rs`，D1，genuine red→green×2）| `getSignaturesOfSymbol`(去重)/`reportCallResolutionErrors` | `function f(x:number):void; function f(x:string):void; function f(x:any){} f(true);` → 1×2769「No overload matches this call.」+ 链 2770「The last overload gave the following error.」→ 2345（红：先 0 诊断【impl 吞掉】→去重后 bare 2769 无链）；leaf 印 `false \| true`（divergence）| ✓ |
| `exhaustive_switch_default_narrows_discriminant_to_never`（`check_test.rs`，E1）| `narrowTypeBySwitchOnDiscriminant`(default 补集) | `...switch(x:"a"\|"b"){case "a":return 1;case "b":return 2;default:const y:never=x;return y;}` → 0（穷尽 default→never）| ✓ |
| `non_exhaustive_switch_default_narrows_to_leftover_member`（`check_test.rs`，E1b）| 同上 | `x:"a"\|"b"\|"c"` 缺 c → 1×2322「Type '"c"' is not assignable to type 'never'.」| ✓ |
| `exhaustive_switch_without_trailing_return_is_ok`（`check_test.rs`，E1c）| `checkAllCodePathsInNonVoidFunctionReturnOrThrow` | `function f(x:"a"\|"b"):number { switch(x){case "a":return 1;case "b":return 2;} }` → 0（穷尽 ⇒ 不报 2366；非穷尽 2366 DEFER）| ✓ |
| `trailing_reference_after_exhaustive_throw_switch_is_never`（`check_test.rs`，E1d）| `narrowTypeBySwitchOnDiscriminant`(no-match 补集) | 穷尽 all-throw 后 `const y: number = x;` → 0（`x`=never）| ✓ |
| `trailing_reference_after_non_exhaustive_throw_switch_is_leftover`（`check_test.rs`，E1e）| 同上 | 非穷尽 all-throw 后 `const y: number = x;` → 1×2322「Type 'string' is not assignable to type 'number'.」（leftover 加宽）| ✓ |
| `namespace_value_type_exposes_exports`（`declared_types_test.rs`，单测）| `getTypeOfFuncClassEnumModuleWorker` | `get_type_of_symbol(N)` → object 暴露 `x`（`number` 字面量基底）；`y` 不存在 | ✓ |
| `resolve_entity_name_resolves_qualified_namespace_member`（`declared_types_test.rs`，单测）| `resolveEntityName`/`resolveQualifiedName` | `N.T` qualified name → 导出别名 `T`（`TYPE_ALIAS`）| ✓ |

> 红→绿证据：A1/A2/A1b/B1/C2/D1 **genuine RED**（见 impl.md「C-D2 落地记录」表）；A3/A4/B2/C1/E1×5 green-on-arrival（建立在 A1 机器 / 既有 4t 流分析之上）如实记录。**既有测试更新（Go-verified）**：**无**（新行为均加法式，旧路径返 `any`/含 impl 签名/0 诊断/error_type，无既有测试断言其旧值）；**无既有测试弱化/删除**（C-D1 696 全绿）。**测试增量**：**714 单测**（+18，相对 C-D1 基线 696：check_test +16 / declared_types_test +2）+ **165 doctest**（±0）。gate：`cargo test -p tsgo_checker`（714 lib + 165 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` 绿。**与 Go 实测一致**：namespace 值/类型/嵌套/合并、this-type/多态 this、abstract 2511/1244、overload 2769→2770→2345 链、switch never-narrowing —— 均按 `cmd/tsgo --strict` 实测匹配。**已知偏离（记录）**：2769 leaf 印 `false\|true`（vs `boolean`）、abstract 用 `declare abstract class`（parser 不识别 bare）、switch 非穷尽缺 2366（binder 不暴露 `EndFlowNode`）。**DEFER（带 blocked-by）**：2366/2367/noImplicitReturns（binder EndFlowNode + `is_reachable_flow_node` &self，out-of-scope）、bare abstract / 前导修饰符声明（parser guard，out-of-scope）、复杂声明合并、多态 this 深用例、泛型 overload 链、ambient/import-equals/export-equals namespace、类型位 `E.A`。详见 impl.md「C-D2 落地记录」。**C-D（umbrella）= 可达子集完成**（C-D1 strictNull+enums + C-D2 namespaces+this+abstract+overload-elaboration+exhaustiveness）。

## C-E 测试（EmitResolver const-value + `IsDeclarationVisible` 扩展 + lib globals Promise 双侧）

> const-enum 真值经 `cmd/tsgo --target esnext --module preserve` 实跑捕获后断言（`var x = E.A` → `var x = 10`）。EmitResolver 方法经 `StubProgram::parse_and_bind` + `get_emit_resolver` 直驱（emit-only 方法无端到端诊断，用返回值单测）；lib globals 经 `new_checker` + 合成 global 单测，并验"无 program/无 lib → 优雅降级 None"双侧。

| Rust 测试（文件 / slice）| Go 锚 | input → expected（实测）| 完成 |
|---|---|---|---|
| `get_constant_value_of_enum_member_node`（`emit_resolver_test.rs`，S1 tracer，genuine red→green）| `services.go:GetConstantValue`（EnumMember 臂）/`checker.go:getEnumMemberValue` | `const enum E { A = 10 }` 的 `EnumMember` `A` → `EvalValue::Num(10)`（红：`get_constant_value` 方法不存在）| ✓ |
| `get_constant_value_of_const_enum_property_access`（`emit_resolver_test.rs`，S2，genuine red→green）| `services.go:GetConstantValue`（resolved symbol + `IsEnumConst`）| `const enum E { A=10, B=A+5 }\nvar x=E.A;\nvar y=E.B;` → `E.A`=`Num(10)`、`E.B`=`Num(15)`（红：缩到仅 EnumMember 臂时 `E.A` 返 `None`）；cmd/tsgo 验 `var x = 10 /* E.A */; var y = 15 /* E.B */;` | ✓ |
| `get_constant_value_of_non_const_enum_property_access_is_none`（`emit_resolver_test.rs`，S2 guard）| 同上（`IsEnumConst` false 分支）| 非-const `enum E { A=10 }\nvar x=E.A;` → `E.A`=`EvalValue::None`（不内联）；cmd/tsgo 验 `var x = E.A;` 保留 | ✓ |
| `is_declaration_visible_for_exported_variable_declaration`（`emit_resolver_test.rs`，S3，genuine red→green）| `emitresolver.go:determineIfDeclarationIsVisible` + `utilities.go:GetCombinedModifierFlags`/`GetDeclarationContainer` | `export const x = 1;\nconst y = 2;` → `VariableDeclaration` `x`=visible（combined `export`→容器 SourceFile）、`y`=not visible（红：旧仅查 node 自身 `export` 修饰，`VariableDeclaration` 无修饰→false）| ✓ |
| `exported_declaration_is_visible`（`emit_resolver_test.rs`，回归）| `emitresolver.go:IsDeclarationVisible` | `export function f() {}\nfunction g() {}` → `f`=visible、`g`=not visible（C-E 重写后仍绿）| ✓ |
| `get_global_promise_type_resolves_and_degrades`（`mod_test.rs`，S4，genuine red→green）| `checker.go:getGlobalPromiseType` | 合成 `interface Promise<T>{ then: () => void }` → `get_global_promise_type()`=Some(object，二次同 id)；无 Promise/无 program → None（红：方法不存在；优雅降级不 panic）| ✓ |
| `get_global_promise_constructor_symbol_resolves_and_degrades`（`mod_test.rs`，S5，genuine red→green）| `checker.go:getGlobalPromiseConstructorSymbol` | 合成 `declare var Promise: { resolve(): void };` → `get_global_promise_constructor_symbol()`=Some(VALUE 符号)；无 → None（红：方法不存在；优雅降级）| ✓ |

> 红→绿证据：S1/S4/S5 **genuine RED（编译错误：方法不存在）**；S2 **genuine RED**（先把 `get_constant_value` 缩成仅 `EnumMember` 臂跑红，`E.A` 返 `None`≠`Num(10)`，再补属性访问解析+`is_enum_const` 转绿）；S3 **genuine RED**（旧 `is_declaration_visible` 对 `VariableDeclaration` 返 false，应 true）。S2 非-const guard + S4/S5 的"无 program→None"为同切片双侧断言（present↔absent），如实记录。**既有测试更新（Go-verified）**：**无**（`is_declaration_visible` 重写为加法式行为扩展，既有 `exported_declaration_is_visible` 仍绿；其余全新增方法）；**无既有测试弱化/删除**（C-D2 714 全绿不变）。**测试增量**：**720 单测**（+6，相对 C-D2 基线 714：emit_resolver_test +4 / mod_test +2）+ **169 doctest**（+4：`get_enum_member_value`/`get_constant_value`/`get_global_promise_type`/`get_global_promise_constructor_symbol`）。gate：`cargo test -p tsgo_checker`（720 lib + 169 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` + `-p tsgo_transformers` 绿。**与 Go 实测一致**：const-enum 内联 `var x = 10`/`var y = 15`、非-const 不内联、`export const x` 可见。**已知偏离（记录）**：`GetConstantValue`/`GetEnumMemberValue` 返 `EvalValue` 而非 `any`/`evaluator.Result`（`None`=Go nil；bookkeeping flags DEFER）、`get_global_promise_type` 缺时返 `None`（非 `emptyGenericType`）、`IsDeclarationVisible` 非导出→保守 false（全局脚本案 blocked-by external-module detection）。**DEFER（带 blocked-by）**：声明 type-node 合成（`create_type_of_declaration` 等）→ **交接 D-F**、`IsEntityNameVisible`/`ReferenceResolver` 委托面/`IsLateBound`/`RequiresAddingImplicitUndefined`/… → 各自基建（P5/P6）、`get_type_reference_serialization_kind` Promise 臂 wiring（已备 helper，接入需 `resolveEntityName` 一致性 + 真 lib，P6）。详见 impl.md「C-E 落地记录」。**C-E = 可达子集完成 ⇒ P4 checker 可达子集 C-A..C-E 全部完成**。

## D-F2 测试（声明 emit 句法 type-node 合成：`type_to_type_node` + `create_type_of_declaration` + literal-const）

> C-E 把 type-node 合成交接 D-F；本轮在 checker 侧落地其可达核心（由 transformers `ensure_type` 消费，见 transformers `tests.md`「declarations D-F2」）。`.d.ts` Go 真值经 `go build -o /tmp/tsgo ./cmd/tsgo` 实跑 `--declaration --emitDeclarationOnly --strict` 逐 snippet 捕获（python `repr`）。EmitResolver 方法经 `StubProgram::parse_and_bind` + `get_emit_resolver` + `&mut Checker` 直驱；`type_to_type_node` 经构造类型直驱。

| Rust 测试（文件 / slice）| Go 锚 | input → expected（实测）| 完成 |
|---|---|---|---|
| `create_type_of_declaration_widens_let_number`（`emit_resolver_test.rs`，C1 tracer，genuine red→green）| `emitresolver.go:CreateTypeOfDeclaration`→`serializeTypeForDeclaration` | `let n = 1;` → `Keyword(NumberKeyword)`（红：方法不存在）| ✓ |
| `create_type_of_declaration_widens_let_string_and_boolean`（`emit_resolver_test.rs`，C2，genuine red→green）| 同上（`getWidenedLiteralType`）| `let s="a"`→`Keyword(String)`、`let b=true`→`Keyword(Boolean)`（红：`true` 初产 `Union([false,true])`——boolean 是 `false\|true` union 单例无 `BOOLEAN` flag → 加 `ty==boolean_type()` 判定）| ✓ |
| `create_type_of_declaration_widens_const_literal`（`emit_resolver_test.rs`，C3）| 同上 | `const x = 1;` → `Keyword(Number)`（无条件 widen；字面 const 保初始化器逻辑在 transformer）| ✓ |
| `create_type_of_declaration_array_is_array_type_node`（`emit_resolver_test.rs`，C4）| `nodebuilderimpl.go:typeToTypeNode`（array reference 臂）| `interface Array<T>{}\nconst xs=[1,2];` → `Array(Keyword(Number))`（`number[]`）| ✓ |
| `create_type_of_declaration_object_literal_is_type_literal`（`emit_resolver_test.rs`，C5）| `createAnonymousTypeNode` | `const o={a:1};` → `TypeLiteral([a: Number])`（属性 widened）| ✓ |
| `create_return_type_of_signature_declaration_infers_from_body`（`emit_resolver_test.rs`，C6，genuine red→green）| `CreateReturnTypeOfSignatureDeclaration`→`getReturnTypeFromBody` | `function f(){return 1}` → `Keyword(Number)`（红：初产 `Any`——`function_like_body` 缺 FunctionDeclaration 臂 → 扩之 + `get_return_type_from_body` 升 `pub(crate)`）| ✓ |
| `is_literal_const_declaration_detects_fresh_primitive_const`（`emit_resolver_test.rs`，C7）| `emitresolver.go:IsLiteralConstDeclaration` | `const x=1` literal-const；`let n=1`/`const o={a:1}`/`const xs=[1]` 非（红：方法不存在）| ✓ |
| `create_literal_const_value_reachable_primitives`（`emit_resolver_test.rs`，C7）| `emitresolver.go:CreateLiteralConstValue` | `const x=1`→`Number{"1"}`、`const s="a"`→`String("a")`、`const b=true`→`Boolean(true)` | ✓ |
| `type_to_type_node_primitive_keywords`（`nodebuilder_test.rs`，C8）| `typeToTypeNode`（keyword 臂）| `string`/`number`/`boolean`/`any` → `Keyword(..)` | ✓ |
| `type_to_type_node_literal_types`（`nodebuilder_test.rs`，C8）| `typeToTypeNode`（literal 臂）| 字面 `"x"`→`StringLiteral`、`1`→`NumberLiteral` | ✓ |
| `type_to_type_node_array_reference`（`nodebuilder_test.rs`，C8）| `typeReferenceToTypeNode`（global Array）| `Array<number>` → `Array(Keyword(Number))` | ✓ |
| `type_to_type_node_type_reference_with_args`（`nodebuilder_test.rs`，C8）| `typeToTypeNode`（reference 臂）| `Box<string>` → `TypeReference{Box,[String]}` | ✓ |
| `type_to_type_node_union`（`nodebuilder_test.rs`，C8）| `typeToTypeNode`（union 臂）| `A \| B`（命名接口）→ `Union([Ref(A),Ref(B)])` | ✓ |
| `type_to_type_node_anonymous_object_type_literal`（`nodebuilder_test.rs`，C8）| `createAnonymousTypeNode` | 匿名 `{ value: string }` → `TypeLiteral([value: String])` | ✓ |
| `type_to_type_node_tuple`（`nodebuilder_test.rs`，C8）| `typeToTypeNode`（tuple 臂）| `[string, number]` → `Tuple{[String,Number],readonly=false}` | ✓ |

> 红→绿证据：C1/C4/C5/C7/C8 **genuine RED（方法/枚举不存在=编译错误）**；C2 **genuine RED**（boolean union 单例无 flag → `Union` 而非 `Boolean`，加单例判定转绿）；C6 **genuine RED**（`function_like_body` 缺 FunctionDeclaration → `Any`）。**既有测试更新（Go-verified）**：**无**（全新增方法/枚举；`function_like_body` 加法扩臂，唯一调用者 `get_return_type_from_body` 原仅收 arrow/fn-expr，新臂不改既有行为）；**无既有测试弱化/删除**（C-E 720 全绿不变）。**测试增量**：**720→735 单测**（+15：emit_resolver_test +8 / nodebuilder_test +7）+ **169→177 doctest**（+8：`type_to_type_node`/`SynthesizedTypeNode`/`SynthesizedProperty`/`LiteralConstValue` + 4 个 EmitResolver 方法）。gate：`cargo test -p tsgo_checker`（735 lib + 177 doc）绿、clippy `-D warnings` 干净、fmt 干净、`cargo build -p tsgo_compiler` + `-p tsgo_transformers` 绿。**与 Go `.d.ts` 实测一致（byte-for-byte，经 transformers 端到端）**：`let n=1`→`declare let n: number;`、`const x=1`→`declare const x = 1;`、`function f(){return 1}`→`declare function f(): number;`、`class C{x=1}`→`x: number`、`const xs=[1,2]`→`number[]`、`const o={a:1}`→`{ a: number; }`。**DEFER（带 blocked-by）**：异域 type-node（type param/function·constructor 类型/`keyof`·conditional·mapped·template·string-mapping/import-type/bigint 字面/enum 点号；`type_to_type_node`→`None`→`any`）、`SymbolTracker` 可访问性报告（D-F3）、accessor write-type/`requiresAddingImplicitUndefined`/注解复用/mapper、`isDeclarationReadonly` literal-const 臂。详见 impl.md「D-F2 落地记录」。
