...
### §8.6 补充单测（Go 无对应 `func Test*`，按"每函数一测"补）

| Rust 测试 | 验证内容 | input → expected | Go 对照 | 完成 |
|---|---|---|---|---|
| `has_directory_reports_presence` | `has_directory` 会对 key 补尾随分隔符再查 | 不带分隔符查未登记 → `false`；`set_directory` 后再以不带分隔符的同路径查 → `true` | `knownsymlinks.go:HasDirectory`（Go 无直测） | ✓ |
| `set_file_realpath_mapping` | `set_file` 同时建立 `files_by_realpath` 反查 | `set_file(symlink, path, "/real/path/file.ts")` → `files_by_realpath[to_path(realpath)]` 含 `symlink` | `knownsymlinks.go:SetFile`（Go 的 `TestSetFile` 未断言反查） | ✓ |

## 与 impl.md 的对齐核对

- [x] 每个 Go `func Test*`（8 个）都已映射
- [x] 每个表驱动子用例都已逐行列出（`guessDirectorySymlink` 5 + `isNodeModules...` 6）
- [x] expected 值均取自 Go 测试字面量
- [x] 每条带 `// Go:` 锚点
- [x] 与 impl.md 双向对齐：每个公开方法都有用例承载
