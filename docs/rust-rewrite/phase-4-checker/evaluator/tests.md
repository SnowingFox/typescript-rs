...
| `eval_no_substitution_template` | 无插值模板串折叠 | `` `abc` `` → `Str("abc")`, `is_syntactically_string=true` | evaluator.go:`KindNoSubstitutionTemplateLiteral` | |
...
| `eval_shift_right` | 有符号右移 | `-8 >> 1` → `Num(-4)` | evaluator.go:`KindGreaterThanGreaterThanToken` | |
| `eval_ushift_right` | 无符号右移 | `-1 >>> 0` → `Num(4294967295)` | evaluator.go:`KindGreaterThanGreaterThanGreaterThanToken` | |
...
| `eval_template_with_number_span` | 模板插值数字 | `` `a${1}b` `` → `Str("a1b")`, `is_syntactically_string=true` | evaluator.go:`evaluateTemplateExpression` | |
| `eval_template_span_none_short_circuits` | span 求值为 None 时短路 | `` `a${x}b` ``（stub entity → None）→ `Result{ None, is_syntactically_string=true }` | evaluator.go:`if spanResult.Value == nil` | |
...
| `eval_propagates_resolved_flags` | `resolved_other_files`/`has_external_references` 从 entity 冒泡 | stub 返回带 `resolved_other_files=true` → 一元/二元结果保留该标志 | evaluator.go 各分支对两 bool 的赋值 | |
...
- [ ] 每条行为级用例带 `// Go:` 锚（「依据」列即上游锚，指向实现源 `internal/evaluator/evaluator.go:<Func>`，因 Go 侧无 `*_test.go`）

- [ ] 每个 Go `func Test*` 都已映射 —— **N/A**（Go 侧 0 单测）
- [ ] 每个表驱动子用例都已逐行列出 —— N/A
- [ ] expected 值均取自 Go 实现语义 / TS spec（非 Rust 推断）
- [ ] 每条对应到 impl.md 的一个实现 TODO（折叠分支 / `any_to_string` / `is_truthy`）
- [ ] 与 impl.md 双向对齐无遗漏：impl.md 的每个折叠分支在此都有至少一条用例
