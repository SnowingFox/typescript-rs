...
| NameGenerator（temp/loop/unique/private + scope） | `namegenerator.rs` (+test) | ✓ | `namegenerator_test.go` 36 中的 16 条 |
...
- **node-based 生成名字**（`generateNameForNode`/`isUniqueLocalName`）：`namegenerator.rs` 内 `todo!()`。`tsgo_binder` 现已 GREEN，可作 dev-dep 接线；对应 `namegenerator_test.go` 另外 20 条。
...
- **NameGenerator 的 node-based 名字（`generateNameForNode`/`isUniqueLocalName`）**：`namegenerator.rs` 内 `todo!()` + `// DEFER(phase-4)` + `// blocked-by:`。阻塞：需 binder 的 `Locals`/`NextContainer`、跨 arena 的外来节点访问、`node.Name()/Body()/Members()/Expression()` 等访问器与 `GetExternalModuleName`。对应 `namegenerator_test.go` 的另外 20 条。
...
- [x] `pub fn generate_name(&mut self, name: NodeId) -> String`（auto/loop/unique 分派已实现；**node 分派 `todo!()` DEFER**）　`// Go: namegenerator.go:GenerateName`
- [x] temp/loop（`_a/_b`、`_i`）、formatted name（prefix/suffix）、reserved-in-nested-scopes、unique（`foo_1`）、private（`#foo_1`）已实现；**node-based（`generateNameForNode`/`isUniqueLocalName`）DEFER(phase-4) — blocked-by binder Locals + 跨 arena**　`// Go: namegenerator.go:generateName*/makeName/...`
...
- [x] 名字工厂：`new_temp_variable(_ex)/new_loop_variable(_ex)/new_unique_name(_ex)/new_unique_private_name(_ex)`　已实现；`NewGeneratedNameForNode(Ex)/NewGeneratedPrivateNameForNode(Ex)` **DEFER**（node-based）　`// Go: factory.go:New*Variable/New*Name*`
