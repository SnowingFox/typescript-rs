...
## Round P6-7 worklog（source-map emission：printer 驱动 `sourcemap::Generator`）

> 这是 round-2 注记里"注释/源映射 emit = no-op 路径"中**源映射的那一半**落地（注释发射仍 no-op）。printer 现在能在 emit 时驱动 `tsgo_sourcemap::Generator`，逐节点记录 generated→source 映射；emitter（P6-7 compiler 侧）据此组装 `.js.map` + `//# sourceMappingURL=` 注释。**纯 additive**：既有 `emit_source_file`（plain 路径）签名/行为不变，新增 `emit_source_file_with_source_map` 入口；`enter_node`/`exit_node` 在无 generator 时仍是 true no-op（`source_map_generator.is_none()` 早退），故所有非-source-map 测试逐字节不变。

### Go ground truth（先抓后断言；`cmd/tsgo --sourceMap --module esnext`）

Go 全管线对 `const x = 1;` 会经 `estransforms/usestrict.go` 前置 `"use strict";`（module 管线，Rust emit 可达子集 = 仅 type eraser，**不**跑 module transform，故无 `"use strict";`）。因此 Rust 子集的 mappings = Go mappings **去掉前导 `;`**（缺失 prologue 的 generated-line 偏移）：

| 输入 | Go `.map` mappings（带 use strict） | Rust 子集 mappings（无 use strict） |
|---|---|---|
| `0` | `;AAAA,CAAC,CAAA` | `AAAA,CAAC,CAAA` |
| `const x = 1;` | `;AAAA,MAAM,CAAC,GAAG,CAAC,CAAC` | `AAAA,MAAM,CAAC,GAAG,CAAC,CAAC` |
| `const x: number = 1;` | `;AAAA,MAAM,CAAC,GAAW,CAAC,CAAC` | `AAAA,MAAM,CAAC,GAAW,CAAC,CAAC` |
| `const x = 1;\nconst y = 2;` | `;AAAA,MAAM,CAAC,GAAG,CAAC,CAAC;AACZ,MAAM,CAAC,GAAG,CAAC,CAAC` | `AAAA,MAAM,CAAC,GAAG,CAAC,CAAC;AACZ,MAAM,CAAC,GAAG,CAAC,CAAC` |

`.map` JSON 字节序（key 顺序）：`{"version":3,"file":"...","sourceRoot":"","sources":[...],"names":[],"mappings":"..."}`（与 `RawSourceMap` serde 已对齐；`sourcesContent` 在空时省略）。**节点级 mappings 与 Go 逐段一致**（Rust `enter_node`/`exit_node` 1:1 镜像 Go `emitSourceMapsBeforeNode`/`AfterNode`），仅 use-strict 行偏移不同——slice 1/4 实测全字符串相等，证明 printer 节点级覆盖与 Go 一致。

### red→green 逐切片

**Slice 1 — tracer（`source_map_records_first_mapping`）**：先写测试断言 `const x = 1;` 经 `emit_source_file_with_source_map` 产出 `text == "const x = 1;\n"` + 首段解码 gen(0,0)→src(0,0) + VLQ 全串 `AAAA,MAAM,CAAC,GAAG,CAAC,CAAC` → 跑 → **RED**（`emit_source_file_with_source_map`/`PrinterOptions.inline_sources` 不存在，编译失败）。实现（见下"增长"）→ **GREEN**，VLQ 串与 Go 实测一致。

**Slice 4 — 多语句 + inline-sources（`source_map_multi_statement_lines` / `source_map_inline_sources_embeds_content`）**：2 语句 → 解码各 generated 行映射回正确 source 行（line0→src0、line1→src1，各自 col 0 起），VLQ 全串 `AAAA,...;AACZ,...` 与 Go 实测一致；`inline_sources=true` → `sourcesContent == [Some(原文)]`（绿）。

### upstream（printer）增长（P6-7，全 additive）

- **`PrinterOptions`**：+ `inline_sources: bool`（喂 `setSourceMapSource` 的 `SetSourceContent`）。既有字段不变。
- **`Printer` 新字段**（仅 source-map 路径活跃；plain 路径 `source_maps_disabled=true` + generator `None`）：`source_map_generator: Option<Generator>`（move-in/out）、`source_maps_disabled`、`has_source_map_source`、`source_map_source_index`、`source_map_source_is_json`、`source_map_state_stack: Vec<Option<SourceMapState>>`（镜像 Go `printerState.sourceMapState`，enter push / exit pop，LIFO 配对）、行/列增量缓存 `sm_cached_*`（镜像 `lineCharacterCache`）。
- **新方法**（`// Go:` 锚到 printer.go / utilities.go）：`emit_source_file_with_source_map`（= Go `Write`，move generator in→out + 返回 `(text, Generator)`）、`set_source_map_source`、`emit_pos`、`source_line_and_character`（= `lineCharacterCache.getLineAndCharacter`，增量 UTF-16 计数）、`should_emit_source_maps`、`emit_source_maps_before_node`/`emit_source_maps_after_node`、`source_map_range`（= `EmitContext.SourceMapRange`，新树落到 `arena.loc`）。
- **`enter_node`/`exit_node`**：从 no-op 改为：generator 存在时调用 before/after 并维护状态栈；**generator `None`（plain 路径）时仍立即返回**（true no-op，零开销，零行为变化）。注释发射仍 no-op。
- **借用方案（偏离 Go 指针）**：Go printer 持 `*sourcemap.Generator`（外部传入）；Rust 用 `Option<Generator>` 字段 move-in/out（安全、无 `unsafe`、无生命周期纠缠）。`source_map_range` 新树落 `arena.loc`（`SetSourceMapRange` 旁路表为空——transformers 才会写，DEFER）。
- **复用 `tsgo_sourcemap`**：`Generator::{new,add_source,set_source_content,add_source_mapping,raw_source_map,to_string,base64_data_url,sources}`，无需新增 reachable 方法。复用 `tsgo_tspath::{file_extension_is,EXTENSION_JSON}`、`tsgo_scanner::compute_line_of_position`、`tsgo_core::utf16_len`。Cargo.toml 无改动（deps 既有）。
- **未触碰** `internal/ast/*`/`internal/binder/*`/`internal/checker/*`/`internal/sourcemap/*`。

### DEFER（blocked-by）

| 行为 | blocked-by | 目标 |
|---|---|---|
| token-level source maps（brace `{`/`}` 位置，`emitSourceMapsBeforeToken`/`shouldEmitTokenSourceMaps`/`OmitBraceSourceMapPositions`） | printer `emit_token` 未走 enter/exit token 路径（注释/token-map 同属 deferred token 面） | 后续轮（与注释发射同期） |
| names-mapping（`AddNamedSourceMapping`/`emitPosName`） | Go 自身 `emitPosName` 即注释掉（"TODO: Support emitting nameIndex"）——Rust 同步保留 DEFER | 跟随 Go |
| `EFNoLeadingSourceMap`/`NoTrailing`/`NoNestedSourceMaps` 的真实触发 | 这些 flag 仅由 transformers 设置；reachable 子集（新树）从不置位，故路径已实现但未被触发 | transformers 链成熟后 |
| 注释发射（`enterNode` 的另一半） | `getLeadingCommentRanges` 经 scanner 本地接线（round-2 起即 deferred） | 注释 round |
