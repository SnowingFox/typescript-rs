...
- [x] 每个 Go `func Test*`（30 个）都已逐一映射
- [x] mapping 正例的 `mappings` 串逐字对齐（`"A"`/`";A"`/`"AAAA"`/`"CAAC"`/`"CAAD"`/`"AAAAC,CAAAD"` 等）
- [x] 错误例的错误消息逐字对齐
- [x] `Empty_Serialized` / `ForSecondSourceOnly_Serialized` 的 JSON 整串逐字对齐
- [x] expected 值均取自 Go 测试字面量（非 Rust 推断）
- [x] 每条带 `// Go:` 锚点
- [x] decoder/mapper/util 补测与 impl.md TODO 对齐
