...
| `tstransforms::legacydecorators::tests::class_typed_property_serializes_to_entity_identifier` | tstransforms | **6an headline**（genuine RED→GREEN，**消费 checker 4aw**）：class-typed `TypeReference` `design:type` → 类标识符本身（`TypeWithConstructSignatureAndValue` → `serializeEntityNameAsExpression`）| `class C {}\nclass D { @dec x: C; }`（+emitDecoratorMetadata）→ `…__metadata("design:type", C)], D.prototype, "x", void 0);` | `serializeTypeReferenceNode`（TypeWithConstructSignatureAndValue）/`get_type_reference_serialization_kind` | ✓ |
| `tstransforms::legacydecorators::tests::interface_typed_property_serializes_to_object` | tstransforms | **6an**（green-on-arrival）：type-only `TypeReference` `: I` → `Object`（`ObjectType` 臂；不误发自身标识符）| `interface I {}\nclass D { @dec x: I; }` → `…__metadata("design:type", Object)…`（`interface I {}` 隔离运行透传）| `serializeTypeReferenceNode`（ObjectType）| ✓ |
| `tstransforms::legacydecorators::tests::unresolved_type_reference_property_serializes_to_object` | tstransforms | **6an**（green-on-arrival）：未解析名 `: Missing` → `Object`（`Unknown` 臂；Go `typeof`-条件守卫 DEFER，落 `Object` 尾）| `class D { @dec x: Missing; }` → `…__metadata("design:type", Object)…` | `serializeTypeReferenceNode`（Unknown 可达尾）| ✓ |
...
| `tstransforms::legacydecorators::tests::class_decorator_lowers_to_let_wrap_and_decorate` | tstransforms | **6ar tracer**（genuine RED→GREEN，**替** 6al DEFER 守卫）：class 装饰器 `@dec class C {}` → `let C = class C {…}` 绑定 + 尾随 `C = __decorate([dec], C);`；剥 class 装饰器、保留 `class C` 名；无自引用 alias | `@dec class C {}` → `<__decorate>\nlet C = class C {\n};\nC = __decorate([dec], C);` | `transformClassDeclarationWithClassDecorators`/`getConstructorDecorationStatement`/`NewDecorateHelper`/`NewAssignmentExpression` | ✓ |
| `tstransforms::legacydecorators::tests::multiple_class_decorators_preserve_source_order` | tstransforms | **6ar**（green-on-arrival，守卫顺序）：多 class 装饰器按源序入数组（`@a @b` → `[a, b]`）| `@a @b class C {}` → `…C = __decorate([a, b], C);` | `getAllDecoratorsOfClass`/`transformDecorators`（源序）| ✓ |
| `tstransforms::legacydecorators::tests::constructor_parameter_decorator_decorates_class_constructor` | tstransforms | **6ar slice3 headline**（genuine RED→GREEN）：装饰的**构造函数参数**使 class 走包裹路径（无 class 装饰器也是）；`__param(0, pdec)` 进 **CLASS** `__decorate` 数组（目标 `C`，非成员）；构造函数参数装饰器/类型从体剥离（`constructor(a) { }`）| `class C { constructor(@pdec a: number) {} }` → `<__decorate>\n<__param>\nlet C = class C {\n    constructor(a) { }\n};\nC = __decorate([__param(0, pdec)], C);` | `ClassOrConstructorParameterIsDecorated`/`getAllDecoratorsOfClass`（`getDecoratorsOfParameters(ctor)`）/`transformDecoratorsOfParameters`/`visitConstructorDeclaration` | ✓ |
| `tstransforms::legacydecorators::tests::class_decorator_with_decorated_method_emits_member_then_class_decorate` | tstransforms | **6ar slice4a**（green-on-arrival，守卫语句序）：class 装饰器 + 装饰**方法** → 成员 `__decorate`（`C.prototype`）**先于** class 级 `C = __decorate([dec], C)` | `@dec class C { @mdec m() {} }` → `…let C = class C {\n    m() { }\n};\n__decorate([mdec], C.prototype, "m", null);\nC = __decorate([dec], C);` | `transformClassDeclarationWithClassDecorators`（`[var] ++ memberDecorations ++ constructorDecoration`）| ✓ |
| `tstransforms::legacydecorators::tests::class_constructor_paramtypes_metadata_under_emit_decorator_metadata` | tstransforms | **6ar slice4b headline**（genuine RED→GREEN，**消费既有 checker API**）：`--emitDecoratorMetadata` 下装饰类（有构造体）追加 `design:paramtypes`（构造函数参类型经 checker，`: number`→`Number`），class **仅**发 `design:paramtypes`（无 `design:type`/`design:returntype`，成员专属）；构造体参类型剥离 | `@dec class C { constructor(a: number) {} }`（+meta）→ `…C = __decorate([dec, __metadata("design:paramtypes", [Number])], C);` | `shouldAddParamTypesMetadata`（class→`GetFirstConstructorWithBody`）/`getOldTypeMetadata`/`serializeParameterTypesOfNode` | ✓ |
| `tstransforms::legacydecorators::tests::class_decorator_without_constructor_emits_no_paramtypes_metadata` | tstransforms | **6ar slice4b**（green-on-arrival，守卫构造条件）：装饰类无构造函数 → 即便 `--emitDecoratorMetadata` 也不发 `design:paramtypes`（`GetFirstConstructorWithBody==nil` 门）| `@dec class C {}`（+meta）→ `…C = __decorate([dec], C);` | `shouldAddParamTypesMetadata`（`GetFirstConstructorWithBody`）| ✓ |
| `tstransforms::legacydecorators::tests::constructor_parameter_decorator_with_metadata_orders_param_before_paramtypes` | tstransforms | **6ar slice4b** coverage（green-on-arrival，守卫顺序）：构造参装饰器 + metadata → `__param(0, pdec)` **先于** `design:paramtypes [Number]`；三 helper 按 priority(2/3/4) prologue | `class C { constructor(@pdec a: number) {} }`（+meta）→ `…C = __decorate([__param(0, pdec), __metadata("design:paramtypes", [Number])], C);` | `transformAllDecoratorsOfDeclaration`（decorators ++ __param ++ metadata）| ✓ |
| `tstransforms::legacydecorators::tests::array_typed_property_serializes_to_array_constructor` | tstransforms | **6am**（genuine RED→GREEN，**协调 checker 4av**）：数组类型属性 `design:type` → 全局 `Array`（`ArrayType`→`SerializedTypeNode::Array`，经 `serialized_type_to_expression`）| `class C { @dec x: number[]; }`（+emitDecoratorMetadata）→ `…__decorate([dec, __metadata("design:type", Array)], C.prototype, "x", void 0);` | `serializeTypeNode`（KindArrayType）/`serialized_type_to_expression` | ✓ |
| `tstransforms::legacydecorators::tests::tuple_typed_property_serializes_to_array_constructor` | tstransforms | **6am**（genuine RED→GREEN）：元组类型属性 `design:type` → 全局 `Array`（`TupleType` 与 `ArrayType` 同组）| `class C { @dec x: [number, string]; }` → `…__metadata("design:type", Array)…` | `serializeTypeNode`（KindTupleType）| ✓ |
| `tstransforms::legacydecorators::tests::function_typed_property_serializes_to_function_constructor` | tstransforms | **6am**（genuine RED→GREEN，**协调 checker 4av**）：函数类型属性 `design:type` → 全局 `Function`（`FunctionType`→`SerializedTypeNode::Function`）| `class C { @dec x: () => void; }` → `…__decorate([dec, __metadata("design:type", Function)], C.prototype, "x", void 0);` | `serializeTypeNode`（KindFunctionType）/`serialized_type_to_expression` | ✓ |
| `tstransforms::legacydecorators::tests::instance_method_decorator_lowers_to_decorate_call` | tstransforms | **6ao tracer bullet**（genuine RED→GREEN）：实例**方法**装饰器 → 尾随 `__decorate([dec], C.prototype, "m", null)`，第 4 参 `null`（非属性 `void 0`），方法剥装饰器/返回类型；无 metadata | `class C { @dec m() {} }` → `class C {\n    m() { }\n}\n__decorate([dec], C.prototype, "m", null);` | `visitMethodDeclaration`/`generateClassElementDecorationExpression`（descriptor `KindNullKeyword`）| ✓ |
| `tstransforms::legacydecorators::tests::static_method_decorator_uses_class_name_prefix` | tstransforms | **6ao**（green-on-arrival）：static 方法前缀 `C`（非 `C.prototype`），保留 `static`（slice 1 把 `is_static_member` 泛化到方法；前缀差异证非硬编码）| `class C { @dec static m() {} }` → `…\n__decorate([dec], C, "m", null);` | `getClassMemberPrefix`（`IsStatic`）| ✓ |
| `tstransforms::legacydecorators::tests::method_decorator_emits_design_type_function_and_void_returntype` | tstransforms | **6ao headline + 6ap slice2**（genuine RED→GREEN；6ap 更新含 `design:paramtypes`）：`--emitDecoratorMetadata` 下方法发 `design:type=Function`（硬编码）+ `design:paramtypes`（6ap，0 参→`[]`）+ `design:returntype`（恒发，无返回注解→`void 0`），metadata 居末 | `class C { @dec m() {} }`（+meta）→ `…__decorate([dec, __metadata("design:type", Function), __metadata("design:paramtypes", []), __metadata("design:returntype", void 0)], C.prototype, "m", null);` | `getOldTypeMetadata`/`serializeTypeOfNode`（KindMethodDeclaration→Function）/`serializeParameterTypesOfNode`/`shouldAddReturnTypeMetadata` | ✓ |
| `tstransforms::legacydecorators::tests::method_decorator_serializes_return_type_annotation` | tstransforms | **6ao headline + 6ap**（genuine RED→GREEN，**消费既有 checker API**；6ap 更新含 `design:paramtypes []`）：方法 `design:returntype` 序列化返回注解（`: number` → `Number`，经 `serialize_type_node_for_metadata`，非 `void 0` 回退）；0 参→`design:paramtypes []` | `class C { @dec m(): number { return 1; } }`（+meta）→ `…__metadata("design:type", Function), __metadata("design:paramtypes", []), __metadata("design:returntype", Number)…` | `serializeReturnTypeOfNode`（`node.Type()` → `serializeTypeNode`）| ✓ |
| `tstransforms::legacydecorators::tests::method_decorator_emits_design_paramtypes_for_typed_params` | tstransforms | **6ap slice1**（genuine RED→GREEN，**消费既有 checker API**）：方法 `design:paramtypes` 数组逐参序列化（`: number`→`Number`、`: string`→`String`），插于 `design:type` 与 `design:returntype` 之间；参数类型注解从方法体剥离（`m(a: number, b: string)`→`m(a, b)`）| `class C { @dec m(a: number, b: string) {} }`（+meta）→ `…__decorate([dec, __metadata("design:type", Function), __metadata("design:paramtypes", [Number, String]), __metadata("design:returntype", void 0)], C.prototype, "m", null);` | `serializeParameterTypesOfNode`/`serializeTypeOfNode(parameter)`/`visitParamerDeclaration` | ✓ |
| `tstransforms::legacydecorators::tests::parameter_decorator_lowers_to_param_helper` | tstransforms | **6ap slice3 tracer**（genuine RED→GREEN）：参数装饰器 → `__param(0, pdec)` 进装饰器数组（方法本身无装饰器但 `NodeOrChildIsDecorated` 真）；注册 `PARAM_HELPER`(priority 4)；参数装饰器从方法体剥离（`m(@pdec a)`→`m(a)`）；无 metadata | `class C { m(@pdec a) {} }` → `<__decorate>\n<__param>\nclass C {\n    m(a) { }\n}\n__decorate([__param(0, pdec)], C.prototype, "m", null);` | `transformDecoratorsOfParameters`/`NewParamHelper`/`getDecoratorsOfParameters`/`ChildIsDecorated` | ✓ |
| `tstransforms::legacydecorators::tests::method_and_parameter_decorator_order_preserved` | tstransforms | **6ap slice4**（green-on-arrival，守卫顺序）：方法装饰器 + 参数装饰器 → `[dec, __param(0, pdec)]`（成员装饰器先于 `__param`，Go `transformAllDecoratorsOfDeclaration` 序）| `class C { @dec m(@pdec a) {} }` → `…__decorate([dec, __param(0, pdec)], C.prototype, "m", null);` | `transformAllDecoratorsOfDeclaration`（decorators ++ __param ++ metadata）| ✓ |
| `tstransforms::legacydecorators::tests::parameter_decorator_with_metadata_emits_param_and_object_paramtype` | tstransforms | **6ap** coverage（green-on-arrival）：`__param` + metadata 组合，无注解参数 → `design:paramtypes [Object]`（Go `serializeTypeNode(nil)`）；`__param` 先于 metadata；三 helper 按 priority(2/3/4) prologue | `class C { m(@pdec a) {} }`（+meta）→ `…__decorate([__param(0, pdec), __metadata("design:type", Function), __metadata("design:paramtypes", [Object]), __metadata("design:returntype", void 0)], C.prototype, "m", null);` | `transformAllDecoratorsOfDeclaration`/`serializeParameterTypesOfNode`（nil→Object）| ✓ |
| `tstransforms::legacydecorators::tests::instance_get_accessor_decorator_lowers_to_decorate_call` | tstransforms | **6aq tracer**（genuine RED→GREEN）：实例 **get 访问器**装饰器 → 尾随 `__decorate([dec], C.prototype, "x", null)`，第 4 参 `null`（同方法，非属性 `void 0`）；访问器剥装饰器、getter 体保留；无 metadata | `class C { @dec get x() { return 1; } }` → `class C {\n    get x() { return 1; }\n}\n__decorate([dec], C.prototype, "x", null);` | `visitGetAccessorDeclaration`/`generateClassElementDecorationExpression`（descriptor `KindNullKeyword`）| ✓ |
| `tstransforms::legacydecorators::tests::instance_set_accessor_decorator_lowers_to_decorate_call` | tstransforms | **6aq**（green-on-arrival）：对称 **set 访问器**装饰器 → `__decorate([dec], C.prototype, "x", null)`，值参重建（剥类型）| `class C { @dec set x(v) {} }` → `class C {\n    set x(v) { }\n}\n__decorate([dec], C.prototype, "x", null);` | `visitSetAccessorDeclaration` | ✓ |
| `tstransforms::legacydecorators::tests::get_accessor_decorator_emits_design_type_from_return_and_empty_paramtypes` | tstransforms | **6aq headline**（genuine RED→GREEN）：`--emitDecoratorMetadata` 下 getter-only 访问器发 `design:type`（来自 getter 返回类型 `getAccessorTypeNode`，`: number`→`Number`）**且** `design:paramtypes []`（`shouldAddParamTypesMetadata` 对访问器亦真；无 setter→空参），**无** `design:returntype`（method-only）。**纠正任务"ONLY design:type"提示**（对照 baseline decoratorOnClassAccessor8 class E）| `class C { @dec get x(): number { return 1; } }`（+meta）→ `…__decorate([dec, __metadata("design:type", Number), __metadata("design:paramtypes", [])], C.prototype, "x", null);` | `getOldTypeMetadata`/`shouldAddTypeMetadata`+`shouldAddParamTypesMetadata`/`getAccessorTypeNode` | ✓ |
| `tstransforms::legacydecorators::tests::set_accessor_decorator_emits_design_type_and_paramtypes_from_value_param` | tstransforms | **6aq**（green-on-arrival）：setter-only `design:type`=值参类型（`getSetAccessorTypeAnnotationNode`）+ `design:paramtypes`=自身参（`v: number`→`[Number]`）；体内值参剥类型（对照 baseline class F）| `class C { @dec set x(v: number) {} }`（+meta）→ `…__decorate([dec, __metadata("design:type", Number), __metadata("design:paramtypes", [Number])], C.prototype, "x", null);` | `getAccessorTypeNode`/`GetSetAccessorValueParameter`/`serializeParameterTypesOfNode` | ✓ |
| `tstransforms::legacydecorators::tests::get_set_pair_emits_single_decorate_owned_by_getter` | tstransforms | **6aq 合并 headline**（green-on-arrival）：get/set 对 → 两访问器留体内 + **单** `__decorate([dec], C.prototype, "x", null)`（getter 拥有，未装饰 setter 不发自己的）；无 metadata | `class C { @dec get x() { return 1; } set x(v) {} }` → `class C {\n    get x() { return 1; }\n    set x(v) { }\n}\n__decorate([dec], C.prototype, "x", null);` | `getAllDecoratorsOfAccessors`/`getAllAccessorDeclarations`（`firstAccessorWithDecorators`）| ✓ |
| `tstransforms::legacydecorators::tests::get_set_pair_metadata_crosses_to_setter_value_param` | tstransforms | **6aq 跨访问器 metadata**（green-on-arrival）：装饰的是 getter（无返回注解，单独会 `Object`），但 `design:type`=`Number`（setter 值参赢）+ `design:paramtypes [Number]`（借 setter 参）；setter 体值参剥类型（对照 baseline class A）| `class C { @dec get x() { return 0; } set x(value: number) {} }`（+meta）→ `…__decorate([dec, __metadata("design:type", Number), __metadata("design:paramtypes", [Number])], C.prototype, "x", null);` | `getAccessorTypeNode`（setter 优先）/`getParametersOfDecoratedDeclaration`（getter 借 setter 参）| ✓ |
| `tstransforms::legacydecorators::tests::get_set_pair_setter_decorated_second_owns_single_decorate` | tstransforms | **6aq 归属变体**（green-on-arrival）：setter 声明序第二且被装饰 → setter 拥有单 `__decorate`，未装饰 getter（第一）不发（对照 baseline class B）| `class C { get x() { return 0; } @dec set x(value: number) {} }`（+meta）→ `…__decorate([dec, __metadata("design:type", Number), __metadata("design:paramtypes", [Number])], C.prototype, "x", null);` | `getAllDecoratorsOfAccessors`（`firstAccessorWithDecorators` 取声明序首个**带装饰器**者）| ✓ |
| `tstransforms::legacydecorators::tests::static_get_accessor_decorator_uses_class_name_prefix` | tstransforms | **6aq**（green-on-arrival）：static get 访问器前缀 `C`（非 `C.prototype`），保留 `static`，第 4 参 `null` | `class C { @dec static get x() { return 1; } }` → `…\n__decorate([dec], C, "x", null);` | `getClassMemberPrefix`（`IsStatic`）| ✓ |
| `tstransforms::legacydecorators::tests::static_get_accessor_decorator_with_metadata_uses_class_name_prefix` | tstransforms | **6aq** coverage（green-on-arrival）：static + metadata 正交——前缀 `C` + `design:type Number`（返回类型）+ `design:paramtypes []` | `class C { @dec static get x(): number { return 1; } }`（+meta）→ `…__decorate([dec, __metadata("design:type", Number), __metadata("design:paramtypes", [])], C, "x", null);` | `getClassMemberPrefix`/`getOldTypeMetadata`（accessor 臂）| ✓ |
...
| legacydecorators + metadata | `tests/cases/conformance/decorators/**`（`--experimentalDecorators --emitDecoratorMetadata`） | `__decorate`/`__metadata`/`__param` helper 注入与设计类型序列化 | **6al+6am+6an+6ao+6ap+6aq ✓ 子集**（属性装饰器 instance/static → `__decorate([dec], C.prototype/C, "x", void 0)`；**6ao 方法装饰器** → `__decorate([dec], C.prototype/C, "m", null)`（第 4 参 `null`）+ `design:type=Function`（硬编码）+ `design:returntype`（恒发，注解经 checker/否则 `void 0`）；**6ap** `design:paramtypes` 数组（逐参经 checker 序列化、无注解→`Object`、0 参→`[]`，插于 type 与 returntype 之间）+ **参数装饰器 `__param(i, dec)`**（`PARAM_HELPER` priority 4；置于成员装饰器后、metadata 前）；**6aq 访问器（get/set）装饰器** → `__decorate([dec, …], C.prototype/C, "x", null)`（第 4 参 `null`）+ get/set 对合并为**单** `__decorate`（`getAllAccessorDeclarations`+`firstAccessorWithDecorators` 归属）+ `design:type`（`getAccessorTypeNode`：setter value 参/getter 返回类型）**且** `design:paramtypes`（`shouldAddParamTypesMetadata` 对访问器亦真；getter 借 setter 参/自身参/`[]`），**无** `design:returntype`；`design:type` 元数据消费 checker 4at/4av/4aw，keyword→`Number`/`String`/…、`Array`/`Function`、**`TypeReference` class→类标识符、interface/unresolved→`Object`**；`__decorate`/`__metadata`/`__param` helper prologue）/ DEFER（class 装饰器 `let C=…;C=__decorate(...)` 包裹、计算/私有访问器名、accessor 参数装饰器、构造函数参数装饰器、`this`-参数偏移/体擦除 + rest-参数 `GetRestParameterElementType`、async 方法/访问器 `Promise` 返回、计算名、`TypeReference` 的 lib-globals/qualified-name 类、装饰器求值序）+ P10 |
| typeserializer | （随 metadata）`decorators/**/metadata/**` | 类型 → `Object`/`Function`/`Number`… 元数据表达式 | **6al+6am+6an ✓ 子集**（keyword 类型经 checker 4at `SerializedTypeNode`→AST：`number→Number`/`string→String`/`boolean→Boolean`/`bigint→BigInt`/`symbol→Symbol`/`void·undefined·never·null→void 0`；`ArrayType`/`TupleType`→`Array`、`FunctionType`/`ConstructorType`→`Function`；**`TypeReference` 经 `serialize_type_reference_node` 消费 4aw `get_type_reference_serialization_kind`：class→类标识符、interface/type-alias/unresolved→`Object`**）/ DEFER（`TypeReference` 的 lib-globals 类（Promise/array/primitive，checker 落 `ObjectType`）、`Unknown` 的 `typeof`-条件守卫、qualified-name、union/intersection/conditional 递归）+ P10 |
