...
| `tstransforms::legacydecorators::tests::method_decorator_emits_design_type_function_and_void_returntype` | tstransforms | **6ao headline**（genuine RED→GREEN）：`--emitDecoratorMetadata` 下方法发 `design:type=Function`（硬编码，无 checker）+ `design:returntype`（恒发，无返回注解→`void 0`），metadata 居末 | `class C { @dec m() {} }`（+meta）→ `…__decorate([dec, __metadata("design:type", Function), __metadata("design:returntype", void 0)], C.prototype, "m", null);`（DEFER `design:paramtypes`）| `getOldTypeMetadata`/`serializeTypeOfNode`（KindMethodDeclaration→Function）/`shouldAddReturnTypeMetadata` | ✓ |
| `tstransforms::legacydecorators::tests::method_decorator_serializes_return_type_annotation` | tstransforms | **6ao headline**（genuine RED→GREEN，**消费既有 checker API**）：方法 `design:returntype` 序列化返回注解（`: number` → `Number`，经 `serialize_type_node_for_metadata`，非 `void 0` 回退）| `class C { @dec m(): number { return 1; } }`（+meta）→ `…__metadata("design:returntype", Number)…` | `serializeReturnTypeOfNode`（`node.Type()` → `serializeTypeNode`）| ✓ |
...
| legacydecorators + metadata | `tests/cases/conformance/decorators/**`（`--experimentalDecorators --emitDecoratorMetadata`） | `__decorate`/`__metadata` helper 注入与设计类型序列化 | **6al+6am+6an+6ao ✓ 子集**（属性装饰器 instance/static → `__decorate([dec], C.prototype/C, "x", void 0)`；**6ao 方法装饰器** → `__decorate([dec], C.prototype/C, "m", null)`（第 4 参 `null`）+ `design:type=Function`（硬编码）+ `design:returntype`（恒发，注解经 checker/否则 `void 0`）；`design:type` 元数据消费 checker 4at/4av/4aw，keyword→`Number`/`String`/…、`Array`/`Function`、**`TypeReference` class→类标识符、interface/unresolved→`Object`**；`__decorate`/`__metadata` helper prologue）/ DEFER（class 装饰器 `let C=…;C=__decorate(...)` 包裹、accessor 装饰器、参数装饰器 `__param`+`design:paramtypes`、async 方法 `Promise` 返回、计算名、`TypeReference` 的 lib-globals/qualified-name 类、装饰器求值序）+ P10 |
