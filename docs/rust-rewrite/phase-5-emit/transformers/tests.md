...
| legacydecorators + metadata | `tests/cases/conformance/decorators/**`（`--experimentalDecorators --emitDecoratorMetadata`） | `__decorate`/`__metadata` helper 注入与设计类型序列化 | **6al+6am+6an ✓ 子集**（属性装饰器 instance/static → `__decorate([dec], C.prototype/C, "x", void 0)`；`design:type` 元数据消费 checker 4at/4av/4aw，keyword→`Number`/`String`/…、`Array`/`Function`、**`TypeReference` class→类标识符、interface/unresolved→`Object`**；`__decorate`/`__metadata` helper prologue）/ DEFER（class 装饰器 `let C=…;C=__decorate(...)` 包裹、method/accessor 装饰器（`design:type=Function`/`design:returntype`）、参数装饰器 `__param`+`design:paramtypes`、计算名、`TypeReference` 的 lib-globals/qualified-name 类、装饰器求值序）+ P10 |
