...
| legacydecorators + metadata | `tests/cases/conformance/decorators/**`（`--experimentalDecorators --emitDecoratorMetadata`） | `__decorate`/`__metadata`/`__param` helper 注入与设计类型序列化 | **6al+6am+6an+6ao+6ap ✓ 子集**（属性装饰器 instance/static → `__decorate([dec], C.prototype/C, "x", void 0)`；**6ao 方法装饰器** → `__decorate([dec], C.prototype/C, "m", null)`（第 4 参 `null`）+ `design:type=Function`（硬编码）+ `design:returntype`（恒发，注解经 checker/否则 `void 0`）；**6ap** `design:paramtypes` 数组（逐参经 checker 序列化、无注解→`Object`、0 参→`[]`，插于 type 与 returntype 之间）+ **参数装饰器 `__param(i, dec)`**（`PARAM_HELPER` priority 4；置于成员装饰器后、metadata 前）；`design:type` 元数据消费 checker 4at/4av/4aw，keyword→`Number`/`String`/…、`Array`/`Function`、**`TypeReference` class→类标识符、interface/unresolved→`Object`**；`__decorate`/`__metadata`/`__param` helper prologue）/ DEFER（class 装饰器 `let C=…;C=__decorate(...)` 包裹、accessor 装饰器、构造函数参数装饰器、`this`-参数偏移 + rest-参数 `GetRestParameterElementType`、async 方法 `Promise` 返回、计算名、`TypeReference` 的 lib-globals/qualified-name 类、装饰器求值序）+ P10 |
