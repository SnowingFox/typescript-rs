...
| `declarations/transform.go` | `declarations/transform.rs`（子 module `declarations/mod.rs` 聚合）| ✅ **D-F1 CORE** + **D-F2** 推断 + **D-F3** 可见性/可访问性：非导出 elision（module/script 检测）、import/export `.d.ts`、`SymbolTracker` 4025、isolatedDeclarations 9007/9008、empty-exports marker；DEFER `DeclarationEmitHost`/`OutputPaths`（host 抽象，P6） |
| `declarations/tracker.go` | `declarations/tracker.rs` | ✅ **D-F3** `SymbolTracker`（共享诊断 sink，Go `SymbolTrackerImpl`+shared state）+ `create_diagnostic_for_node`；DEFER node-builder 回调（`ReportInaccessibleThis*`/`TrackSymbol`/...）blocked-by pseudo-type node builder |
| `declarations/diagnostics.go` | `declarations/diagnostics.rs` | ✅ **D-F3** 诊断消息选择表可达子集：`get_symbol_accessibility_diagnostic_message`（变量→4025）、`get_error_by_declaration_kind`（fn→9007/method→9008）、`get_related_suggestion_by_declaration_kind`（9031/9034）；DEFER property/method/accessor/param/type-alias 臂 + from-module 4023/4024 |
| `declarations/util.go` | `declarations/util.rs` | ✅ **D-F1** 修饰符 flag / 结构谓词 + **D-F3** `is_external_module`/`get_binding_name_visible`/`is_declaration_and_not_visible`/`is_external_module_indicator`/`needs_scope_marker`/`is_scope_marker` 可达子集 |
...
- [x] **D-F3 可见性/可访问性 + import/export + 诊断**：
  - **module/script 检测 + 非导出 elision**：checker 侧 `determine_if_declaration_is_visible` 用真实 `is_global_source_file`（`external_module_indicator`）替换原 "approximate false"（de-defer 脚本案）；transformer 侧 `transform_top_level_statement` 经 `is_declaration_and_not_visible` 省略 module 内非导出顶层声明（函数/类/接口/类型别名/枚举），`transform_variable_statement` 经 `get_binding_name_visible` 省略非导出变量；**仅当 resolver 接入时** elision 生效（无 resolver 保 D-F1/D-F2 行为）　`// Go: transform.go:transformTopLevelDeclaration/transformVariableStatement + util.go:isDeclarationAndNotVisible/getBindingNameVisible + emitresolver.go:determineIfDeclarationIsVisible`
  - **import/export `.d.ts`**：`transform_import_declaration`（命名 import 经 `EmitReferenceResolver::is_referenced` 过滤，被引用留、未引用 elide；default/namespace/side-effect import 处理）、`transform_export_declaration`（直通）、`transform_export_assignment`（identifier 直通，非 identifier `_default` 合成 DEFER）+ empty-exports marker（`create_empty_exports` → `export {};`，module 输出无 external-module indicator 时追加）+ `record_marker_flags`　`// Go: transform.go:transformImportDeclaration/visitDeclarationStatements/transformExportAssignment/transformSourceFile/createEmptyExports`
  - **`SymbolTracker` 4025**（`tracker.rs`）：`SymbolTracker`（共享 `Rc<RefCell<Vec<Diagnostic>>>` sink）+ `create_diagnostic_for_node` + `EmitReferenceResolver::entity_name_accessibility`（resolve_reference + is_declaration_visible 可达 `IsEntityNameVisible`）；`ensure_type` 注解臂经 `check_declaration_type_visibility`/`walk_type_entity_names` 检查 `typeof X` entity-name 可见性 → 不可见报 4025 "Exported variable 'b' has or is using private name 'a'."（`new_declarations_transformer_with_diagnostics` 暴露 sink）　`// Go: transform.go:checkEntityNameVisibility + tracker.go:handleSymbolAccessibilityError + diagnostics.go:getVariableDeclarationTypeVisibilityDiagnosticMessage`
  - **isolatedDeclarations 9007/9008**（`diagnostics.rs`）：`report_isolated_declarations_return_type`（无返回注解 + body 无 value-return → fn 9007 / method 9008，+ 9031/9034 related suggestion；可达子集 = void body，经 `body_has_value_return`/`function_body` 句法判定）　`// Go: tracker.go:ReportInferenceFallback + diagnostics.go:createReturnTypeError/getErrorByDeclarationKind/getRelatedSuggestionByDeclarationKind`
- [ ] **DEFER**：`DeclarationEmitHost`/`OutputPaths`（host 抽象，需 `compiler.Program` P6）；enum/namespace/`export =`·`export default` 非 identifier 合成/index signature/binding-pattern var·param-prop/JS·expando/declaration-map/triple-slash；**late-painted statements 完整图**（module 内被 emit 类型引用的非导出顶层声明：Go 经 late-painting 保留，可达子集 elide，DEFER）；**isolatedDeclarations 余下**（9013 表达式/9011 参数/9039 私名/条件·多 return → 9007 漏报；object/array/computed/expando/enum）blocked-by pseudo-type node builder；**4023/4024 from-module + property/method/accessor/param/type-alias 可访问性臂** blocked-by 跨模块 `SymbolAccessibilityResult` + `GetEffectiveDeclarationFlags`　`// Go: transform.go:NewDeclarationTransformer`
...
## D-F3 worklog（declarations 可见性/可访问性 + import/export + 诊断 — red→green 推进记录）

> **目标**：收口 P5 declarations 可达子集 —— module/script 检测 + 非导出 elision、import/export `.d.ts` 处理、`SymbolTracker` 私名族（4025）、`--isolatedDeclarations` 显式注解诊断（9007/9008）。**跨 crate**（checker 加法式 de-defer `IsGlobalSourceFile` 脚本案；transformers 主体）。

### Go ground truth（`/tmp/tsgo -p tsconfig{declaration,emitDeclarationOnly,strict[,isolatedDeclarations]}` 实测，逐 snippet，python `repr`）

| 输入 | `.d.ts` / 诊断输出 | 说明 |
|---|---|---|
| `export const a = 1;\nconst b = 2;` | `export declare const a = 1;\n` | **module 内非导出 `b` elided** |
| `const b = 2;` | `declare const b = 2;\n` | **script 保留**（全局） |
| `import { T } from "./m"; export const x: T = null as any;` | `import { T } from "./m";\nexport declare const x: T;\n` | import 被 emit 类型引用 → 保留 |
| `import { T } from "./m"; export const x = 1;` | `export declare const x = 1;\n` | import 未引用 → elided |
| `import { T, U } from "./m"; export const x: T = null as any;` | `import { T } from "./m";\nexport declare const x: T;\n` | 仅引用的 specifier 留 |
| `import { T } from "./m"; export { T };` | `import { T } from "./m";\nexport { T };\n` | re-export 保留 + 续命 import |
| `{ var a = ""; }\nexport let b: typeof a;` | `export declare let b: typeof a;\n` + `a.ts(4,22): error TS4025: Exported variable 'b' has or is using private name 'a'.` | block-scoped `a` 不可见 → 4025 |
| `export function noReturn() {}`（`--isolatedDeclarations`）| `export declare function noReturn(): void;\n` + `error TS9007: Function must have an explicit return type annotation with --isolatedDeclarations.`（+related 9031）| void body 无注解 → 9007 |
| `export class C { m() {} }`（`--isolatedDeclarations`）| `error TS9008: Method must have an explicit return type annotation with --isolatedDeclarations.` | method void body → 9008 |
| `export function f() { return 1; }`（`--isolatedDeclarations`）| `export declare function f(): number;\n`（无诊断）| 字面 return 句法可推 → 不报 |

> **关键发现 / 偏离纠正（FOLLOW GO）**：briefing 切片 4 写 `class P {} export const x = new P();` → 4025，但**真实 Go 经 late-painting 保留**非导出 `P`（`declare class P {}` + `export {};`），**不报 4025**。真实 4025 触发是 `typeof a`（block-scoped）这类**注解里的 entity-name 不可见**（`declarationEmitVarInElidedBlock` baseline）。briefing 切片 5 写 `export function f(){return 1}` → 9007，但**真实 Go 不报**（字面 return 句法可推）；真实 9007 触发是 **void body**（`noReturn(){}`）。两处均遵 "FOLLOW GO" 落实测行为并在 DEFER 标注差异。

### red→green 垂直切片（每片：测试 → 实测 → 最小实现）

1. **checker de-defer 脚本可见性**（前置）：`determine_if_declaration_is_visible` 非导出臂 `false` → `is_global_source_file(container)`（新 `is_global_source_file` 查 `external_module_indicator`）。RED：`nonexported_declaration_is_visible_in_a_script`（脚本 fn 应 visible 但返 false）。GREEN 后 module 案（`nonexported_declaration_is_not_visible_in_a_module`）仍 false。既有 `is_declaration_visible_*`/`exported_declaration_is_visible`（均 module）不变。
2. **module 非导出 elision**（tracer）：`export const a = 1;\nconst b = 2;` → 仅 `a`。RED：D-F2 emit 两者。最小绿：`transform_variable_statement` 经 `get_binding_name_visible` 过滤 + `transform_top_level_statement` 经 `is_declaration_and_not_visible`（仅 resolver 接入时）。
3. **script 保留全部**：`const b = 2;` → `declare const b = 2;`（脚本无 indicator → visible）。+ 非导出 fn module elide / script 多声明保留 各一片。
4. **import kept-iff-referenced**：`is_referenced` 过滤命名 import；被引用留、未引用 elide；仅引用 specifier 留；`export { T }` 续命 import。empty-exports marker（`record_marker_flags` + `create_empty_exports`）。
5. **4025 私名诊断**：`{var a=""} export let b: typeof a;` → 4025。RED：无诊断。GREEN：`tracker.rs`（sink）+ `diagnostics.rs`（消息表）+ `EmitReferenceResolver::entity_name_accessibility` + `check_declaration_type_visibility`/`walk_type_entity_names`（`typeof` entity-name 不可见 → 4025）。
6. **isolatedDeclarations 9007/9008**：`export function noReturn(){}` → 9007（+9031 related）；`export class C{m(){}}` → 9008。RED：无诊断。GREEN：`report_isolated_declarations_return_type`（无注解 + `!body_has_value_return` → `get_error_by_declaration_kind`）。注解/字面 return/无 isolatedDeclarations 各负向片。
7. **无回归**：D-F1/D-F2 全 35 declarations 切片 + 全 transformers 套绿（既有用例本就以 export/script 上下文写，elision 不改其期望，**零既有测试改动**）。

### 架构决策

- **elision 门控 resolver**：Go 真实 transformer 恒有 resolver+host；Rust 无 resolver 的 `check()` 路径是测试便利，故 elision/可见性/诊断**仅当 resolver 接入**生效，无 resolver 保 D-F1/D-F2 全量 emit（既有 `check()` 用例不变）。module/script marker 逻辑恒跑（仅需 `external_module_indicator`，不需 resolver）。
- **诊断 sink 桥**：`Transformer` 闭包捕获 state 不可外取，故 `new_declarations_transformer_with_diagnostics` 返回 `(Transformer, Rc<RefCell<Vec<Diagnostic>>>)`（加法构造器；旧 `new_declarations_transformer` 签名不变，内部建私有 sink）。诊断用 `tsgo_checker::Diagnostic`（Go `*ast.Diagnostic` 经 `checker.NewDiagnosticForNode`），`create_diagnostic_for_node` 镜像 checker `diagnostic_for_node`（`loc.pos()` 直接，不 skip-trivia——与 checker 端口一致）。
- **`is_referenced` 作 import 可见性 stand-in**：Go declaration emit 用 `IsDeclarationVisible(specifier)`（on-demand isVisible link，经 entity-name 标记设）；可达子集无 late-painting link 机制，改用 `is_referenced`（被引用即留），对测试用例 byte-for-byte 匹配 Go。
- **isolatedDeclarations void-body 句法判定**：Go 经 pseudo-type node builder `ReportInferenceFallback`（未移植）；可达子集用 `body_has_value_return`（句法扫描，停在嵌套函数边界）判 void → 9007/9008。**sound under-report**：条件/多 return（Go 报 9007）漏报，非字面单 return（Go 报 9013）/私名 return（9039）/参数（9011）均 DEFER，绝不误报。

### 交付（`internal/checker/**` + `internal/transformers/**` + docs）

- **checker**：`core/emit_resolver.rs` `determine_if_declaration_is_visible` 非导出臂改 `is_global_source_file(container)` + 新增 `is_global_source_file`（加法 private fn）；`is_declaration_visible` doc 更新（de-defer 脚本案）。**ZERO 既有 `pub fn` 签名变更**。
- **transformers**：新建 `declarations/tracker.rs`（`SymbolTracker` + `create_diagnostic_for_node`）+ `declarations/diagnostics.rs`（3 消息表）；`util.rs` +6 谓词（`is_external_module`/`get_binding_name_visible`/`is_declaration_and_not_visible`/`is_external_module_indicator`/`needs_scope_marker`/`is_scope_marker`）；`transform.rs` 加 elision/import/export/marker/4025/9007-8 逻辑 + 自由函数（`create_empty_exports`/`first_identifier`/`collect_children`/`declaration_name_text`/`function_body`/`body_has_value_return`）；`lib.rs` `EmitReferenceResolver` +2 加法透传（`is_declaration_visible`/`entity_name_accessibility`）；`mod.rs` re-export `new_declarations_transformer_with_diagnostics` + `DeclarationDiagnostics`。读 `opt.compiler_options.isolated_declarations`（既有字段，**未改 `TransformOptions`**）。**ZERO** `internal/ast`/`printer`/`binder` 改动；ZERO `.go` 改动。`cargo build -p tsgo_compiler` 绿（证 API 加法）。

### 测试计数

- `tsgo_checker` **735→737 unit**（+2：脚本/ module 可见性）+ doctest **177**（不变）。
- `tsgo_transformers` **295→311 unit**（+16：D-F3 切片 = 2 elision + 2 script + 4 import/export + 2 tracker 4025 + 6 isolatedDeclarations）+ doctest **57→69**（+12：tracker 4 + diagnostics 3 + util 3 + transform/lib 2）。
- **无既有测试弱化/删除**。gate 全绿（test/clippy `-D warnings`/fmt 双 crate + compiler build）。未 `git commit`（父级提交）。

### P5 declarations 完成度

**P5 declarations（D-F1 注解核心 + D-F2 推断 type-node + D-F3 可见性/可访问性/import-export/诊断）现已对可达子集完成。** 余下 blocked-by P6（`compiler.Program`/host）或 pseudo-type node builder（isolatedDeclarations 余下族）或跨模块 `SymbolAccessibilityResult`（4023/4024 + 成员可访问性臂）或 late-painted statements 完整图（module 内被引用的非导出顶层声明保留）——均按上方 DEFER 标注。

### DEFER（本轮确认 blocked-by）

- **late-painted statements 完整图**：module 内被 emit 类型引用的非导出顶层声明（Go 经 `collectLinkedAliases`/`transformAndReplaceLatePaintedStatements` 保留 + `export {};`），可达子集 elide。blocked-by：visibility-link 标记 + 跨语句依赖图（P6 program graph）。
- **isolatedDeclarations 余下族**：9013（表达式不可推）/9011（参数无注解）/9039（私名 in type）/条件·多 return → 9007（漏报）/object·array·computed·spread·expando·enum 成员。blocked-by：pseudo-type node builder 逐构造 `ReportInferenceFallback`。
- **可访问性余下臂**：4023/4024（from external/private module，需 `ErrorModuleName`）+ property/method/accessor/parameter/type-parameter/heritage/`import =`/type-alias `getSymbolAccessibilityDiagnostic` 臂 + TypeReference/ExpressionWithTypeArguments entity-name 检查（顶层 type-ref late-paint）。blocked-by：跨模块 `SymbolAccessibilityResult` + `GetEffectiveDeclarationFlags` 成员可见性。
- **`export =`/`export default` 非 identifier**：`_default` unique-name 合成 + 类型合成 dance。blocked-by：`NewUniqueNameEx` + `CreateLiteralConstValue`/`ensureType` 在 export-assignment 上下文。
- **CommonJS module 检测**：`is_external_module` 仅查 `external_module_indicator`（无 `common_js_module_indicator` 字段）。blocked-by：JS 文件 CommonJS indicator。
...
## Track 2-1/2-2 — type-eraser complete erasure

**Goal**: the type eraser was missing cases for ArrowFunction, MethodDeclaration,
Constructor, GetAccessor, and SetAccessor — their return-type annotations and
type parameters passed through unerased. Also missing: per-specifier
`import { type x }` elision.

### Changes (TDD vertical, each RED→GREEN)

| Node kind | Go case | Rust location | Test name |
|---|---|---|---|
| `ArrowFunction` | `typeeraser.go:visit/KindArrowFunction` | `typeeraser.rs:Kind::ArrowFunction` | `arrow_function_type_params_and_return_erased` |
| `MethodDeclaration` | `typeeraser.go:visit/KindMethodDeclaration` | `typeeraser.rs:Kind::MethodDeclaration` | `method_declaration_type_params_and_return_erased` |
| `Constructor` | `typeeraser.go:visit/KindConstructor` | `typeeraser.rs:Kind::Constructor` | `constructor_type_annotations_erased` |
| `GetAccessor` | `typeeraser.go:visit/KindGetAccessor` | `typeeraser.rs:Kind::GetAccessor` | `get_accessor_return_type_erased` |
| `SetAccessor` | `typeeraser.go:visit/KindSetAccessor` | `typeeraser.rs:Kind::SetAccessor` | `set_accessor_param_type_erased` |
| `ImportSpecifier` | `typeeraser.go:visit/KindImportSpecifier` | `typeeraser.rs:Kind::ImportSpecifier` | `import_type_specifier_elided` |
| `NamedImports` | `typeeraser.go:visit/KindNamedImports` | `typeeraser.rs:Kind::NamedImports` | (covered by `import_type_specifier_elided`) |
| `ImportClause` | `typeeraser.go:visit/KindImportClause` | `typeeraser.rs:Kind::ImportClause` | (covered by `type_only_imports_are_elided`) |
| `ImportDeclaration` | `typeeraser.go:visit/KindImportDeclaration` | `typeeraser.rs:Kind::ImportDeclaration` | (covered by `import_type_specifier_elided`) |

### Guard tests

- `no_annotations_unchanged` — plain JS with no type annotations passes unchanged
- `method_overload_is_elided` — bodyless method overload is removed
- `constructor_overload_is_elided` — bodyless constructor overload is removed

### Gate results

- `cargo test -p tsgo_transformers`: 320 passed, 0 failed (was 311 before, +9 new tests)
- `cargo test -p tsgo_compiler`: 11 passed, 0 failed
- `cargo clippy -p tsgo_transformers --all-targets -- -D warnings`: clean
