...
## P6-9b 已落地单测（新 crate `tsgo_incremental`，23 `#[test]` + 4 doctest，全绿）

> `.tsbuildinfo` emit/read + 文件版本/签名 + affected-files recompute + up-to-date 检查，逐个 red→green（见 impl.md 的 P6-9b worklog）。crate 位于 `internal/execute/incremental/`（镜像 Go `internal/execute/incremental` 包，**非** `internal/incremental/`）。`expected` 取自 `cmd/tsgo -p tsconfig.json`（两文件 `a.ts` import `b.ts`，`incremental:true`+`noLib:true`）实抓的 `.tsbuildinfo`。**未改 `tsgo_compiler`**（增量 `Program` 是消费 compiler `Program` 的 wrapper）。

**抓取的真实 `.tsbuildinfo`（ground truth）**：
```json
...
### BuildInfo serde（`build_info_test.rs`，`build_info.rs`）

| Rust 测试 | 验证内容 | input → expected | Go 对照 | 完成 |
|---|---|---|---|---|
| `serializes_to_go_compact_tsbuildinfo_shape` | **tracer**：序列化出 Go 紧凑形（裸字符串 fileInfo / `[start,end]` root / `[fileId,fileIdListId]` ref-map / 裸 fileId 语义诊断） | 构造 BuildInfo → `to_string` == 实抓 buildinfo 子集（逐字节，减 `errors`/语义错误态） | buildInfo.go:BuildInfo.MarshalJSON | ✓ |
| `round_trips_through_json` | marshal→unmarshal→相等 | BuildInfo → json → BuildInfo == 原 | buildInfo.go（round-trip） | ✓ |
| `parses_go_tsbuildinfo_with_one_based_file_ids` | 解析实抓 buildinfo，1-based id | `referencedMap[0]`=`{2,1}`、`fileIdsList[0]`=`[1]`、`fileInfos[0].get_file_info().version`=`90312e...`、`root[0]`=range(1,2) | buildInfo.go:BuildInfo.UnmarshalJSON | ✓ |

### 文件版本 + d.ts 签名（`snapshot_test.rs`，`snapshot.rs`）

| Rust 测试 | 验证内容 | input → expected | Go 对照 | 完成 |
|---|---|---|---|---|
| `compute_hash_matches_go_tsbuildinfo_versions` | **scaffold tracer**：xxh3 与 `zeebo/xxh3` 字节对齐 | `compute_hash("export const b = 1;\n",false)`=`90312e1cbc42534115cfa9601aa41950`；`a.ts`→`b6df5f...` | snapshot.go:ComputeHash | ✓ |
| `compute_file_version_is_stable_and_text_sensitive` | 版本稳定 + 文本敏感 | `const x = 1;` 稳定、==`compute_hash(..,false)`、≠`const x = 2;` | programtosnapshot.go:computeProgramFileChanges | ✓ |
| `compute_signature_hashes_declaration_text` | 可达 `.d.ts` 签名（文本哈希近似） | `compute_signature(dts)`==`compute_hash(dts,false)`、不同声明→不同签名 | snapshot.go:computeSignatureWithDiagnostics | ✓ |
| `fresh_file_info_uses_version_as_signature` | fresh build `signature=version` | `for_fresh_text("export const b = 1;\n",..)` → version=`90312e...`、signature==version、!affects、CommonJS | programtosnapshot.go:computeProgramFileChanges（oldProgram==nil） | ✓ |

### affected-files recompute（`reference_map_test.rs` + `affectedfileshandler_test.rs`）

| Rust 测试 | 验证内容 | input → expected | Go 对照 | 完成 |
|---|---|---|---|---|
| `referenced_by_inverts_the_references` | 反图：谁引用我 | `A refs B` → `referenced_by(B)`=`[A]`、`referenced_by(A)`=`[]`、`referenced_by(C)`=`[]` | referencemap.go:referenceMap.getReferencedBy | ✓ |
| `referenced_by_returns_all_referrers_sorted` | 多引用者排序 | `Z refs S`+`A refs S` → `referenced_by(S)`=`[A,Z]` | referencemap.go:getReferencedBy | ✓ |
| `affected_set_includes_direct_referrers` | A imports B；B 变→`{B,A}`；无关 C→`{C}` | `affected(B)`=`[A,B]`、`affected(A)`=`[A]`、`affected(C)`=`[C]` | affectedfileshandler.go:getFilesAffectedBy | ✓ |
| `affected_set_is_transitive` | 传递闭包 | `A→B→C`，C 变→`[A,B,C]`；B 变→`[A,B]` | affectedfileshandler.go:forEachFileReferencedBy | ✓ |
| `affected_set_terminates_on_cycle` | 环终止 | `A↔B`，A 变→`[A,B]` | affectedfileshandler.go（seen 守卫） | ✓ |
| `affected_set_from_build_info_referenced_map` | 从实抓 buildinfo 的 `referencedMap:[[2,1]]` | `affected(./b.ts)`=`[./a.ts,./b.ts]`、`affected(./a.ts)`=`[./a.ts]` | buildinfotosnapshot.go:setReferencedMap | ✓ |
| `collect_all_affected_files_unions_changed_set` | 多变更并集 | `[B,Y]`→`[A,B,X,Y]` | affectedfileshandler.go:collectAllAffectedFiles | ✓ |
| doctest `get_files_affected_by` | A imports B，B 变→`{B,A}` | 同上 | affectedfileshandler.go:getFilesAffectedBy | ✓ |

### up-to-date 检查（`uptodatestatus_test.rs`，`uptodatestatus.rs`）

| Rust 测试 | 验证内容 | input → expected | Go 对照 | 完成 |
|---|---|---|---|---|
| `missing_tsbuildinfo_is_out_of_date` | buildinfo 缺失 | `None` → `OutputMissing`（!up_to_date） | buildtask.go:getUpToDateStatus（buildInfo==nil） | ✓ |
| `input_newer_with_changed_text_is_out_of_date` | 输入更新 + 文本变 | mtime↑ + 文本变 → `InputFileNewer` | getUpToDateStatus（version mismatch） | ✓ |
| `input_newer_with_unchanged_text_is_up_to_date` | 输入更新 + 文本未变 | mtime↑ + 文本同 → `UpToDateWithInputFileText`（up_to_date） | getUpToDateStatus（inputTextUnchanged） | ✓ |
| `all_inputs_older_than_buildinfo_is_up_to_date` | 全旧 | mtime↓ → `UpToDate` | getUpToDateStatus（UpToDate） | ✓ |
| `missing_input_file_is_out_of_date` | 输入缺失 | mtime None → `InputFileMissing` | getUpToDateStatus（inputTime.IsZero） | ✓ |
| `version_mismatch_is_out_of_date` | buildinfo 版本不符 | `version="0.0.0-old"` → `TsVersionOutputOfDate` | getUpToDateStatus（!IsValidVersion） | ✓ |

### Program 集成 emit/read（`program_test.rs`，`program.rs`）

| Rust 测试 | 验证内容 | input → expected | Go 对照 | 完成 |
|---|---|---|---|---|
| `emits_tsbuildinfo_to_expected_path_in_go_shape` | **集成 tracer**：真 compiler Program → 写 `.tsbuildinfo` | `tsBuildInfoFile=/proj/tsconfig.tsbuildinfo` → 写到该路径、内容**逐字节** == 实抓 buildinfo（减 `errors`/语义态）、host fs 真写入 | program.go:emitBuildInfo + snapshotToBuildInfo | ✓ |
| `non_incremental_program_emits_nothing` | 非增量不写 | 无 incremental/composite/tsBuildInfoFile → `build_info_file_name()==""`、`build_info()`/`emit_build_info()` 皆 `None` | outputpaths.go:GetBuildInfoFileName（→""） | ✓ |
| `reads_back_tsbuildinfo_to_seed_reuse` | 回读 seed reuse | 回读 `Snapshot`：`b.ts` version 复原、复原图上 `affected(b.ts)`=`[a.ts,b.ts]`、`affected(a.ts)`=`[a.ts]` | incremental.go:ReadBuildInfoProgram + buildInfoToSnapshot | ✓ |
| doctest `compute_hash`/`compute_file_version`/`compute_signature` | 见 snapshot.rs doctest | — | snapshot.go | ✓ |

### P6-9b DEFER（blocked-by）

| 行为 | blocked-by | 目标 |
|---|---|---|
| compiler 真实 build 触发 `.tsbuildinfo` emit/read + skip-emit reuse | **`--build`/`tsc` 编排在 `internal/execute`（P9）** | P9 execute |
| watch 增量重建 | **watch 子系统（`execute/watcher.go`）** | P6-9c / P9 |
| 老程序 diff / change-set / pending-emit / emit signatures / 缓存诊断 payload / `errors` 态 / 完整 options 集 / 跨包 `build.getUpToDateStatus` 编排 | UpdateProgram + checker 诊断序列化 + declaration emit 签名 + tsoptions option-反射 + execute build orchestrator | 见 impl.md P6-9b DEFER 表 |
