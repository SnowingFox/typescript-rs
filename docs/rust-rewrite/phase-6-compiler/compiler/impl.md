...
| **P6-9b+（下轮，建议）** | ① `.tsbuildinfo` 读写 + 增量 affected-files；② **并行化** emit + 跨 checker 诊断收集（待 checker 切 `Arc<dyn BoundProgram + Send + Sync>`）；③ `Program::semantic_diagnostics(file)` per-file 过滤入口（`GetDiagnosticsForFile`）；④ 跨文件**声明合并**（`mergeSymbol`）+ `globalThis`；⑤ 全 lib 图绑定（待 binder 计算属性名）；⑥ emit resolver → declaration（`.d.ts`）emit / importElision / 完整 `getScriptTransformers`；⑦ source-of-reference redirect + faking VFS + UpdateProgram；⑧ sourcemap declaration map；⑨ `/// <reference path>`/`<reference types>`（待 parser 暴露指令 + ATA） | blocked-by: P6-9b tsbuildinfo；checker `Arc` 程序 + 跨文件 `mergeSymbol` + emit resolver；binder 计算属性名；parser path/types reference 指令；P9 `--build` 编排 + module redirect |
