...
| `core/projectreference.go` + `host.go:GetResolvedProjectReference` + `projectreferenceparser.go` + `projectreferencefilemapper.go` + `program.go:verifyProjectReferences` + `execute/build/orchestrator.go:setupBuildTask` + `tsoptions/parsedcommandline.go`（部分） | `internal/compiler/projectreference.rs`（**P6-9a 新增**） | **合并模块**：project-reference 解析 + 拓扑构建序 + 环检测（TS6202）+ composite/noEmit 校验（TS6306/6310/6053）+ outDir/rootDir 输出路径 + rootDir 校验（TS6059）的**可达子集**。Go 散落在多文件 + build 包；本轮把 P9 要驱动的"解析+序+输出路径"机器合并成一个内聚模块（受认可的偏离，理由：完整的 parser/filemapper 需 module redirect + faking VFS + build 编排，均 DEFER） |
...
### `projectreference.rs`（**P6-9a 新增**，合并 project-reference 可达子集）

> Go 的 project-reference 逻辑散落在 `core/projectreference.go`、`compiler/host.go`、`projectreferenceparser.go`、`projectreferencefilemapper.go`、`program.go:verifyProjectReferences`、`execute/build/orchestrator.go`、`tsoptions/parsedcommandline.go`。P6-9a 把 **P9 要驱动的解析+序+输出路径可达子集**合并进 `projectreference.rs`（见上文"文件清单"的合并说明）。`internal/tsoptions/**` 未改（`references`/`composite`/`outDir`/`rootDir` 已被 tsconfigparsing 解析）。

- [x] **P6-9a** `pub fn get_resolved_project_reference(host, config_name) -> Option<ParsedCommandLine>`（读+解析引用配置，镜像 `host.go:GetResolvedProjectReference`→`GetParsedCommandLineOfConfigFilePath`）+ 单测　`// Go: host.go:compilerHost.GetResolvedProjectReference`
- [x] **P6-9a** `struct ProjectReferenceNode { config_name, resolved, reference_paths }`（镜像 `projectReferenceParseTask`）+ `pub struct ResolvedProjectReferences { nodes, root_path, current_directory, use_case_sensitive_file_names }`（镜像 `projectReferenceFileMapper` 的 `configToProjectReference`+`referencesInConfigFile`）　`// Go: projectreferenceparser.go + projectreferencefilemapper.go`
- [x] **P6-9a** `pub fn resolve_project_references(host, root_name, root) -> ResolvedProjectReferences`（worklist 按路径去重的图构建，镜像 `projectReferenceParser.parse`+`initMapperWorker`，顺序版）+ doctest　`// Go: projectreferenceparser.go:projectReferenceParser.parse`
- [x] **P6-9a** `ResolvedProjectReferences::{get_resolved_project_references, get_resolved_reference_for, to_path}`（镜像 `getResolvedProjectReferences`/`getResolvedReferenceFor`）+ 单测
- [x] **P6-9a** `get_build_order() -> BuildOrder { order, circular_diagnostics }` + `setup_build_task`（拓扑后序 + TS6202 环检测，镜像 `orchestrator.go:setupBuildTask`）+ 单测　`// Go: execute/build/orchestrator.go:setupBuildTask`
- [x] **P6-9a** `verify_project_references() -> Vec<ProjectReferenceDiagnostic>` + `verify_worker`（TS6306/6310/6053，seenRef 去重遍历，镜像 `verifyProjectReferences`+`rangeResolvedReferenceWorker`）+ 单测　`// Go: program.go:verifyProjectReferences + projectreferencefilemapper.go:rangeResolvedReferenceWorker`
- [x] **P6-9a** `get_output_declaration_file_name` / `get_output_js_file_name`（经 `config_output_paths_host` 计算 CSD + `tsgo_outputpaths` worker）+ `common_source_files`（CSD 文件过滤）+ 单测　`// Go: tsoptions/parsedcommandline.go:getOutputDeclarationAndSourceFileNames`
- [x] **P6-9a** `check_source_files_belong_to_root_dir() -> Vec<ProjectReferenceDiagnostic>`（TS6059，镜像 `parsedcommandline.go:checkSourceFilesBelongToPath`；注：任务 brief 误标 TS6307，实为 TS6059）+ 单测
- [x] **P6-9a** `pub struct ProjectReferenceDiagnostic { message, args }`（`code()`/`text()` 经 `localize`）+ `pub struct BuildOrder`
- [ ] `ParseInputOutputNames` 的完整 source↔outputDts map（`SourceOutputAndProjectReference`）/ redirect / faking VFS / 并行（`core.WorkGroup`）/ `.tsbuildinfo` — DEFER 见 P6-9a DEFER 表

### `projectreferenceparser.rs`（Go: `projectreferenceparser.go`）——**并发点 2**

> **注（P6-9a）**：可达子集（解析图 + 去重）已落地在 `projectreference.rs::resolve_project_references`（顺序版）。下列仍为 DEFER 的是 tracing / 并行 `WorkGroup` / `initMapper` 的 source↔output map 完整面。

- [ ] `struct ProjectReferenceParseTask { config_name, resolved, sub_tasks }` + `parse(...)`（含 tracing；`ParseInputOutputNames`；递归子引用）— P6-9a 落地无-tracing/顺序的等价物　`// Go: projectreferenceparser.go`
- [ ] `fn create_project_reference_parse_tasks(refs)`　`// Go: projectreferenceparser.go`
- [ ] `struct ProjectReferenceParser { loader, wg, tasks_by_file_name: DashMap }` + `parse`/`start`（并行）/`init_mapper`（单线程）/`init_mapper_worker`（深度优先 seen 去重，子覆盖父）— DEFER 并行化 + source↔output map　`// Go: projectreferenceparser.go`
...
| **P6-9a（本轮，✅ 部分）** | **project references 解析 + 构建序 + outDir/rootDir/composite 输出路径**：新增 `projectreference.rs`——① `get_resolved_project_reference`（读+解析引用配置）；② `resolve_project_references`（按路径去重的引用图，`ResolvedProjectReferences`）；③ `get_build_order`（拓扑序 + TS6202 环检测，镜像 `setupBuildTask`）；④ `verify_project_references`（TS6306 composite / TS6310 noEmit / TS6053 not-found，镜像 `verifyProjectReferences`）；⑤ `get_output_declaration_file_name`/`get_output_js_file_name`（经 CSD adapter 调 `tsgo_outputpaths`）；⑥ `check_source_files_belong_to_root_dir`（TS6059）；⑦ `Program` 集成（`get_resolved_project_references`/`project_reference_diagnostics`/`build_order`，`NewProgram` 内跑 verify）。consume `tsconfigparsing` 已解析的 `references`/`composite`/`outDir`/`rootDir`（**未改 tsoptions**） | 已绿（83 单测 + 11 doctest）。**仍 blocked**：见下表 P6-9a DEFER —— `--build` 编排执行/up-to-date/watch（P9）、`.tsbuildinfo`+增量（P6-9b）、诊断节点锚定、TS6307 + source-of-reference redirect + faking VFS（P9） |
| **P6-9b（本轮，✅ 部分）** | **`.tsbuildinfo` emit/read + 程序状态 diff（文件版本/签名）+ affected-files recompute + up-to-date 检查**：新增独立 crate **`tsgo_incremental`**（位于 `internal/execute/incremental/`，镜像 Go `internal/execute/incremental` 包；**不是** `internal/incremental/`——任务 brief 误标，FOLLOW GO）——① `BuildInfo` serde（compact JSON，1-based file ids，与 `cmd/tsgo` 字节对齐）；② `compute_hash`/`compute_file_version`（xxh3-128，与 `zeebo/xxh3` 字节对齐）+ 可达 `.d.ts` `compute_signature`；③ `ReferenceMap` + `get_files_affected_by`（`referencedMap` 反图传递闭包）；④ `get_up_to_date_status`（buildinfo 缺失/输入更新判定，镜像 `build/getUpToDateStatus` 可达子集）；⑤ `Program`（增量）：`program_to_snapshot`→`snapshot_to_build_info`→`emit_build_info`（写 `.tsbuildinfo`）+ `read_build_info(_program)`（回读 seed reuse）。consume `tsgo_compiler::Program`（**未改 compiler `Program`**） | 已绿（23 单测 + 4 doctest，clippy/fmt 净）。**仍 blocked**：见下表 P6-9b DEFER —— 把 compiler 真实 build 接到 `.tsbuildinfo` emit/read + skip-emit reuse → **P9 execute**；watch → **P6-9c**；老程序 diff/change-set/pending-emit、emit signatures、诊断 payload、跨包 `build/getUpToDateStatus` 编排 |
| **P6-9c+（下轮，建议）** | ① watch 模式增量重建；② **并行化** emit + 跨 checker 诊断收集（待 checker 切 `Arc<dyn BoundProgram + Send + Sync>`）；③ `Program::semantic_diagnostics(file)` per-file 过滤入口（`GetDiagnosticsForFile`）；④ 跨文件**声明合并**（`mergeSymbol`）+ `globalThis`；⑤ 全 lib 图绑定（待 binder 计算属性名）；⑥ emit resolver → declaration（`.d.ts`）emit / importElision / 完整 `getScriptTransformers`；⑦ source-of-reference redirect + faking VFS + UpdateProgram；⑧ sourcemap declaration map；⑨ `/// <reference path>`/`<reference types>`（待 parser 暴露指令 + ATA） | blocked-by: P6-9c watch；checker `Arc` 程序 + 跨文件 `mergeSymbol` + emit resolver；binder 计算属性名；parser path/types reference 指令；P9 `--build` 编排 + module redirect |
...
### P6-9a red→green worklog（逐行为，project references：解析 + 构建序 + 输出路径）

> 本轮把 Go 散落在多个文件里的 **project-reference 可达子集**合并进一个新模块 `internal/compiler/projectreference.rs`（+ `projectreference_test.rs`）：解析 `references[]`、拓扑构建序 + 环检测（TS6202）、composite 要求（TS6306）、以及 project references 依赖的 outDir/rootDir/declarationDir 输出路径解析。`internal/tsoptions/**` **未改**（`references`/`composite`/`outDir`/`rootDir`/`declarationDir` 已被 `tsconfigparsing` 解析进 `ParsedCommandLine.parsed_config.project_references` / `CompilerOptions`，本轮直接消费）。**全 additive**（新模块 + Program 新增 3 个 pub 访问器；无既有 `pub fn` 签名改动、无 `.go`/`internal/ast`/`internal/checker`/`internal/transformers` 改动）。
>
> **Go ground truth 先抓后断言（`cmd/tsgo -b` / `-p`）**：① 线性 A→B→C 的 "Projects in this build" = `[c, b, a]`；② 环 A→B→C→A → `error TS6202: Project references may not form a circular graph. Cycle detected: /abs/a/tsconfig.json\n/abs/b/tsconfig.json\n/abs/c/tsconfig.json`（2-cycle → `a\nb`，cycle 体 = 检测时 analyzing 链的 configNames 绝对路径，`\n` 连接）；③ B 非 composite、A references B（A 有 files）→ `TS6306: Referenced project '/abs/b' must have setting "composite": true.`（arg = `ref.Path` = 规范化绝对**引用目录**，非 tsconfig.json；`-b` 与 `-p` 均在 `NewProgram` 的 `verifyProjectReferences` 报出）；④ `outDir:./bin` + `rootDir:./src` + `declaration` → `src/index.ts`→`bin/index.js`+`bin/index.d.ts`、`src/sub/util.ts`→`bin/sub/util.{js,d.ts}`；⑤ rootDir 外文件 → **TS6059**（`File '{0}' is not under 'rootDir' '{1}'. ...`），**不是 TS6307**（任务 brief 误标——TS6307 是 "File not listed within the file list of project" 的另一诊断，留 P9）。

1. **解析单个引用配置（tracer，`get_resolved_project_reference_parses_config`）**：`get_resolved_project_reference(host, config_name)` 先 `todo!()` → 跑 → **RED**（not yet implemented）。再实现（镜像 Go `compilerHost.GetResolvedProjectReference`→`GetParsedCommandLineOfConfigFilePath`：规范化名 + host fs 读文本 + `new_tsconfig_source_file_from_file_path` + `parse_json_source_file_config_file_content`，base path = 配置目录）→ **GREEN**：composite/files 往返、缺失配置 → `None`。
2. **解析单个引用图（`resolves_single_project_reference`）**：`resolve_project_references(host, root_name, root)` 先 `todo!()` → **RED**。再实现（worklist 按规范路径去重：每个 `ProjectReference` 经 `resolve_project_reference_path` → 读+解析子配置 → 递归；`get_resolved_project_references()` 返回 root 直接引用）→ **GREEN**：`../lib` 解析到 `/lib/tsconfig.json`（composite + `/lib/index.ts`）。
3. **拓扑构建序（`build_order_is_topological`）**：`setup_build_task` 先 `todo!()` → **RED**。再实现（镜像 `orchestrator.go:setupBuildTask` 的 completed/analyzing/stack 深度优先后序，本步不报环错）→ **GREEN**：A→B→C → `order==[c, b, a]`、无环诊断。
4. **环检测 TS6202（`build_order_reports_circular_graph` + `_three_nodes`）**：先写 2-cycle/3-cycle 测试 → **RED**（`circular_diagnostics.len()` 0≠1）。再在 analyzing 命中分支 push `PROJECT_REFERENCES_MAY_NOT_FORM_A_CIRCULAR_GRAPH...`（cycle 体 = `state.stack.join("\n")`）→ **GREEN**：2-cycle → `a\nb`、3-cycle → `a\nb\nc`、order 仍 `[c,b,a]`（与 Go `cmd/tsgo -b` 实测逐字节一致）。
5. **composite 要求 TS6306（`verify_reports_missing_composite` + `verify_accepts_composite_reference`）**：`verify_project_references` 先 `todo!()` → **RED**。再实现（镜像 `program.go:verifyProjectReferences` + `rangeResolvedReferenceWorker` 的 seenRef 去重遍历：`!composite||noEmit` 且 parent 有 files → `!composite`→TS6306 / `noEmit`→TS6310 / config 缺失→TS6053，arg = `ref.path` 规范化绝对引用目录）→ **GREEN**：非 composite → 1×TS6306 `'/lib'`、composite → 空。
6. **输出路径 outDir/rootDir（`computes_reference_output_paths_with_out_dir_and_root_dir`）**：`config_output_paths_host` 先 `todo!()` → **RED**。再实现（按 `ParsedCommandLine.CommonSourceDirectory` 计算 CSD = rootDir，建 `OutputPathsHost` adapter，调 `tsgo_outputpaths::get_output_declaration_file_name_worker`/`get_output_js_file_name`）→ **GREEN**：`/lib/src/index.ts`→`/lib/bin/index.{js,d.ts}`、`/lib/src/sub/util.ts`→`/lib/bin/sub/util.{js,d.ts}`（与 `cmd/tsgo -p` emit 实测一致）。
7. **rootDir 外文件 TS6059（`reports_file_outside_root_dir` + `accepts_files_inside_root_dir`）**：`check_source_files_belong_to_root_dir` 先 `todo!()` → **RED**。再实现（镜像 `parsedcommandline.go:checkSourceFilesBelongToPath`：经 `get_common_source_directory` 的 check 回调，`RefCell` 收集，`!ContainsPath(rootDir, file)` → TS6059 `[absoluteFile, rootDir]`）→ **GREEN**：`other/x.ts` 不在 `/proj/src` → 1×TS6059 `'/proj/other/x.ts' ... 'rootDir' '/proj/src'`、全在内 → 空。**注**：任务 brief 标 "TS6307" 实为 TS6059（FOLLOW GO + 文档化）。
8. **Program 集成（`program_surfaces_project_reference_diagnostics_and_build_order` + `program_without_config_file_has_no_references`）**：`new_program` 在 `config_file_path` 非空时解析引用图 + 跑 `verify_project_references`，新增 `Program::get_resolved_project_references()` / `project_reference_diagnostics()` / `build_order()`（镜像 Go `NewProgram` 内 `verifyProjectReferences` + `GetResolvedProjectReferences`）。配置驱动程序（`/app` references 非-composite `/lib`）→ 1×TS6306 + build order `[lib, app]`；无配置文件程序（命令行构造）→ 三者皆空（无回归）。

**文件清单**：新增 `internal/compiler/projectreference.rs`（合并 Go `core/projectreference.go` + `compiler/host.go:GetResolvedProjectReference` + `projectreferenceparser.go` + `projectreferencefilemapper.go:{getResolvedProjectReferences,getResolvedReferenceFor,rangeResolvedReferenceWorker}` + `program.go:verifyProjectReferences` + `execute/build/orchestrator.go:setupBuildTask` + `tsoptions/parsedcommandline.go:{ResolvedProjectReferencePaths(经 core),getOutputDeclarationAndSourceFileNames,checkSourceFilesBelongToPath}` 的**可达子集**，每个 fn 带 `// Go:` 锚）+ `projectreference_test.rs`。

**自检门禁（仅 `-p tsgo_compiler`；未改 tsoptions 故不需 `-p tsgo_tsoptions`）**：`cargo test`（**83 单测 + 11 doctest** 全绿，较 P6-options/P6-7 基线 71+10 → +12 单测/+1 doctest）/ `cargo clippy --all-targets -- -D warnings`（净）/ `cargo fmt -p tsgo_compiler -- --check`（净）/ `cargo build -p tsgo_compiler`（净）。

### P6-9a DEFER（blocked-by）

| 行为 | blocked-by | 目标 |
|---|---|---|
| `--build` 编排（up-to-date 检查、构建执行、watch downStream 跟踪、并行 `rangeTask`） | **execute/build 编排（P9）** —— 本轮只产出 P9 要驱动的 **解析 + 序 + 输出路径**机器 | P9 execute |
| `.tsbuildinfo` 读写 + 增量 affected-files + `Cannot_write_file_..._tsbuildinfo` overwrite 检查 | **P6-9b tsbuildinfo** | P6-9b（下轮） |
| 诊断锚定到 tsconfig `reference` 语法节点（Go `CreateDiagnosticAtReferenceSyntax`） | **tsconfig option-syntax AST 诊断面** —— 本轮产 compiler 诊断（message/code/args 与 Go 一致，仅 location 不同） | option-syntax 诊断轮 |
| TS6307（"File not listed within the file list of project"）/ source-of-project-reference redirect / `projectReferenceDtsFakingHost` | **program source-of-reference redirect + faking VFS（P9）** | P9 |
| `disableReferencedProjectLoad`/`disableSolutionSearching` 性能旗标、`prepend` 输出拼接、solution-style（`files:[]`+`references`）特化、project reference 并行解析（Go `core.WorkGroup`） | 上述各自子系统 | 后续轮 |
| `ProjectReference.Circular`（来自 extends 的显式循环标记）的端到端来源 | tsconfig `references` 的 circular 解析面 | 后续轮（本轮已忠实消费 `circular` 字段，默认 false） |

### P6-9b red→green worklog（逐行为，`.tsbuildinfo` 增量基石：新 crate `tsgo_incremental`）

> 本轮把 Go 的增量编译机器（`.tsbuildinfo` emit/read + 程序状态 diff + affected-files recompute + up-to-date 检查）的**可达子集**移植成一个**独立 crate `tsgo_incremental`**，位于 `internal/execute/incremental/`，与 Go 源码 side-by-side（PORTING §2）。
>
> **crate 位置（FOLLOW GO + 文档化）**：任务 brief 假设 Go 的 incremental 包在 `internal/incremental/`，但真实位置是 **`internal/execute/incremental/`**（`incremental` 是 `execute` 的子包，被 `execute/build` 的 `--build` 驱动）。故按"`.rs` 紧贴 `.go` 同目录同名"把新 crate 放在 `internal/execute/incremental/`（而非 `internal/incremental/`）。`internal/execute/` 尚无 crate（execute 是 P9），但子目录作为独立 crate 成员是合法的、纯 additive。crate 名 `tsgo_incremental`，已注册进根 `Cargo.toml` workspace members。Go 包依赖 `internal/compiler`，故 crate 依赖 `tsgo_compiler`（无环：compiler 不依赖 incremental）。
>
> **依赖**：`serde`/`serde_json`/`indexmap`(serde feature)/`xxh3`(0.1.1)/`rustc-hash` + `tsgo_{collections,compiler,core,outputpaths,tspath,vfs}`；dev-dep `tsgo_{parser,tsoptions}`（测试建 `Program`）。
>
> **Go ground truth 先抓后断言（`cmd/tsgo -p tsconfig.json`，两文件 `a.ts` import `b.ts`，`incremental:true`+`noLib:true`）**：写出的 `tsconfig.tsbuildinfo` 字节为
> ```json
> {"version":"7.0.0-dev","errors":true,"root":[[1,2]],"fileNames":["./b.ts","./a.ts"],"fileInfos":["90312e1cbc42534115cfa9601aa41950","b6df5f2b27e276d9e3e67069347c11a5"],"fileIdsList":[[1]],"options":{"module":99,"target":99},"referencedMap":[[2,1]],"semanticDiagnosticsPerFile":[1,2]}
> ```
> 要点（**与 Go 字节对齐处**）：① `fileNames` 相对 buildInfo 目录（`./b.ts`）；② `fileInfos` 紧凑形——`version==signature && !affectsGlobalScope && impliedNodeFormat==CommonJS(1)` 时是裸 version-hash 字符串（项目文件），否则对象（lib 文件 `{version,affectsGlobalScope,impliedNodeFormat}`）；③ file ids **1-based**（`b.ts`=1、`a.ts`=2）；④ `referencedMap` = `[[fileId, fileIdListId]]`（`a.ts`(2) 引 list#1=`[1]`=`b.ts`）；⑤ `root`=`[[1,2]]`（连续 id 区间）；⑥ `options` 仅 `AffectsBuildInfo` 子集（`module`/`target`=99=ESNext），结构体字段序即 JSON key 序；⑦ version-hash = `zeebo/xxh3.HashString128(text).Bytes()` 的 hex（128-bit canonical 大端）。完整带 lib 的 buildinfo（93 lib + 2 源）也抓取并核对了形状。

1. **xxh3 字节对齐（scaffold tracer，`compute_hash_matches_go_tsbuildinfo_versions`）**：先写测试断言 `compute_hash("export const b = 1;\n", false)=="90312e1cbc42534115cfa9601aa41950"`（取自实抓 buildinfo）→ 跑 → **GREEN**（`xxh3::hash128_with_seed(text,0)` + `{:032x}` 与 `zeebo/xxh3` `.Bytes()` 逐字节一致，确认 crate `xxh3` 0.1.1 与 Go 同 canonical 大端布局；scaffold 步先验证哈希基石）。
2. **BuildInfo serde compact 形（`serializes_to_go_compact_tsbuildinfo_shape` + `round_trips_through_json` + `parses_go_tsbuildinfo_with_one_based_file_ids`）**：先 naive `#[derive(Serialize)]` → 跑 → **RED**（`root` 出 `{"start":1,"end":2,"non_incremental":""}`、`fileInfos` 出对象、`referencedMap` 出 `{"file_id":..}`，且 `[1,2]` 解析进 3 字段 struct 失败）。再为 `BuildInfoRoot`/`BuildInfoFileInfo`/`BuildInfoReferenceMapEntry`/`BuildInfoSemanticDiagnostic` 写自定义 `Serialize`/`Deserialize`（镜像 Go 各 `MarshalJSON`/`UnmarshalJSON` 的 union 分支）→ **GREEN**：序列化与实抓 buildinfo 子集**逐字节一致**、round-trip 相等、1-based id 正确解析。
3. **文件版本 + d.ts 签名（`compute_file_version_is_stable_and_text_sensitive` + `compute_signature_hashes_declaration_text` + `fresh_file_info_uses_version_as_signature`）**：先写测试引用 `compute_file_version` → 跑 → **RED**（函数不存在）。再实现 `compute_file_version(text)=compute_hash(text,false)`（Go `computeProgramFileChanges` 的 `version:=computeHash(file.Text())`）+ `compute_signature(dts)`（可达文本哈希近似，**真 declaration emit 签名 DEFER**）+ `FileInfo::for_fresh_text`（fresh build `signature=version`，Go `oldProgram==nil` 臂）→ **GREEN**。
4. **affected-files 反图传递闭包（`referenced_by_inverts_the_references` → `affected_set_includes_direct_referrers`/`_is_transitive`/`_terminates_on_cycle` → `affected_set_from_build_info_referenced_map` → `collect_all_affected_files_unions_changed_set`）**：先 `ReferenceMap::get_referenced_by` / `get_files_affected_by` 写 `todo!()` → 跑 → **RED**（panic）。再实现 `get_referenced_by`（按需反转 `references`，排序确定性）+ `get_files_affected_by`（从变更文件起 BFS 走 `referencedBy`，`seen` 兼作环守卫）→ **GREEN**：`A imports B`，B 变 → `{B,A}`；无关 C → `{C}`；`A→B→C`，C 变 → `{C,B,A}`；环终止；且经 `BuildInfo::reference_map_by_name` 用实抓 `referencedMap:[[2,1]]` 验证 `affected(b.ts)=={a.ts,b.ts}`。
5. **up-to-date 谓词（`missing_tsbuildinfo_is_out_of_date` / `input_newer_with_{changed,unchanged}_text` / `all_inputs_older` / `missing_input_file` / `version_mismatch`）**：先 `get_up_to_date_status` 写 `todo!()` → **RED**。再实现（镜像 `buildtask.go:getUpToDateStatus` 可达子集：buildinfo 缺失→`OutputMissing`；版本不符→`TsVersionOutputOfDate`；输入缺失→`InputFileMissing`；输入 mtime 晚于 buildinfo 且文本哈希变→`InputFileNewer`，文本未变→`UpToDateWithInputFileText`；否则→`UpToDate`）→ **GREEN**。
6. **Program 集成 emit/read（`emits_tsbuildinfo_to_expected_path_in_go_shape` + `non_incremental_program_emits_nothing` + `reads_back_tsbuildinfo_to_seed_reuse`）**：建真 `tsgo_compiler::Program`（`noLib`+`incremental`+`tsBuildInfoFile`+`module/target ESNext`，两文件 `a.ts` import `b.ts`）→ `incremental::Program::new(&cp)`（`program_to_snapshot`：逐文件 `compute_file_version` + 相对 import 解析建 `referencedMap`）→ `emit_build_info()`（`snapshot_to_build_info`→serde→host fs 写）。断言：① 写到 `/proj/tsconfig.tsbuildinfo`；② 内容**逐字节** == 实抓 buildinfo（**减** `errors`/语义错误态，那是 DEFER）；③ host fs 真写入；④ `read_build_info_program` 回读 seed `Snapshot`，`b.ts` version 复原、`get_files_affected_by` 在复原图上 `affected(b.ts)=={a.ts,b.ts}`；⑤ 非增量程序 `build_info_file_name()==""` 且 emit/build_info 均 `None`。

**文件清单（Go → Rust，每个 fn 带 `// Go:` 锚）**：

| Go 文件 | Rust 文件 | 落地（可达子集） |
|---|---|---|
| `incremental.go` | `lib.rs`（crate 根） | `mod` 声明 + re-export；`read_build_info`/`read_build_info_program`/`BuildInfoReader` 落在 `program.rs` |
| `buildInfo.go` | `build_info.rs` | `BuildInfo`/`BuildInfoFileId`/`BuildInfoFileIdListId`/`BuildInfoRoot`/`BuildInfoFileInfo`(+No/WithSignature)/`BuildInfoReferenceMapEntry`/`BuildInfoSemanticDiagnostic` + 自定义 serde + `from_file_info`/`get_file_info`/`is_valid_version`/`is_incremental`/`version_of`/`reference_map_by_name` |
| `snapshot.go` | `snapshot.rs` | `ComputeHash`→`compute_hash`/`compute_file_version`/`compute_signature`、`FileInfo`(+`for_fresh_text`)、`Snapshot`、`RESOLUTION_MODE_COMMON_JS` |
| `referencemap.go` | `reference_map.rs`（注：Go 文件名全小写 `referencemap`，Rust 用更可读的 snake `reference_map`） | `ReferenceMap`：`store_references`/`get_references`/`get_paths_with_references`/`get_referenced_by` |
| `affectedfileshandler.go` | `affectedfileshandler.rs` | `get_files_affected_by`（`getFilesAffectedBy`/`forEachFileReferencedBy` 可达子集）+ `collect_all_affected_files`（`collectAllAffectedFiles`） |
| `programtosnapshot.go` | `programtosnapshot.rs` | `program_to_snapshot`（fresh 路径）+ `referenced_files`（`getReferencedFiles` 可达子集：相对 specifier 解析） |
| `snapshottobuildinfo.go` | `snapshottobuildinfo.rs` | `snapshot_to_build_info` + `relative_to_build_info`/`file_id_for`(`toFileId`)/`file_id_list_id_for`(`toFileIdListId`)/`set_file_info`/`set_root_of_incremental_program`/`set_compiler_options`/`set_referenced_map`/`set_semantic_diagnostics` |
| `buildinfotosnapshot.go` | `buildinfotosnapshot.rs` | `build_info_to_snapshot`（名→Path 复原 + `file_infos` + `referencedMap`） |
| `program.go` | `program.rs` | `Program`(增量)/`new`/`snapshot`/`build_info_file_name`/`build_info`/`emit_build_info`/`EmitBuildInfoResult`；`read_build_info`/`read_build_info_program`/`BuildInfoReader`/`HostBuildInfoReader`（来自 `incremental.go`） |
| `build/uptodatestatus.go` + `build/buildtask.go:getUpToDateStatus` | `uptodatestatus.rs`（Go 在 `execute/build` 包，本 crate 内放可达谓词，因它消费 `BuildInfo`+`compute_hash`） | `UpToDateStatusType` + `get_up_to_date_status` + `InputFile` |

**测试增量**：新 crate 从 0 → **23 单测 + 4 doctest 全绿**（`tsgo_compiler` 仍 83 单测 + 11 doctest，**无回归**）。

### P6-9b DEFER（blocked-by）

| 行为 | blocked-by | 目标 |
|---|---|---|
| 把 **compiler 真实 build** 接到 `.tsbuildinfo` emit/read（编译管线里 `--incremental`/`--composite` 触发写 buildinfo + 下次 build 读回 seed） + **skip-emit reuse** 优化 | **`--build`/`tsc` 编排在 `internal/execute`（P9，未移植）** —— 本 crate 已交付库基石（`Program::emit_build_info`/`read_build_info_program`），但驱动它的 build/emit 循环属 execute | **P9 execute** |
| **watch** 模式下的增量重建（downstream 跟踪、文件变更触发） | **watch 子系统（`execute/watcher.go`）** | **P6-9c / P9** |
| 老程序 diff：`reuseFromOldProgram`/`computeProgramFileChanges`（version/affectsGlobalScope/impliedNodeFormat/references 比对）/`changedFilesSet`/`handlePendingEmit`/`handlePendingCheck` | **UpdateProgram + old `Program` 复用（P6 program）** + `tsoptions::CompilerOptionsAffect*` | 后续轮 |
| `affectsGlobalScope`（`fileAffectsGlobalScope`：module augmentation/ambient 判定）+ `impliedNodeFormat` 精确值 | **binder module-augmentations + ast `SourceFileMetaData`/`GetImpliedNodeFormatForEmitWorker`（不可编辑 `internal/ast`）** —— 本轮用可达默认（`false`/CommonJS） | ast/binder 落地后 |
| 真 `.d.ts` emit 签名（`computeSignatureWithDiagnostics`：声明文本 + emit 诊断哈希）+ `emitSignatures` 序列化 | **declarations transformer + checker `EmitResolver`（`.d.ts` emit DEFER）** —— 本轮 `compute_signature` 用文本哈希近似 | declaration emit 轮 |
| `referencedMap` 经 **checker 解析符号**（裸 specifier / triple-slash / type ref / module augmentation / ambient module）`getReferencedFiles` 全量 | **checker `GetSymbolAtLocation` + program resolved-modules 暴露** —— 本轮只跟随**相对** specifier（程序内可达） | checker/program resolution 暴露后 |
| `semanticDiagnosticsPerFile`/`emitDiagnosticsPerFile` 的**缓存诊断对象形**（非裸 fileId）+ `errors`/`semanticErrors`/`checkPending`/`hasErrors` 状态 | **checker 语义诊断 + `ast.Diagnostic` 序列化面（`BuildInfoDiagnostic` category/key/related/repopulateInfo）** —— 本轮只出裸 fileId 形（fresh build 无缓存诊断） | checker 诊断序列化轮 |
| `affectedFilesPendingEmit`/`changeFileSet`/`FileEmitKind`/`getPendingEmitKind`/`latestChangedDtsFile`/`resolvedRoot`/非增量 `Root` 形 | **emit-pending 跟踪 + UpdateProgram（P9 execute build 循环）** | P9 execute |
| `options` 的**完整** `AffectsBuildInfo` 集合 + 声明序 + file-path 相对化（`ForEachCompilerOptionValue` 反射） | **`tsoptions` 反射式 option 遍历（`ForEachCompilerOptionValue`/`AffectsBuildInfo`）** —— 本轮只出 `module`/`target` 可达子集 | tsoptions option-反射轮 |
| `affectsGlobalScope` 触发的 "all files affected" 展开 / `isolatedModules` 快路径 / `assumeChangesOnlyAffectDirectDependencies` / const-enum-export js 失效 / `handleDtsMayChange*` | **declaration emit 签名 + checker const-enum/alias 面** —— 本轮 affected-set 走纯传递闭包（签名变更总传播的等价） | declaration emit / checker 轮 |
| 跨包 `build.getUpToDateStatus` 完整编排（输出文件时间戳、上游 project-reference、pseudo-build、`GetBuildInfoRootInfoReader` 的 root↔resolved 映射） | **`execute/build` orchestrator（P9）** —— 本轮只出 buildinfo-presence + input-newer-than-buildinfo 可达谓词 | P9 execute build |
...
- **project-reference 合并模块（P6-9a）**：Go 把 project-reference 逻辑分散在 `core/projectreference.go`、`host.go`、`projectreferenceparser.go`、`projectreferencefilemapper.go`、`program.go`、`execute/build/orchestrator.go`、`tsoptions/parsedcommandline.go`。Rust 侧把 **P9 要驱动的可达子集**（解析 `references[]` → `ResolvedProjectReferences` 图、拓扑构建序 + TS6202 环检测、TS6306/6310/6053 composite/noEmit/not-found 校验、outDir/rootDir 输出路径、TS6059 rootDir 校验）合并进单一 `projectreference.rs`，每个 fn 仍带 `// Go: <原文件>:<func>` 锚。理由：完整的 `projectReferenceParser`（并行 `WorkGroup` + tracing）/ `projectReferenceFileMapper`（redirect + symlink + faking VFS）/ build 编排需 module redirect + `--build` 执行 + `.tsbuildinfo`，均 DEFER 到 P9/P6-9b；本轮先落地内聚的"解析+序+输出路径"机器。
- **诊断不锚定语法节点（P6-9a）**：Go `verifyProjectReferences` 经 `CreateDiagnosticAtReferenceSyntax` 把 TS6306/6310 锚到 tsconfig `reference` 节点；Rust `ProjectReferenceDiagnostic`（`message`+`args`，`text()` 经 `localize` 格式化）是无位置诊断——message/code/args 与 Go 逐字一致，仅 location 不同。blocked-by: tsconfig option-syntax AST 诊断面。
- **rootDir 诊断码（P6-9a，FOLLOW GO）**：任务 brief 标 "TS6307 file-not-in-rootDir"；Go 的 rootDir 包含检查实际产 **TS6059**（`File '{0}' is not under 'rootDir' '{1}'.`）。TS6307 是另一诊断（"File not listed within the file list of project"，referencing 项目 import 引用项目输出时报，留 P9）。本轮 `check_source_files_belong_to_root_dir` 产 TS6059（与 `cmd/tsgo -p` 实测一致），并在 fn doc 文档化此偏差。
- **解析顺序版（P6-9a，`// PERF(port)`/`DEFER(P9)`）**：Go `projectReferenceParser` 跑在 `core.WorkGroup`（并行）；本轮 `resolve_project_references` 顺序 worklist 按规范路径去重——图与发现顺序无关，故顺序版等价；并行化留 P9。
- **`ParsedCommandLine` 未改（P6-9a）**：`references`/`composite`/`outDir`/`rootDir`/`declarationDir` 已被 `tsoptions::tsconfigparsing` 解析进 `parsed_config.project_references` / `CompilerOptions`；本轮直接消费公开字段，未给 `ParsedCommandLine` 加 `ResolvedProjectReferencePaths`/`ParseInputOutputNames` 方法（这些 Go 方法的可达等价物在 compiler 侧实现，避免改 tsoptions）。

- **增量编译 = 独立 crate `tsgo_incremental`（P6-9b）**：Go 的 `internal/execute/incremental` 是独立包（依赖 `compiler`），故 Rust 侧也拆成独立 crate `tsgo_incremental`（不并入 `tsgo_compiler`），放 `internal/execute/incremental/` 与 Go 源 side-by-side。**未改 compiler `Program`**——增量 `Program` 是一个**消费** `tsgo_compiler::Program` 的 wrapper（`incremental::Program::new(&compiler_program)`），把"建快照→序列化 buildinfo→写/读 `.tsbuildinfo`"作为库基石交付；真正在编译管线里触发 emit/read 的 build 循环属 **P9 execute**（DEFER）。任务 brief 把包位置误标为 `internal/incremental/`——实为 `internal/execute/incremental/`，FOLLOW GO。
- **xxh3 哈希字节对齐（P6-9b）**：Go `incremental.ComputeHash` = `zeebo/xxh3.HashString128(text).Bytes()` 的 hex（128-bit canonical 大端）。Rust 用 workspace 既有的 `xxh3` crate（0.1.1）`hash128_with_seed(bytes,0)` + `format!("{:032x}", u128)`，与 Go 逐字节一致（实抓 buildinfo 的 version-hash 验证）。
- **`affectsGlobalScope`/`impliedNodeFormat` 用可达默认（P6-9b，`// blocked-by:`）**：`program_to_snapshot` 暂用 `affects_global_scope=false`、`impliedNodeFormat=CommonJS`（项目文件的实测默认，使紧凑 fileInfo 出裸字符串与 Go 一致）；真值需 binder module-augmentation 判定 + ast `SourceFileMetaData`（不可编辑 `internal/ast`），DEFER。
- **`referencedMap` 用相对 import 解析（P6-9b，`// blocked-by:`）**：Go `getReferencedFiles` 经 checker `GetSymbolAtLocation` 解析每个 import/triple-slash/type-ref/ambient；本 crate 只跟随**相对** specifier（`./x` → 程序内文件），是 import 图的可达近似；裸 specifier + 非 import 引用 DEFER（checker resolution）。
