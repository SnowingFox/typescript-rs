...
- [ ] `#[repr(i32)] pub enum Category { Warning, Error, Suggestion, Message }`　`// Go: diagnostics.go:Category`
- [ ] `impl Category { pub fn name(&self) -> &'static str }` — `warning/error/suggestion/message`　`// Go: diagnostics.go:Category.Name`
- [ ] `pub type Key = &'static str`（或 newtype）　`// Go: diagnostics.go:Key`
- [ ] `pub struct Message { code: i32, category: Category, key: Key, text: &'static str, reports_unnecessary: bool, elided_in_compatibility_pyramid: bool, reports_deprecated: bool }`　`// Go: diagnostics.go:Message`
- [ ] getters：`code()/category()/key()/reports_unnecessary()/elided_in_compatibility_pyramid()/reports_deprecated()`　`// Go: diagnostics.go:Message.Code 等`
- [ ] `impl fmt::Display for Message`（返回 `text`，调试用）　`// Go: diagnostics.go:Message.String`
- [ ] `impl Message { pub fn localize(&self, locale: Locale, args: &[&str]) -> String }` — 委托 `localize(locale, Some(self), "", args)`　`// Go: diagnostics.go:Message.Localize`
- [ ] `pub fn localize(locale: Locale, message: Option<&'static Message>, key: Key, args: &[&str]) -> String` — message 为 None 时 `key_to_message(key)`；仍为 None 则 `panic("Unknown diagnostic message: ...")`；取译文（命中 locale 表则覆盖 text）→ `format(text, args)`　`// Go: diagnostics.go:Localize`
- [ ] `fn get_localized_messages(tag) -> Option<&'static FxHashMap<Key,String>>` — `Und` 直接 None；查缓存；matcher 匹配 index→`locale_funcs[index]`；存缓存　`// Go: diagnostics.go:getLocalizedMessages`
- [ ] `pub fn format(text: &str, args: &[&str]) -> String` — 无 args 直接返回；args 先 `to_valid_utf8`（U+FFFD 替换）；替换 `{n}`，越界/解析失败 `panic("Invalid formatting placeholder")`　`// Go: diagnostics.go:Format`
- [ ] `pub fn stringify_args(args: &[Arg]) -> Vec<String>` — `string` 原样，否则 `format!("{}", ..)`（对应 Go `fmt.Sprintf("%v")`）；空返回空　`// Go: diagnostics.go:StringifyArgs`
- [ ] locale 缓存静态（`OnceLock<RwLock<FxHashMap<...>>>`）

### `diagnostics_generated.rs`（Go: `internal/diagnostics/diagnostics_generated.go`，生成）

- [ ] ~2153 个 `pub static <NAME>: Message = Message { code, category, key, text, .. };`（由生成器产出）　`// Go: diagnostics_generated.go`
- [ ] `pub fn key_to_message(key: &str) -> Option<&'static Message>`（生成；`match` 或 `phf` 完美哈希）　`// Go: diagnostics_generated.go:keyToMessage`

### `loc_generated.rs`（Go: `internal/diagnostics/loc_generated.go`，生成）

- [ ] `static MATCHER_TAGS: &[&str] = &["en", "zh-CN", "zh-TW", "cs-CZ", "de-DE", "es-ES", "fr-FR", "it-IT", "ja-JP", "ko-KR", "pl-PL", "pt-BR", "ru-RU", "tr-TR"]`（en 在 index 0）　`// Go: loc_generated.go:matcher`
- [ ] `static LOCALE_FUNCS: [..]`（index 0 = None/英语；其余懒加载函数）　`// Go: loc_generated.go:localeFuncs`
- [ ] `fn load_locale_data(data: &[u8]) -> FxHashMap<Key,String>` — flate2 解压 + json 反序列化；失败 `panic`　`// Go: loc_generated.go:loadLocaleData`
- [ ] 14 个 `static XX_DATA: &[u8] = include_bytes!("loc/xx.json.gz");` + `static XX: OnceLock<FxHashMap<Key,String>>`　`// Go: loc_generated.go:zhCN 等`
...
- [ ] `internal/diagnostics/Cargo.toml`（`name = "tsgo_diagnostics"`，deps：`tsgo_core` `tsgo_locale` `tsgo_json` `flate2` `serde` `serde_json` `rustc-hash`）
- [ ] 根 `Cargo.toml` workspace members 追加本 crate
- [ ] `lib.rs`（即 `diagnostics.rs`）声明 `mod diagnostics_generated; mod loc_generated;` + `pub use`
