use super::*;
use crate::registry::{build_index_for_files, FileInput};
use tsgo_core::compileroptions::{
    CompilerOptions, ModuleResolutionKind, ResolutionMode, RESOLUTION_MODE_NONE,
};
use tsgo_modulespecifiers::{HasFileName, SourceOutputAndProjectReference};
use tsgo_packagejson::InfoCacheEntry;
use tsgo_symlinks::{new_known_symlink, KnownSymlinks};
use tsgo_tspath::{to_path, Path};

struct TestHost {
    current_dir: String,
    symlink_cache: KnownSymlinks,
}

impl TestHost {
    fn new(current_dir: &str) -> TestHost {
        TestHost {
            current_dir: current_dir.to_string(),
            symlink_cache: new_known_symlink(current_dir, true),
        }
    }
}

impl ModuleSpecifierGenerationHost for TestHost {
    fn get_symlink_cache(&self) -> Option<&KnownSymlinks> {
        Some(&self.symlink_cache)
    }
    fn common_source_directory(&self) -> String {
        self.current_dir.clone()
    }
    fn get_global_typings_cache_location(&self) -> String {
        String::new()
    }
    fn use_case_sensitive_file_names(&self) -> bool {
        true
    }
    fn get_current_directory(&self) -> String {
        self.current_dir.clone()
    }
    fn get_project_reference_from_source(
        &self,
        _path: &Path,
    ) -> Option<SourceOutputAndProjectReference> {
        None
    }
    fn get_redirect_targets(&self, _path: &Path) -> Vec<String> {
        Vec::new()
    }
    fn get_source_of_project_reference_if_output_included(&self, file: &dyn HasFileName) -> String {
        file.file_name()
    }
    fn file_exists(&self, _path: &str) -> bool {
        true
    }
    fn get_nearest_ancestor_directory_with_package_json(&self, _dirname: &str) -> String {
        String::new()
    }
    fn get_package_json_info(&self, _pkg_json_path: &str) -> Option<&InfoCacheEntry> {
        None
    }
    fn get_default_resolution_mode_for_file(&self, _file: &dyn HasFileName) -> ResolutionMode {
        RESOLUTION_MODE_NONE
    }
}

struct TestSourceFile {
    file_name: String,
}

impl HasFileName for TestSourceFile {
    fn file_name(&self) -> String {
        self.file_name.clone()
    }
    fn path(&self) -> Path {
        to_path(&self.file_name, "/", true)
    }
}

impl SourceFileForSpecifierGeneration for TestSourceFile {
    fn imports(&self) -> Vec<String> {
        Vec::new()
    }
    fn is_js(&self) -> bool {
        false
    }
}

fn bundler_opts() -> CompilerOptions {
    CompilerOptions {
        module_resolution: ModuleResolutionKind::Bundler,
        ..Default::default()
    }
}

// Go: internal/ls/autoimport/view.go:View.GetCompletions (reachable analog)
// Task TDD slice 5: query unresolved `b` from /src/a.ts -> candidate
// { name: "b", specifier: "./b", kind: function }.
#[test]
fn candidate_for_unresolved_function() {
    let files = [
        FileInput::new("/src/a.ts", "import {} from \"\";\n"),
        FileInput::new("/src/b.ts", "export function b(){}\n"),
    ];
    let idx = build_index_for_files(&files, "/", true);
    let host = TestHost::new("/");
    let importing = TestSourceFile {
        file_name: "/src/a.ts".to_string(),
    };

    let candidates = find_import_candidates(
        &idx,
        "b",
        QueryKind::ExactMatch,
        &importing,
        &bundler_opts(),
        &host,
        &UserPreferences::default(),
    );
    assert_eq!(candidates.len(), 1);
    assert_eq!(candidates[0].name, "b");
    assert_eq!(candidates[0].specifier, "./b");
    assert_eq!(candidates[0].kind, ScriptElementKind::FunctionElement);
    assert_eq!(candidates[0].result_kind, ResultKind::Relative);
}

// A query that matches nothing yields no candidates.
#[test]
fn no_candidate_for_unknown_name() {
    let files = [FileInput::new("/src/b.ts", "export function b(){}\n")];
    let idx = build_index_for_files(&files, "/", true);
    let host = TestHost::new("/");
    let importing = TestSourceFile {
        file_name: "/src/a.ts".to_string(),
    };
    let candidates = find_import_candidates(
        &idx,
        "nope",
        QueryKind::ExactMatch,
        &importing,
        &bundler_opts(),
        &host,
        &UserPreferences::default(),
    );
    assert!(candidates.is_empty());
}

// Don't suggest importing a name from the file that is doing the importing.
// Go: internal/ls/autoimport/view.go:View.search (self-import skip)
#[test]
fn does_not_suggest_self_import() {
    let files = [FileInput::new("/src/a.ts", "export function selfFn(){}\n")];
    let idx = build_index_for_files(&files, "/", true);
    let host = TestHost::new("/");
    let importing = TestSourceFile {
        file_name: "/src/a.ts".to_string(),
    };

    // The raw index does contain the export...
    assert_eq!(idx.find("selfFn", true).len(), 1);
    // ...but searching as the importing file excludes it.
    let hits = search_index(&idx, "selfFn", QueryKind::ExactMatch, &importing);
    assert!(hits.is_empty());
    // ...and no candidate is produced.
    let candidates = find_import_candidates(
        &idx,
        "selfFn",
        QueryKind::ExactMatch,
        &importing,
        &bundler_opts(),
        &host,
        &UserPreferences::default(),
    );
    assert!(candidates.is_empty());
}

// Word-prefix search reaches inner words (`Bar` of `fooBar`).
#[test]
fn word_prefix_query() {
    let files = [FileInput::new("/src/b.ts", "export const fooBar = 1;\n")];
    let idx = build_index_for_files(&files, "/", true);
    let host = TestHost::new("/");
    let importing = TestSourceFile {
        file_name: "/src/a.ts".to_string(),
    };
    let candidates = find_import_candidates(
        &idx,
        "bar",
        QueryKind::WordPrefix,
        &importing,
        &bundler_opts(),
        &host,
        &UserPreferences::default(),
    );
    assert_eq!(candidates.len(), 1);
    assert_eq!(candidates[0].name, "fooBar");
    assert_eq!(candidates[0].specifier, "./b");
    assert_eq!(candidates[0].kind, ScriptElementKind::VariableElement);
}
